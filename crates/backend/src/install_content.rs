...
use crate::{
    ...
    metadata::{
        items::{
            ...
            CurseforgeProjectItem, ModrinthProjectVersionsMetadataItem,
...
static FILE_LOCKS: Lazy<Mutex<FxHashMap<Arc<Path>, KeepAliveNotifySignal>>> =
...
impl BackendState {
    pub async fn install_content(
        ...
        let installed_content_ids: Option<Mutex<InstalledContentIds>> =
            if needs_installed_content_ids {
                let mut installed_content_ids = InstalledContentIds::default();

                if let InstallTarget::Instance(instance) = content.target {
                    let content_futures = ContentFolder::iter()
                        .map(|folder| Instance::load_content(self.clone(), instance, folder));
        ...
        let sources = files.iter().filter_map(|install| {
            if install.content_file.content_source != ContentSource::Manual {
                Some((
                    install.hash.clone(),
                    install.content_file.content_source.clone(),
                ))
            } else {
                None
            }
            ...
            for install in files {
                let Some(install_path) = install.install_path else {
                    self.send.send_warning(format!(
                        "Unable to determine install path for {}",
                        install.filename
                    ));
                    continue;
                };
                ...
                match crate::fs::fastcopy(&install.from, &target_path, true, true) {
                    ...
                    Err(err) => {
                        ...
                        let message = format!(
                            "Failed to install content to {}: {err}",
                            target_path.display()
                        );
        ...
        let content_install_file = match content_file.download {
            ContentDownload::Modrinth {
                ...
                if let Some(installed_content_ids) = installed_content_ids {
                    let unique = installed_content_ids
                        .lock()
                        .modrinth_projects
                        .insert(project_id.clone());
                ...
                let version = if let Some(version_id) = version_id {
                    let version = self
                        .meta
                        .fetch(ModrinthVersionMetadataItem(version_id.clone()))
                        .await?;
                    ...
                    let mut result = self
                        .meta
                        .fetch(ModrinthProjectVersionsMetadataItem(
                            &ModrinthProjectVersionsRequest {
                    ...
                    if not_found && modrinth_loader != ModrinthLoader::Unknown {
                        ...
                        result = self
                            ...
                            .fetch(ModrinthProjectVersionsMetadataItem(
                                &ModrinthProjectVersionsRequest {
                                    project_id: project_id.clone(),
                                    game_versions: Some(Arc::new([content
                                        .minecraft_version
                                        .into()])),
                                    loaders: None,
                                },
                    ...
                    tracker.set_finished(ProgressTrackerFinishType::from_err(
                        not_found || result.is_err(),
                    ));
                    drop(tracker);
                ...
                let Some(safe_filename) = SafePath::new(&install_file.filename) else {
                    return Err(ContentInstallError::InvalidFilename(
                ...
                let content_folder_base =
                    if let Some(content_folder) = mod_summary.extra.content_folder() {
                ...
                let install_path = match &content_file.path {
                    ...
                    ContentInstallPath::Safe(safe_path) => {
            ...
            ContentDownload::Curseforge {
                ...
                let mut result = self
                    .meta
                    .fetch(CurseforgeGetModFilesMetadataItem(
            ...
            ContentDownload::File {
                ...
                let title = format!(
                    "Copying {}",
                    copy_path.file_name().unwrap().to_string_lossy()
                );
                let tracker = modal_action.push_tracker(title.into());
                ...
                let mod_summary = {
                    ...
                    tokio::task::spawn_blocking(move || {
                        let valid_hash_on_disk =
                            crate::fs::check_sha1_hash(&path, hash).unwrap_or(false);
                        ...
                        std::io::Result::Ok(
                            mod_metadata_manager.get_bytes(&data, extension.as_deref()),
                        )
                    })
                    .await
                    .unwrap()?
    ...
    fn replace_aux_path(&self, replace: &Path, new_summary: &Arc<ContentSummary>, new_path: &Path) {
        ...
        let Some(old_aux_path) =
            crate::fs::pandora_aux_path(&old_summary.id, &old_summary.name, &replace)
        else {
            return;
        };
        ...
        let Some(new_aux_path) =
            crate::fs::pandora_aux_path(&new_summary.id, &new_summary.name, new_path)
        else {
            _ = std::fs::remove_file(&old_aux_path);
            return;
        };
    ...
    fn replace_shaderpack_settings_path(old_path: &Path, new_path: &Path) {
        ...
        if old_txt != new_txt && old_txt.exists() && !new_txt.exists() {
            if let Err(err) = std::fs::rename(&old_txt, &new_txt) {
                log::error!(
                    "Failed to rename shaderpack settings file from {:?} to {:?}: {err}",
                    old_txt,
                    new_txt
                );
            }
            ...
            if let Ok(files) = files_result {
                ...
                if !manual_download_tasks.is_empty() {
                    let curseforge_projects =
                        futures::future::join_all(manual_download_tasks).await;
    ...
    async fn download_file_into_library_inner(
        ...
        let mut builder = self.http_client_provider.redirecting().get(&*url).header(
            "modrinth-download-meta",
            serde_json::to_string(&download_meta).unwrap_or_default(),
        );
