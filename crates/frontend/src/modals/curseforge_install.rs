...
use schema::{
    ...
    curseforge::{
        CURSEFORGE_RELATION_TYPE_REQUIRED_DEPENDENCY, CurseforgeClassId, CurseforgeFile,
        CurseforgeGetModFilesRequest, CurseforgeGetModFilesResult, CurseforgeHit,
        CurseforgeModLoaderType, CurseforgeReleaseType,
    },
    loader::Loader,
};
    ...
    if let Some(install_for) = install_for {
        ...
        if project_type == CurseforgeClassId::Mod || project_type == CurseforgeClassId::Modpack {
            valid_loader = instance_loader == Loader::Vanilla
                || loaders
                    .loaders
                    .contains(instance_loader.as_curseforge_loader());
        }
        if !valid_loader {
        ...
        let entries: Arc<[InstanceEntry]> = instance_entries
            ...
            .filter_map(|(_, instance)| {
                ...
                if let Some(loaders) = version_matrix.get(minecraft_version) {
                    let mut valid_loader = true;
                    if project_type == CurseforgeClassId::Mod
            ...
            for existing_content in instance.read(cx).content.values() {
                if let Some(existing_content) = existing_content.read(cx) {
                    for summary in existing_content.iter() {
                        let ContentSource::CurseforgeProject {
                            project_id: project,
                    ...
                    .on_click(cx.listener(move |this, _, window, cx| {
                        let path = match this.project_type {
                            CurseforgeClassId::Mod => {
                                RelativePath::new("mods").join(&*selected_file.file_name)
                            }
                            CurseforgeClassId::Modpack => {
                                RelativePath::new("mods").join(&*selected_file.file_name)
                            }
                            CurseforgeClassId::Resourcepack => {
                                RelativePath::new("resourcepacks").join(&*selected_file.file_name)
                            }
                            CurseforgeClassId::Shader => {
                                RelativePath::new("shaderpacks").join(&*selected_file.file_name)
                            }
                            _ => {
                        ...
                        let loader = if let Some(loader) = this.target_loader {
                            ...
                            Loader::iter()
                                ...
                                .find(|loader| {
                                    single_loader_set.contains(loader.as_curseforge_loader())
                                })
        ...
        if self.mod_version_select_state.is_none() {
            ...
            match result {
                ...
                FrontendMetadataResult::Error(shared_string, alive) => {
                    ...
                    return t::instance::content::install::error_loading_files(&shared_string)
                        .into_any_element();
                }
            }
        }
