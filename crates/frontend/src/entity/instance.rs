...
use bridge::{
    ...
    instance::{
        ContentFolder, InstanceContentSummary, InstanceID, InstancePlaytime, InstanceServerSummary,
...
impl InstanceEntries {
    pub fn add(
        ...
        entity.update(cx, |entries, cx| {
            ...
            entries
                .entries
                .insert_before(0, id, cx.new(|_| instance.clone()));
            cx.emit(InstanceAddedEvent { instance });
        });
    }

    pub fn find_title_by_name(
        entity: &Entity<Self>,
        name: &SharedString,
        cx: &App,
    ) -> Option<SharedString> {
...
impl InstanceEntry {
    ...
    fn create_title(&self) -> SharedString {
        ...
        let contains_minecraft_version =
            if let Some(index) = lower.find(self.configuration.minecraft_version.as_str()) {
        ...
        match (contains_loader, contains_minecraft_version) {
            (false, false) => format!(
                "{} ({} {})",
                self.name, loader_string, self.configuration.minecraft_version
            )
            .into(),
