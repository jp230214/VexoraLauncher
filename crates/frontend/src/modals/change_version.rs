...
use schema::{
    ...
    curseforge::{
        CurseforgeChangelogRequest, CurseforgeChangelogResult, CurseforgeFile,
        CurseforgeGetModFilesRequest, CurseforgeGetModFilesResult, CurseforgeModLoaderType,
...
fn is_project(source: &ContentSource) -> bool {
    matches!(
        source,
        ContentSource::ModrinthProject { .. } | ContentSource::CurseforgeProject { .. }
    )
}
    ...
    match project {
        ContentSource::ModrinthProject { project_id } => {
            MetadataRequest::ModrinthProjectVersions(ModrinthProjectVersionsRequest {
                ...
                game_versions: minecraft_version.map(|minecraft_version| {
                    vec![Arc::<str>::from(minecraft_version.as_str())].into()
                }),
...
fn curseforge_version_item(file: &CurseforgeFile) -> VersionItem {
    ...
    let mc_version =
        display_game_version(game_versions).unwrap_or_else(|| t::common::unknown().to_string());
...
fn is_release(item: &VersionItem) -> bool {
    match &item.source {
        VersionSource::Modrinth(version) => {
            version.version_type == Some(ModrinthVersionType::Release)
        }
...
fn is_alpha(item: &VersionItem) -> bool {
    match &item.source {
        VersionSource::Modrinth(version) => {
            version.version_type == Some(ModrinthVersionType::Alpha)
        }
        VersionSource::Curseforge(file) => {
...
pub fn open(
    ...
    let modrinth_loaders = summary
        .content_summary
        .extra
        .modrinth_loaders(loader.as_modrinth_loader());
    let curseforge_loader = summary.content_summary.extra.curseforge_loader();

    let content_name: SharedString = summary
        .content_summary
        .name
        .clone()
...
impl ChangeVersionDialog {
    ...
    fn request_versions(&mut self, cx: &mut Context<Self>) {
        if self.content_source == ContentSource::ModrinthUnknown {
            self.versions_error =
                Some("Unable to get versions for this project. Please reinstall it.".into());
            return;
        }
    ...
    fn process_version_metadata(
        ...
        match &self.content_source {
                    ...
                    FrontendMetadataResult::Loaded(versions) => {
                        self.items = Some(
                            versions
                                .0
                                .iter()
                                .filter_map(modrinth_version_item)
                                .collect(),
                        );
                    }
            ...
            ContentSource::CurseforgeProject { .. } => {
                let result: FrontendMetadataResult<CurseforgeGetModFilesResult> =
                    versions.read(cx).result();
                match result {
                    FrontendMetadataResult::Loaded(result) => {
                        self.items =
                            Some(result.data.iter().map(curseforge_version_item).collect());
                    }
                    FrontendMetadataResult::Loading => {}
                    FrontendMetadataResult::Error(error, alive) => {
                        self.versions_error = Some(error);
    ...
    fn process_changelog_metadata(
        ...
        self.changelog_display = match key {
            VersionKey::Modrinth(_) => {
                let result: FrontendMetadataResult<ModrinthChangelogResult> =
                    changelog.read(cx).result();
                match result {
                    ...
                    FrontendMetadataResult::Loaded(changelog) => ChangelogDisplay::Loaded(
                        changelog
                            ...
                            .map(|changelog| {
                                ChangelogContent::Markdown(SharedString::from(
                                    changelog.to_string(),
                                ))
                            }),
                    ),
            ...
            VersionKey::Curseforge(_) => {
                let result: FrontendMetadataResult<CurseforgeChangelogResult> =
                    changelog.read(cx).result();
                match result {
                    FrontendMetadataResult::Loading => ChangelogDisplay::Loading,
                    FrontendMetadataResult::Loaded(changelog) => ChangelogDisplay::Loaded(
                        changelog
                            ...
                            .map(|data| {
                                ChangelogContent::Html(SharedString::from(data.to_string()))
                            }),
                    ),
                    FrontendMetadataResult::Error(error, alive) => {
                        self.changelog_error = Some(error);
        ...
        let state = if let Some(state) = self.version_select_state.clone() {
            ...
            let state = cx.new(|cx| {
                let mut select_state =
                    SelectState::new(SearchableVec::new(items.clone()), None, window, cx)
                        .searchable(true);
    ...
    fn render_changelog_area(
        &mut self,
        selected_key: Option<&VersionKey>,
        cx: &mut Context<Self>,
    ) -> AnyElement {
        ...
        if let Some(error) = self.changelog_error.clone() {
            return ErrorAlert::new(
                t::instance::content::change_version::error_loading_changelog().into(),
                error,
            )
            .into_any_element();
        }
        ...
        let body = match display {
            ...
            ChangelogDisplay::Loaded(Some(ChangelogContent::Html(changelog))) => {
                TextView::html("changelog", changelog)
                    .scrollable(true)
                    .h_full()
                    .w_full()
                    .into_any_element()
    ...
    fn render(&mut self, modal: Dialog, window: &mut Window, cx: &mut Context<Self>) -> Dialog {
        ...
        let selected_date = selected
            .as_ref()
            .and_then(|selected| selected.date_published.clone());
        ...
        let same_version = selected
            .as_ref()
            .and_then(|selected| selected.sha1.as_deref())
            == Some(self.installed_sha1.as_ref());

        let label: SharedString = match (&selected, is_downgrade) {
            (Some(selected), true) => {
                t::instance::content::change_version::downgrade_to(selected.raw_name.as_str())
                    .into()
            }
            (Some(selected), false) => {
                        ...
                        let install_file = match &selected.source {
                            VersionSource::Modrinth(version) => {
                                ...
                                if hex::decode_to_slice(&*file.hashes.sha1, &mut hash).is_err() {
                                    ...
                                    window
                                        .push_notification((NotificationType::Error, warning), cx);
                                    return;
                                }
                            ...
                            VersionSource::Curseforge(file) => {
                                let Some(sha1) = file
                                    ...
                                    window.push_notification(
                                        (
                                            ...
                                            t::instance::content::install::missing_sha1_hash(),
                                        ),
                                        cx,
                                    );
                                    return;
                                ...
                                let Some(url) = file.download_url.clone() else {
                                    let warning =
                                        t::instance::content::install::no_third_party_downloads();
                                    window
                                        .push_notification((NotificationType::Error, warning), cx);
                                    return;
                                };
        ...
        let changelog_area =
            self.render_changelog_area(selected.as_ref().map(|selected| &selected.key), cx);
