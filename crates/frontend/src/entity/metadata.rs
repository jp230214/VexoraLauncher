...
use schema::{
    ...
    modrinth::{
        ModrinthChangelogResult, ModrinthProjectResult, ModrinthProjectVersionsResult,
    ...
    pub fn as_typeless(self) -> TypelessFrontendMetadataResult {
        match self {
            ...
            FrontendMetadataResult::Error(error, alive) => {
                TypelessFrontendMetadataResult::Error(error, alive)
            }
        }
    }
}
            ...
            fn result(&self) -> FrontendMetadataResult<'_, $t> {
                match self {
                    ...
                    FrontendMetadataState::Loaded { result, alive, .. } => match result {
                        ...
                        Ok(_) => FrontendMetadataResult::Error(
                            t::system::metadata_error().into(),
                            alive.clone(),
                        ),
                        Err(error) => FrontendMetadataResult::Error(
                            SharedString::new(error.clone()),
                            alive.clone(),
                        ),
                    },
                }
