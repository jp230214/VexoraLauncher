            ...
            let _ = cx.update_global(|cache: &mut PngRenderCache, cx| {
                ...
                while let Some(entry) = cursor.get() {
                    if now > entry.expiring.load(Ordering::Relaxed) {
                        ...
                        cache
                            .map
                            .remove(&(entry.source.clone(), entry.transform))
                            .expect("present");
...
impl PngRenderCache {
    fn get_or_create(
        ...
        if let Some(result) = self.map.get(&key) {
            // Update expiry
            result.expiring.store(
                Instant::now() + Duration::from_secs(EXPIRY_SECONDS),
                Ordering::Relaxed,
            );
            unsafe {
                self.expiring
                    .cursor_mut_from_ptr(Rc::as_ptr(result))
                    .remove();
            }
            ...
            match transform {
                ...
                ImageTransformation::ResizeToWidth { width } => {
                    ...
                    if old_width == 0 || old_height == 0 {
                        log::warn!("ResizeToWidth on empty image; skipping transform");
                    } else {
                ...
                ImageTransformation::CropAndScale { min_x, min_y, width, height, scale } => {
                    // Guard against out-of-bounds crops (e.g. small/corrupt capes).
                    // crop_imm panics on OOB, which would crash the whole launcher.
                    let (img_w, img_h) = (image.width(), image.height());
                    if min_x.saturating_add(width) > img_w || min_y.saturating_add(height) > img_h {
                        log::warn!("CropAndScale OOB: image {img_w}x{img_h}, crop ({min_x},{min_y}) {width}x{height}; falling back to resize");
                        let scale_w = width.saturating_mul(scale).max(1);
                        let scale_h = height.saturating_mul(scale).max(1);
                        image = image.resize_exact(scale_w, scale_h, FilterType::Nearest);
                    } else {
