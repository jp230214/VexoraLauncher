...
use crate::{
    ...
    icon::PandoraIcon,
    pages::modrinth_page::{
        InstalledContent, PrimaryAction, env_display, get_primary_action, icon_for,
    },
};
...
impl ModrinthProjectPage {
    pub fn new(
        ...
        let mut specific_installed_content: enum_map::EnumMap<
            ContentFolder,
            ...
            if let Some(entry) = data.instances.read(cx).entries.get(&install_for) {
                ...
                for content_folder in ContentFolder::iter() {
                    ...
                    cx.observe(&content, move |page, entity, cx| {
                        ...
                        for content_folder in ContentFolder::iter() {
                            page.all_installed_content.extend_from_slice(
                                page.specific_installed_content[content_folder].as_slice(),
                            );
                        }
                    })
                    .detach();
    ...
    fn get_primary_action(&self, cx: &App) -> PrimaryAction {
        get_primary_action(
            ...
            Some(&self.all_installed_content),
            cx,
        )
    }
    ...
    fn update_project_from_metadata(
        &mut self,
        state: Entity<FrontendMetadataState>,
        cx: &mut Context<Self>,
    ) {
        self.project = None;
        self.error = None;
    ...
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let content: AnyElement = if let Some(error) = &self.error {
            ...
            let (env_icon, env_name) =
                env_display(project.client_side.unwrap(), project.server_side.unwrap());
            ...
            let categories_el: Option<AnyElement> = {
                ...
                if cats.is_empty() {
                    ...
                    let text = cats
                        .iter()
                        .map(|c| {
                            t::modrinth::category::get(c.as_str(), false).unwrap_or(c.as_str())
                        })
                        .collect::<Vec<_>>()
                        .join(", ");
                    Some(
            ...
            let license_el: Option<AnyElement> = project.license.as_ref().map(|lic| {
                let display_id = match lic.id.as_ref() {
                    ...
                    id if id.contains("LicenseRef") => {
                        id.replace("LicenseRef-", "").replace("-", " ")
                    }
                    id => id.to_string(),
                };
            ...
            let versions_el: Option<AnyElement> = project
                .game_versions
                .as_deref()
                .filter(|v| !v.is_empty())
                .map(|gv| {
            ...
            let body_el: AnyElement = if active_tab == 1 {
                v_flex()
                    ...
                    .child(h_flex().flex_wrap().gap_3().children(
                        gallery.into_iter().flatten().enumerate().map(|(idx, img)| {
                            v_flex()
                                .rounded_lg()
                                .h_80()
                                .child(
                                ...
                                .child(v_flex().p_1().max_w_full().min_w_0().child(
                                    div().text_sm().child(SharedString::new(
                                        img.title.as_deref().unwrap_or_default(),
                                    )),
                                ))
                        }),
                    ))
                    ...
                    h_flex()
                        ...
                        .child(
                            v_flex()
                                ...
                                .child(
                                    div().max_w(rems(40.0)).min_w_0().child(
                                        project
                                            .description
                                            .clone()
                                            .map(SharedString::new)
