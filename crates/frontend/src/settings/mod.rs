...
/// Parse, clamp and commit an integer setting from its text field,
/// then normalize the displayed text back to the stored value.
fn commit_integer(
...
impl SettingsRoot {
    /// Rebuild all setting pages from the current config (e.g. after a
    /// reset) so dropdown selections and input texts reflect the new
    /// values. Keeps the selected page/group and re-applies the search.
    fn rebuild_pages(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        let selected_page = self.settings.selected_page;
        let selected_group = self.settings.selected_group;
        let query: String = self.search_state.read(cx).value().to_string();
        self.settings = create_settings(&self.data, window, cx);
        self.settings.selected_page = selected_page;
        self.settings.selected_group = selected_group;
        if !query.trim().is_empty() {
            self.settings.search(&query);
        }
        cx.notify();
    }

    fn backend_config(&self) -> Option<&BackendConfig> {
        self.temp_backend_config
            .as_ref()
    ...
    pub fn set_proxy_settings(&mut self, proxy_settings: ProxyConfig, cx: &mut Context<Self>) {
        ...
        self.backend_handle
            .send(MessageToBackend::SetProxyConfiguration {
    ...
    pub fn update_backend_configuration(&mut self, cx: &mut Context<Self>) {
        ...
        self.backend_handle
            .send(MessageToBackend::GetBackendConfiguration { channel: send });
    }
}

impl Render for SettingsRoot {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let version = option_env!("VEXORA_RELEASE_VERSION").unwrap_or("Dev");
        ...
        let sidebar = v_flex()
            ...
            .child(
                h_flex()
                    .text_sm()
                    .gap_2()
                    .child(version_icon.clone())
                    .child(version_string.clone()),
            );
...
impl SettingsRoot {
    pub fn render_sidebar_items(&self, cx: &mut Context<Self>) -> Vec<AnyElement> {
        let (pages, mut page_elements) = if let Some(searched) = &self.settings.searched_pages {
            (
                itertools::Either::Left(searched.iter().copied()),
                Vec::with_capacity(searched.len()),
            )
        } else {
            (
        ...
        for page_ix in pages {
            ...
            if self.settings.selected_page == Some(page_ix) {
                ...
                page_element = page_element.hover(|this| {
                    this.bg(theme.sidebar_accent.opacity(0.7))
                        .text_color(theme.sidebar_accent_foreground)
                })
            }
            ...
            if page.groups.len() > 1 {
                let (groups, mut group_elements) = if let Some(searched) = &page.searched_groups {
                    ...
                    (
                        itertools::Either::Right(0..page.groups.len()),
                        Vec::with_capacity(page.groups.len()),
                    )
                };
                ...
                for group_ix in groups {
                    ...
                    if self.settings.selected_page == Some(page_ix)
                        && self.settings.selected_group == Some(group_ix)
                    {
    ...
    pub fn render_page(&mut self, cx: &mut Context<Self>) -> AnyElement {
        ...
        list(
            self.group_list_state.clone(),
            move |mut group_ix, window, cx| {
                cx.update_entity(&entity, |root, cx| {
                    ...
                    let (items, mut item_elements) = if let Some(searched) = &group.searched_items {
                        ...
                        (
                            itertools::Either::Right(0..group.items.len()),
                    ...
                    for item_ix in items {
                        ...
                        let widget = match &item.widget {
                            ...
                            SettingItemWidget::Switch(get, set) => {
                                ...
                                Switch::new((
                                    "switch-element",
                                    page_ix * 10000 + group_ix * 100 + item_ix,
                                ))
                                .checked(value)
                            ...
                            SettingItemWidget::Integer { get, set, min, max } => {
                                ...
                                if initialized {
                                    ...
                                    cx.subscribe_in(
                                        &state,
                                        window,
                                        move |_, entity, event: &InputEvent, window, cx| {
                                            if !matches!(
                                                event,
                                            ...
                                            commit_integer(&entity, get, set, min, max, window, cx);
                                        },
                                    )
                                    .detach();
                                    // The +/- stepper buttons only change the
                                    // text field; without this the new value
                                    // would sit uncommitted until Enter/blur.
                                    // Commit immediately on every step.
                                    cx.subscribe_in(
                                        &state,
                                        window,
                                        move |_, entity, event: &NumberInputEvent, window, cx| {
                                            ...
                                            let current = entity
                                                ...
                                                .unwrap_or_else(|_| (get)(cx));
                                            let mut val = current + delta;
                                            if let Some(min) = min {
                                                val = val.max(min);
                                            }
                                            if let Some(max) = max {
                                            ...
                                            entity.update(cx, |input, cx| {
                                                input.set_value(format!("{val}"), window, cx);
                                            });
                                            (set)(val, cx);
                                        },
                                    )
                                    ...
                                    // Existing state (e.g. after an external
                                    // reset): refresh stale text, but never
                                    // fight the user while typing.
                                    let current = format!("{}", (get)(cx));
                                    let focused = state.focus_handle(cx).is_focused(window);
                                    if !focused && state.read(cx).value().to_string() != current {
                                        state.update(cx, |input, cx| {
                                            input.set_value(current, window, cx);
                                        });
                                    }
                                }
                                gpui_component::input::NumberInput::new(&state)
                                    .w(px(200.0))
                                    .into_any_element()
                            }
                        ...
                        let item_element = h_flex()
                            ...
                            .child(
                                h_flex()
                                    ...
                                    .items_center()
                                    .max_w_2_5()
                                    .when_some(item.reset, |this, reset| {
                                        this.child(
                                            Button::new((
                                                "reset",
                                                page_ix * 10000 + group_ix * 100 + item_ix,
                                            ...
                                            .ghost()
                                            .small()
                                            .tooltip(t::settings::reset_to_default())
                                            .on_click({
                                                let entity = entity.clone();
                                                move |_, window, cx| {
                                                    (reset)(cx);
                                                    _ = entity.update(cx, |root, cx| {
...
struct SettingItem {
    ...
    /// Optional "reset to default" action. Shown as a small button next to
    /// the widget; after running, setting pages are rebuilt so every
    /// control reflects the restored value.
    reset: Option<fn(&mut App)>,
        ...
        if self.last_language_id != Some(lang_id) {
            ...
            for page in &mut self.pages {
                ...
                for group in groups {
                    ...
                    for item in items {
                        item.casefolded_title =
                            Some(casefold::simple_fold((item.title)().to_string()));
                        item.casefolded_description =
                            Some(casefold::simple_fold((item.description)().to_string()));
                    }
                }
            }
        }
...
pub fn open_settings_window(main_window: &Window, data: &DataEntities, cx: &mut App) {
    ...
    _ = cx.open_window(options, |window, cx| {
        let settings_root = cx.new(|cx| {
            ...
            let search_state =
                cx.new(|cx| InputState::new(window, cx).placeholder(t::common::search()));

            cx.subscribe(&search_state, SettingsRoot::on_search)
                .detach();
