    ...
    let notification = notification
        ...
        .content(move |notification, window, cx| {
            ...
            let (mut progress_entries, needs_animation) =
                render_progress_trackers(&modal_action, 0.0);
            if needs_animation {
            ...
            if let Some(visit_url) = modal_action.get_visit_url() {
                ...
                progress_entries.push(
                    div()
                        .p_3()
                        .child(Button::new("visit").success().label(message).on_click(
                            move |_, _, cx| {
                                cx.open_url(&url);
                            },
                        )),
                ...
                .on_focus_out(&focus, cx, {
                    ...
                    move |_, window, _| {
                        if modal_action.get_finished_at().is_some()
                            && modal_action.get_error_message().is_none()
                        {
                            window.remove_window();
                        }
                    }
