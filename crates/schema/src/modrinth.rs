...
#[derive(
    Default, Debug, Copy, Clone, Hash, PartialEq, Eq, PartialOrd, Ord, Serialize, EnumIter,
)]
...
impl ModrinthLoader {
    pub fn install_directory(self) -> Option<&'static str> {
        match self {
            ModrinthLoader::Fabric | ModrinthLoader::Forge | ModrinthLoader::NeoForge => {
                Some("mods")
            }
...
#[derive(Debug, Clone, Deserialize)]
pub struct ModrinthVersionsFromHashesResponse(
    pub std::collections::HashMap<Arc<str>, Option<ModrinthProjectVersion>>,
);
