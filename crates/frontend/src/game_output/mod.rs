        ...
        Self {
            ...
            item_state: Some(GameOutputItemState {
                ...
                cached_shaped_lines: CachedShapedLines {
                    last_time: None,
                    last_time_millis: 0,
                    item_lines: LruCache::with_hasher(
    ...
    pub fn compute_wrapped_text<'a>(
        ...
        if let Some(last_wrapped) = cache.item_lines.get(&self.index)
            && (last_wrapped.wrap_width == wrap_width
                || (last_wrapped.lines.len() == 1
                    && last_wrapped.lines.first().unwrap().width < wrap_width))
        {
            recompute = false;
        }
        ...
        if recompute {
            ...
            for (original_line_index, line) in self.text.iter().enumerate() {
                ...
                let mut handle_segment = |wrapped_line: SharedString, from, to| {
                    let runs: &[TextRun] = if let Some((highlight_line, highlight_range)) =
                ...
                |_, window, cx| {
                    ...
                    cx.update_entity(&self.game_output, |game_output, cx| {
                        ...
                        let mut line_wrapper = window
                            .text_system()
                            .line_wrapper(game_output.font.clone(), font_size);
                            ...
                            && !item_state.items.is_empty()
                        {
                            if scroll_render_info.reverse {
                                paint_lines::<true>(
                                    item_state.items[..scroll_render_info.item + 1]
                                        .iter_mut()
                                        .rev(),
                                    visible_bounds,
                                    bounds,
...
impl GameOutput {
    fn update_scrolling(
        ...
        let max_offset = (item_state.total_line_count * line_height
            - scroll_state.bounds.size.height)
            .max(px(1.0));
            ...
            GameOutputScrolling::Top { offset } => {
                ...
                for check_scrolled_items in [true, false] {
                        ...
                        && let Some(active_drag) = &scroll_state.active_drag
                    {
                        ...
                        let new_max_offset = (item_state.total_line_count * line_height
                            - scroll_state.bounds.size.height)
                            .max(px(1.0));
                        let old_max_offset = (active_drag.start_content_height
                            - scroll_state.bounds.size.height)
                            .max(px(1.0));

                        if offset < drag_pivot {
                            effective_offset = (offset - drag_pivot)
                                / (-old_max_offset - drag_pivot)
                    ...
                    let (item_index, remainder_lines) =
                        item_state.item_sizes.index_of_with_remainder(top_line + 1);
                    ...
                    if check_scrolled_items && item_index < item_state.last_scrolled_item {
                        ...
                        for item in item_state.items[from..item_state.last_scrolled_item].iter_mut()
                        {
                            ...
                            if line_count != item.total_lines {
                                resized_above +=
                                    line_count * line_height - item.total_lines * line_height;
                                if item.total_lines < line_count {
                                    item_state
                                        .item_sizes
                                        .add_at(item.index, line_count - item.total_lines);
                                    item_state.total_line_count += line_count - item.total_lines;
                                } else {
                                    item_state
                                        .item_sizes
                                        .sub_at(item.index, item.total_lines - line_count);
                        ...
                        if changed {
                            if let Some(active_drag) = &mut scroll_state.active_drag {
                                ...
                                if let GameOutputScrolling::Top { offset } =
                                    &mut scroll_state.scrolling
                                {
                                    *offset -= resized_above;
                                }
                            }
                    ...
                    let render_offset =
                        -(remainder_lines * line_height) + line_remainder + line_height
                            - top_offset_for_inset;

                    if scroll_state.active_drag.is_some() {
                        let mut remaining_lines =
                            ((scroll_state.bounds.size.height - render_offset) / line_height)
                                as usize
                                + 1;
                        let mut changed = false;
...
fn paint_lines<'a, const REVERSE: bool>(
    ...
    for item in items {
        ...
        level_origin.x +=
            *time_column_width + level_column_width - item.level.width - font_size / 2.0;
        _ = item
            .level
            .paint(level_origin, line_height, TextAlign::Left, None, window, cx);
...
impl GameOutputRoot {
    ...
    fn on_search_input_event(
        ...
        let item_state = self
            .game_output
            .update(cx, |game_output, _| game_output.item_state.take());
        ...
        if search_pattern.trim().is_empty() {
            self._search_task = cx.spawn_in(window, async move |this, window| {
                ...
                this.update_in(window, |this, window, cx| {
                    ...
                    this.search_state
                        .update(cx, |input, cx| input.set_loading(false, window, cx));
                    cx.notify();
                })
                .unwrap();
            });
                ...
                for item in &mut item_state.items {
                    ...
                    for (line_index, line) in item.text.iter().enumerate() {
                        if let Some(found) = line.find(search_pattern.as_str()) {
                            contains =
                                Some((line_index, found..found + search_pattern.as_str().len()));
                            break;
                        }
                    }
                    if contains.is_some() {
        ...
        v_flex()
            ...
            .on_scroll_wheel(cx.listener(|root, event: &ScrollWheelEvent, _, cx| {
                ...
                if current_offset != new_offset {
                    root.scroll_handler
                        .set_offset(Point::new(Pixels::ZERO, new_offset));
                    cx.notify();
                }
            }))
