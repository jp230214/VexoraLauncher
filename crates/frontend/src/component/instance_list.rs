...
use crate::{
    entity::{
        DataEntities,
        instance::{
            InstanceAddedEvent, InstanceEntry, InstanceModifiedEvent, InstanceRemovedEvent,
        },
    },
...
impl InstanceList {
    pub fn create_table(
        ...
        cx.new(|cx| {
            ...
            let _instance_removed_subscription =
                cx.subscribe::<_, InstanceRemovedEvent>(&instances, |table, _, event, cx| {
                    table
                        .delegate_mut()
                        .items
                        .retain(|instance| instance.id != event.id);
                    cx.notify();
                });
            let _instance_modified_subscription =
                cx.subscribe::<_, InstanceModifiedEvent>(&instances, |table, _, event, cx| {
                    if let Some(entry) = table
                        ...
                        .items
                        .iter_mut()
                        .find(|entry| entry.id == event.instance.id)
                    {
            ...
            let _instance_moved_to_top_subscription =
                cx.subscribe::<_, crate::entity::instance::InstanceMovedToTopEvent>(
                    &instances,
                    |table, _, event, cx| {
                        let delegate = table.delegate_mut();
                        if let Some(pos) = delegate
                            .items
                            .iter()
                            .position(|entry| entry.id == event.instance.id)
                        {
                            let entry = delegate.items.remove(pos);
                            delegate.items.insert(0, entry);
                            cx.notify();
                        }
                    },
                );
    ...
    pub fn render_card(&self, index: usize, cx: &mut App) -> Div {
        ...
        let loader_string = item.configuration.loader.pretty_name();
        let version_string = item.configuration.minecraft_version.as_str();
        let loader_and_version = format!("{} {}", loader_string, version_string);
        ...
        v_flex()
            ...
            .shadow_sm()
            .child(
                h_flex().w_full().gap_4().child(icon).child(
                    v_flex()
                        .truncate()
                        .w_full()
                        .gap_2()
            ...
            .child(div().h(px(1.0)).bg(theme.border).w_full())
            .child(
                h_flex().gap_2().child(play_button.flex_1().small()).child(
                    Button::new(("view", index))
                        .flex_1()
                        .small()
                        .ghost()
    ...
    pub fn render_empty(cx: &mut App) -> Div {
        let theme = cx.theme();
        v_flex()
            .size_full()
            .items_center()
...
impl TableDelegate for InstanceList {
            ...
            match col.key.as_ref() {
                "name" => self.items.sort_by(|a, b| match sort {
                    ColumnSort::Descending => {
                        lexical_sort::natural_lexical_cmp(&a.name, &b.name).reverse()
                    }
    ...
    fn render_td(
        ...
        if let Some(col) = self.columns.get(col_ix) {
            match col.key.as_ref() {
                "name" => {
                    let icon: AnyElement = if let Some(icon_bytes) = item.icon.clone() {
                        let transform = png_render_cache::ImageTransformation::Resize {
                            width: 24,
                            height: 24,
                        };
                        png_render_cache::render_with_transform(icon_bytes, transform, cx)
                            .rounded(cx.theme().radius)
                            .size_6()
                            .min_w_6()
                            .min_h_6()
...
fn render_play_button(item: &InstanceEntry, index: usize, data: DataEntities) -> Button {
    ...
    match item.status {
        ...
        InstanceStatus::Running => Button::new(("kill_instance", index))
            .danger()
