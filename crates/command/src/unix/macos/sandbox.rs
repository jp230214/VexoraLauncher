...
use crate::{
    PandoraChild, PandoraCommand, PandoraSandbox, spawner::SpawnContext,
    unix::unix_helpers::RawStringVec,
};
...
pub fn spawn(
    ...
    if sandbox.is_jvm {
        ...
        if let Ok(temp_path) = Path::new(OsStr::from_bytes(&temp_dir)).canonicalize() {
            ...
            profile
                .push("(allow file-write* file-read* file-map-executable process-exec (prefix \"");
        ...
        if let Some(cache_dir) = confstr(libc::_CS_DARWIN_USER_CACHE_DIR) {
            ...
            if let Ok(cache_path) = cache_path.canonicalize() {
                profile.push(
                    "(allow file-write* file-read* file-map-executable process-exec (subpath \"",
                );
...
fn allow_read(profile: &mut OsString, path: &Path) {
    ...
    if path.is_dir() {
        profile.push(
            "(allow file-read* file-map-executable process-exec file-issue-extension (subpath \"",
        );
    } else {
        profile.push(
            "(allow file-read* file-map-executable process-exec file-issue-extension (literal \"",
        );
    }
    profile.push(path);
    profile.push("\"))\n");
