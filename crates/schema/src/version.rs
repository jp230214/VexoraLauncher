...
impl<'de> Deserialize<'de> for LaunchArgument {
        ...
        D: Deserializer<'de>,
    {
        serde_untagged::UntaggedEnumVisitor::new()
            .string(|single| {
                Ok(LaunchArgument::Single(LaunchArgumentValue::Single(
                    Ustr::from(single),
                )))
            })
    ...
    pub fn apply_to(self, other: &MinecraftVersion) -> MinecraftVersion {
        ...
        if let Some(new_arguments) = self.arguments {
            if let Some(curr_arguments) = &mut version.arguments {
                curr_arguments.game = curr_arguments
                    .game
                    .iter()
                    .chain(new_arguments.game.iter())
                    .cloned()
                    .collect();
                curr_arguments.jvm = curr_arguments
                    .jvm
                    .iter()
                    .chain(new_arguments.jvm.iter())
                    .cloned()
                    .collect();
            } else {
