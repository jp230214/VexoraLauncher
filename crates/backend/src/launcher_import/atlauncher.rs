...
use schema::{
    instance::{
        InstanceConfiguration, InstanceMemoryConfiguration, InstanceWrapperCommandConfiguration,
    },
    loader::Loader,
};
...
pub async fn import_from_atlauncher(
    ...
    let Ok(launcher_config_bytes) = std::fs::read(import_job.root.join("configs/ATLauncher.json"))
    else {
        modal_action.set_finished_with_error("Unable to find configs/ATLauncher.json file".into());
        return;
    };
    let launcher_config = serde_json::from_slice::<AtLauncherConfig>(&launcher_config_bytes)
        .expect("Failed to parse to json");

    let accounts =
        import_accounts_from_atlauncher(backend, &import_job, &launcher_config, &modal_action)
            .await;
    import_instances_from_atlauncher(
        backend,
        &import_job,
        &launcher_config,
        &modal_action,
...
async fn import_accounts_from_atlauncher(
    ...
    let Ok(accounts_json) = serde_json::from_slice::<Vec<AtLauncherAccount>>(&accounts_bytes)
    else {
        return None;
    };

    let secret_storage = match backend
        .secret_storage
    ...
    for account in &accounts_json {
        ...
        if non_default_creds {
            _ = secret_storage
                .write_credentials(account.uuid, &credentials)
                .await;
        }
    }
...
fn try_load_from_atlauncher(
    ...
    configuration.memory = if let Some(max_memory) = instance_cfg
        .launcher
        .maximum_memory
        .or(launcher_config.maximum_memory)
    {
    ...
    if let Some(enable_commands) = instance_cfg.launcher.enable_commands
        && enable_commands
    {
        configuration.wrapper_command =
            if let Some(wrapper_command) = instance_cfg.launcher.wrapper_command {
...
fn import_instances_from_atlauncher(
    ...
    for to_import in to_import {
        let title = format!(
            "Importing {}",
            to_import.folder.file_name().unwrap().to_string_lossy()
        );
        let tracker = modal_action.push_tracker(title.into());

        let Ok(configuration) =
            try_load_from_atlauncher(&to_import.config_path, launcher_config, accounts)
        else {
        ...
        _ = std::fs::rename(
            &target_dot_minecraft.join("instance.png"),
