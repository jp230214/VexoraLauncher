...
use windows::Win32::{
    Foundation::HANDLE, System::JobObjects::AssignProcessToJobObject, UI::Shell::SHELLEXECUTEINFOW,
};

use crate::{
    PandoraChild, PandoraCommand, process::PandoraProcess, spawner::SpawnContext,
    windows::windows_helpers,
};
...
pub fn spawn(command: PandoraCommand, context: &mut SpawnContext) -> std::io::Result<PandoraChild> {
    if command.inherit_env.is_some() || !command.env.is_empty() {
        return Err(Error::new(
            ...
            "cannot set custom environment for elevated process",
        ));
    }

    if context.job_handle.is_none() {
        context.job_handle = Some(windows_helpers::to_owned_handle(
            windows_helpers::create_job_object()?,
        )?);
    }
