...
pub fn import_instances_from_modrinth(
    ...
    let mut stmt =
        if table_exists(&conn, "instances") && table_exists(&conn, "instance_content_sets") {
            conn.prepare(
            ...
            if let Ok(icon_bytes) = std::fs::read(icon_path) {
                if let Ok(format) = image::guess_format(&icon_bytes) {
                    if format == ImageFormat::Png {
                        _ = crate::fs::write_safe(
                            &to_import.pandora_path.join("icon.png"),
                            &icon_bytes,
                        );
                    } else if let Ok(image) =
                        image::load_from_memory_with_format(&icon_bytes, format)
                    {
...
pub fn read_profiles_from_modrinth_db(modrinth: &Path) -> rusqlite::Result<Option<Vec<Arc<Path>>>> {
    ...
    let custom_dir = conn
        .query_one("SELECT custom_dir FROM settings", [], |row| {
            row.get::<_, String>(0)
        })
        .ok();
    ...
    if let Some(custom_dir) = custom_dir {
        ...
        if custom_dir_path != modrinth {
            log::info!(
                "Changing import root to {:?} because of custom_dir",
                custom_dir_path
            );
    ...
    Ok(Some(paths_from_query(
        profile_dir_main,
        profile_dir_fallback,
        query,
    )?))
}
