...
use gpui_component::{
    ...
    h_flex,
    input::{
        Input, InputEvent, InputState, NumberInput, NumberInputEvent, Textarea, TextareaState,
    },
...
use schema::{
    ...
    instance::{
        ...
        InstanceJvmFlagsConfiguration, InstanceLinuxWrapperConfiguration,
        InstanceMemoryConfiguration, InstanceSystemLibrariesConfiguration,
        InstanceWrapperCommandConfiguration, LwjglLibraryPath, UpdateChannel,
    },
    loader::Loader,
...
impl InstanceSettingsSubpage {
    pub fn new(
        ...
        let sandbox_available = if cfg!(target_os = "linux") {
            command::is_command_available("bwrap")
                && command::is_command_available("xdg-dbus-proxy")
        } else {
            true
        };

        let memory = entry.configuration.memory.unwrap_or_default();
        let wrapper_command = entry
            .configuration
            .wrapper_command
            .clone()
            .unwrap_or_default();
        ...
        let openal_path = system_libraries
            .openal
            .get_or_auto(&*AUTO_LIBRARY_PATH_OPENAL);

        let new_name_input_state =
            cx.new(|cx| InputState::new(window, cx).default_value(instance_name));
        cx.subscribe(&new_name_input_state, Self::on_new_name_input)
            .detach();

        let version_select_state = cx
            .new(|cx| SelectState::new(VersionList::default(), None, window, cx).searchable(true));
        cx.subscribe(&version_select_state, Self::on_minecraft_version_selected)
            .detach();
        ...
        .detach();
        cx.subscribe(&account_items, Self::on_account_selected)
            .detach();
        ...
        cx.subscribe_in(&loader_select_state, window, Self::on_loader_selected)
            .detach();
        ...
        cx.observe_in(instance, window, |page, instance, window, cx| {
                ...
                .is_none()
            {
                let version = entry
                    ...
                    .map(|s| s.as_str())
                    .unwrap_or(loader_version_latest_string);
                page.loader_version_select_state
        ...
        cx.subscribe(
            &loader_version_select_state,
            Self::on_loader_version_selected,
        )
        .detach();
        ...
        cx.subscribe(
            ...
            Self::on_update_channel_selected,
        )
        .detach();

        let memory_min_input_state =
            cx.new(|cx| InputState::new(window, cx).default_value(memory.min.to_string()));
        cx.subscribe_in(&memory_min_input_state, window, Self::on_memory_step)
            .detach();
        cx.subscribe(&memory_min_input_state, Self::on_memory_changed)
            .detach();
        let memory_max_input_state =
            cx.new(|cx| InputState::new(window, cx).default_value(memory.max.to_string()));
        cx.subscribe_in(&memory_max_input_state, window, Self::on_memory_step)
            .detach();
        cx.subscribe(&memory_max_input_state, Self::on_memory_changed)
            .detach();
        ...
        cx.subscribe(
            ...
            Self::on_wrapper_command_changed,
        )
        .detach();
        ...
        cx.subscribe(&jvm_flags_input_state, Self::on_jvm_flags_changed)
            .detach();
        ...
        let mut page = Self {
            ...
            jvm_binary_path: jvm_binary
                .path
                .clone()
                .map(|path| PathLabel::new(path, false)),
...
impl InstanceSettingsSubpage {
    fn update_minecraft_versions(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        self._observe_minecraft_version_subscription = Some(cx.observe_in(
            &minecraft_versions,
            window,
            |page, minecraft_versions, window, cx| {
    ...
    fn process_minecraft_version_metadata(
        ...
        let result: FrontendMetadataResult<MinecraftVersionManifest> =
            minecraft_versions.read(cx).result();
        let versions = match &result {
            ...
            FrontendMetadataResult::Error(_, alive) => {
                if let Some(alive) = alive.clone() {
                    self._minecraft_version_retry_task =
            ...
            FrontendMetadataResult::Loaded(manifest) => manifest
                .versions
                .iter()
    ...
    fn update_loader_versions(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        self.loader_version_select_state
            .update(cx, move |select_state, cx| {
    ...
    pub fn on_minecraft_version_selected(
        ...
        self.backend_handle
            .send(MessageToBackend::SetInstanceMinecraftVersion {
                id: self.instance_id,
    ...
    pub fn on_account_selected(
        ...
        self.backend_handle
            .send(MessageToBackend::SetInstancePreferredAccount {
                id: self.instance_id,
                account: value.clone(),
            });
    }
    ...
    pub fn on_update_channel_selected(
        ...
        self.backend_handle
            .send(MessageToBackend::SetInstanceUpdateChannel {
                id: self.instance_id,
    ...
    pub fn on_loader_selected(
        ...
        if self.loader != loader {
            self.loader = loader;
            self.backend_handle
                .send(MessageToBackend::SetInstanceLoader {
                    id: self.instance_id,
                    loader: self.loader,
                });
    ...
    pub fn on_loader_version_selected(
        ...
        self.backend_handle
            .send(MessageToBackend::SetInstancePreferredLoaderVersion {
                id: self.instance_id,
    ...
    pub fn on_memory_changed(
        ...
        if let InputEvent::Change = event {
            self.backend_handle
                .send(MessageToBackend::SetInstanceMemory {
                    id: self.instance_id,
    ...
    pub fn on_wrapper_command_changed(
        ...
        if let InputEvent::Change = event {
            self.backend_handle
                .send(MessageToBackend::SetInstanceWrapperCommand {
                    id: self.instance_id,
    ...
    pub fn on_jvm_flags_changed(
        ...
        if let InputEvent::Change = event {
            self.backend_handle
                .send(MessageToBackend::SetInstanceJvmFlags {
                    id: self.instance_id,
    ...
    fn create_lwjgl_library_path(
        path: &Option<Arc<Path>>,
        auto: &Option<Arc<Path>>,
    ) -> LwjglLibraryPath {
...
impl Render for InstanceSettingsSubpage {
    fn render(
        ...
        let icon_element: Option<AnyElement> = self.icon.clone().map(|icon| match icon {
            EmbeddedOrRaw::Embedded(path) => Icon::default()
                .path(path)
                .size_8()
                .min_w_8()
                .min_h_8()
                    ...
                    .when(
                        ...
                        |this| {
                            if self.new_name_change_state == NewNameChangeState::InvalidName {
                                ...
                                this.child(
                                    Button::new("setname").label(t::common::update()).on_click({
                                ...
                                crate::modals::select_icon::open_select_icon(
                                    Box::new(move |icon, cx| {
                                        cx.update_entity(&entity, |this, _| {
                                            ...
                                            this.backend_handle.send(
                                                MessageToBackend::SetInstanceIcon {
                                                    id: this.instance_id,
                                                    icon: Some(icon),
                                                },
                                            );
        ...
        match self.version_state {
            TypelessFrontendMetadataResult::Loading => {
                version_content = version_content
                    .child(Skeleton::new().w_full().min_h_8().max_h_8().rounded_md());
            }
            TypelessFrontendMetadataResult::Loaded => {
                version_content = version_content.child(
                    Select::new(&self.version_select_state)
            ...
            TypelessFrontendMetadataResult::Error(ref error, ..) => {
                version_content = version_content.child(format!(
                    "{}: {}",
                    t::instance::versions_loading::error(),
                    error
                ))
            }
        }
                        ...
                        Select::new(&self.loader_version_select_state)
                            ...
                            .title_prefix(match self.loader {
                                Loader::Fabric => format!(
                                    "{}: ",
                                    t::instance::loader_version(t::modrinth::category::fabric())
                                ),
                                Loader::Forge => format!(
                                    "{}: ",
                                    t::instance::loader_version(t::modrinth::category::forge())
                                ),
                                Loader::NeoForge => format!(
                                    "{}: ",
                                    t::instance::loader_version(t::modrinth::category::neoforge())
                                ),
                                Loader::Vanilla => format!(
                                    "{}: ",
                                    t::instance::loader_version(t::instance::loader())
                                ),
                            })
                            .w_full(),
                    )
            ...
            .child(crate::labelled(
                ...
                Checkbox::new("syncing")
                    ...
                    .on_click(cx.listener(|page, value, _, _| {
                        ...
                        page.backend_handle
                            .send(MessageToBackend::SetInstanceDisableFileSyncing {
                                id: page.instance_id,
            ...
            .child(crate::labelled(
                ...
                Checkbox::new("sandbox")
                    ...
                    .on_click(cx.listener(|page, value, _, _| {
                        page.sandbox = *value;
                        page.backend_handle
                            .send(MessageToBackend::SetInstanceSandboxing {
                                id: page.instance_id,
                                sandbox: *value,
                            });
                    })),
        ...
        let runtime_content = v_flex()
                            ...
                            .on_click(cx.listener(|page, value, _, cx| {
                                if page.jvm_flags_enabled != *value {
                                    ...
                                    page.backend_handle.send(
                                        MessageToBackend::SetInstanceJvmFlags {
                                            id: page.instance_id,
                        ...
                        Checkbox::new("jvm_binary")
                            ...
                            .on_click(cx.listener(|page, value, _, cx| {
                                if page.jvm_binary_enabled != *value {
                                    ...
                                    page.backend_handle.send(
                                        MessageToBackend::SetInstanceJvmBinary {
                                            id: page.instance_id,
                        ...
                        PathLabel::button_opt(&self.jvm_binary_path, "select_jvm_binary")
                            ...
                            .on_click(cx.listener(|this, _, window, cx| {
                                this.select_file(
                                    ...
                                    |this, path| {
                                        this.jvm_binary_path =
                                            path.map(|path| PathLabel::new(path, false));
                                        this.backend_handle.send(
                                            MessageToBackend::SetInstanceJvmBinary {
                                                id: this.instance_id,
                                ...
                                if page.override_glfw_enabled != *value {
                                    ...
                                    page.backend_handle.send(
                                        MessageToBackend::SetInstanceSystemLibraries {
                                            id: page.instance_id,
                                            system_libraries: page
                                                .get_system_libraries_configuration(),
                                        },
                                    );
                                    cx.notify();
                                }
                        ...
                        PathLabel::button_opt(&self.override_glfw_path, "select_glfw")
                            ...
                            .on_click(cx.listener(|this, _, window, cx| {
                                this.select_file(
                                    ...
                                    |this, path| {
                                        this.override_glfw_path =
                                            path.map(|path| PathLabel::new(path, false));
                                        this.backend_handle.send(
                                            MessageToBackend::SetInstanceSystemLibraries {
                                                id: this.instance_id,
                                                system_libraries: this
                                                    .get_system_libraries_configuration(),
                                            },
                                        );
                                    },
            ...
            .child(
                v_flex()
                                ...
                                if page.wrapper_command_enabled != *value {
                                    ...
                                    page.backend_handle.send(
                                        MessageToBackend::SetInstanceWrapperCommand {
                                            id: page.instance_id,
                                            wrapper_command: page
                                                .get_wrapper_command_configuration(cx),
                                        },
                                    );
                                    cx.notify();
                                }
                    ...
                    .child(
                        Textarea::new(&self.wrapper_command_input_state)
                            .disabled(!wrapper_command_enabled),
                    ),
            );
                    ...
                    Checkbox::new("use_mangohud")
                        ...
                        .on_click(cx.listener(|page, value, _, cx| {
                            if page.use_mangohud != *value {
                                ...
                                page.backend_handle.send(
                                    MessageToBackend::SetInstanceLinuxWrapper {
                                        id: page.instance_id,
                    ...
                    .on_click({
                        ...
                        move |_: &ClickEvent, _, cx| {
                            ...
                            cx.spawn(async move |_| {
                                ...
                                backend_handle
                                    .send(MessageToBackend::CreateInstanceShortcut { id, path });
                            })
                            .detach();
                        }
                    }),
                ...
                Button::new("delete")
                    ...
                    .on_click({
                        ...
                        move |click: &ClickEvent, window, cx| {
                            ...
                            if InterfaceConfig::get(cx).quick_delete_instance
                                && click.modifiers().shift
                            {
                                backend_handle
                                    .send(bridge::message::MessageToBackend::DeleteInstance { id });
                            } else {
