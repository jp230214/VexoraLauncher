...
fn find_content_library_path(
    ...
    let disabled_extension = path
        ...
        .and_then(|s| s.to_str());
    let lib_path =
        crate::fs::create_content_library_path(content_library_dir, hash, disabled_extension);
        ...
        for entry in read_dir {
            ...
            let Ok(relative) = path.strip_prefix(&from) else {
                return Err(Error::new(
                    ErrorKind::Other,
                    format!("{path:?} is not a child of {from:?}"),
                ));
            };
            #[cfg(windows)]
            ...
            && source_metadata.number_of_links() > 1
        {
            if let Ok(hash) = hash_file(source_path, &mut buf, check_cancel) {
                if let Some(lib_path) =
                    find_content_library_path(content_library_dir, hash, source_path)
                {
...
pub async fn duplicate_instance(
    ...
    if !crate::fs::is_single_component_path_str(name) {
        modal_action.set_finished_with_error(
            format!("Unable to duplicate instance, name must not be a path: {name}").into(),
        );
        return;
    }
    if !sanitize_filename::is_sanitized_with_options(
        ...
        modal_action.set_finished_with_error(
            format!("Unable to duplicate instance, name is invalid: {name}").into(),
        );
        return;
    }
    if backend
    ...
    {
        modal_action.set_finished_with_error(
            "Unable to duplicate instance, name is already used"
                .to_string()
                .into(),
        );
        return;
    ...
    let source = {
        ...
        let Some(instance) = state.instances.get(id) else {
            modal_action.set_finished_with_error(
                "Unable to duplicate instance, unknown id"
                    .to_string()
                    .into(),
            );
            return;
    ...
    if let Err(err) = fs::create_dir(&dest) {
        modal_action
            .set_finished_with_error(format!("Unable to create instance directory: {err}").into());
        return;
    }
