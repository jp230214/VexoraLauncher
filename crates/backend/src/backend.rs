...
use bridge::{
    ...
    instance::{
        ContentFolder, ContentType, InstanceContentSummary, InstanceID, ModpackFile,
        ModpackFilePath, ModpackFileSource,
    },
...
use crate::{
    ...
    metadata::{
        items::{
            CurseforgeGetFilesMetadataItem, CurseforgeProjectItem,
    ...
    pub fn build(
        ...
        let mut http_builder = (base)().redirect(Policy::custom(|attempt| {
            ...
            if let Some(last) = attempt.previous().last() {
                ...
                if from != to {
                    let error_message =
                        format!("Cross-origin redirect not allowed ({} to {})", from, to);
        ...
        let redirecting_http_client = redirecting_builder
            .build()
            .expect("Failed to build redirecting HTTP client");
...
pub fn start(
    ...
    let http_client_provider = HttpClientProvider::create(
        &*schema::USER_AGENT,
        &proxy_config,
    ...
    let (watcher_tx, watcher_rx) =
        tokio::sync::mpsc::channel::<notify_debouncer_full::DebounceEventResult>(64);
    let watcher =
        notify_debouncer_full::new_debouncer(Duration::from_millis(100), None, move |event| {
            let _ = watcher_tx.blocking_send(event);
        })
        .unwrap();

    let mod_metadata_manager = ModMetadataManager::load(
        directories.content_meta_dir.clone(),
        directories.content_library_dir.clone(),
    );
    ...
    state_file_watching
        .watch_filesystem(directories.root_launcher_dir.clone(), WatchTarget::RootDir);
    ...
    runtime.block_on(async {
        state
            .send
            .send(state.account_info.write().get().create_update_message());
...
impl BackendState {
    ...
    pub async fn load_all_instances(&self) {
        ...
        for (path, _) in paths_with_time {
            ...
            if !success {
                self.file_watching
                    .write()
                    .watch_filesystem(path.into(), WatchTarget::InvalidInstanceDir);
            }
        }
    }
    ...
    pub fn remove_instance(&self, id: InstanceID) {
        ...
        if let Some(instance) = instance_state.instances.remove(id) {
            ...
            self.send
                .send_info(format!("Instance '{}' removed", instance.name));
        }
    }
    ...
    pub fn load_instance_from_path(
        ...
        let instance_id = {
            ...
            let Ok(mut instance) = instance else {
                instance_state.instances.retain_mut(|existing| {
                    if &*existing.root_path == path {
                        self.send
                            .send(MessageToFrontend::InstanceRemoved { id: existing.id });
                        show_errors = true;
                        false
                    } else {
                        true
            ...
            for existing in instance_state.instances.iter_mut() {
                ...
                if show_success {
                    self.send
                        .send_info(format!("Instance '{}' updated", existing.name));
                }
            ...
            instance_state.instances_generation =
                instance_state.instances_generation.wrapping_add(1);
            ...
            if show_success {
                self.send
                    .send_success(format!("Instance '{}' created", instance.name));
            }
        ...
        for instance in instance_state.instances.iter_mut() {
            ...
            instance
                ...
                .retain_mut(|process| match process.try_wait() {
                    ...
                    Err(err) => {
                        log::error!(
                            "An error occured while waiting for process {}: {:?}",
                            process.id(),
                            err
                        );
                        killed = true;
            ...
            instance
                .closing_processes
                .retain_mut(|(process, _)| match process.try_wait() {
                    Ok(None) => true,
                    Ok(Some(status)) => {
                    ...
                    Err(err) => {
                        log::error!(
                            "An error occured while waiting for closing process {}: {:?}",
                            process.id(),
                            err
                        );
                        killed = true;
            ...
            let now = Instant::now();
            let to_kill = instance
                .closing_processes
                .extract_if(.., |(_, deadline)| now > *deadline);
            ...
            self.restore_mods_folder_if_stopped(instance);
            any_process_alive |=
                !instance.processes.is_empty() || !instance.closing_processes.is_empty();
        }
    ...
    pub async fn update_http_clients(&self) {
        ...
        self.http_client_provider.update(
            &*schema::USER_AGENT,
            &proxy_config,
            proxy_password.as_deref(),
        );
        ...
        loop {
            ...
            if let Some(last_stage) = last_auth_stage {
                if stage > last_stage {
                    ...
                    log::error!(
                        "Stage {:?} didn't change. This is most likely a bug with the auth flow!",
                        stage
                    );
            ...
            match credentials.stage() {
                ...
                auth::credentials::AuthStageWithData::MsaRefresh(refresh) => {
                    ...
                    match authenticator
                        .refresh_msa(&refresh, &credentials.msa_refresh_force_client_id)
                        .await
                    {
                        Ok(Some(msa_tokens)) => {
                            credentials.msa_access = Some(msa_tokens.access);
                        ...
                        Err(error) => {
                            ...
                            if !matches!(error, MsaAuthorizationError::InvalidGrant) {
                                log::warn!(
                                    "Error using msa refresh to get msa access: {:?}",
                                    error
                                );
                            }
                            credentials.msa_refresh = None;
                ...
                auth::credentials::AuthStageWithData::MsaAccess(access) => {
                    ...
                    match authenticator.authenticate_xbox(&access).await {
                        ...
                        Err(error) => {
                            ...
                            if !matches!(
                                error,
                                XboxAuthenticateError::NonOkHttpStatus(StatusCode::UNAUTHORIZED)
                            ) {
                    ...
                    match authenticator.authenticate_minecraft(&xsts, &userhash).await {
                        ...
                        Err(error) => {
                            ...
                            if !matches!(
                                ...
                                log::warn!(
                                    "Error using xsts to get minecraft access token: {:?}",
                                    error
                                );
                            }
    ...
    pub fn apply_syncing_to_instance(&self, id: InstanceID) {
        ...
        let (disable, path) = if let Some(instance) = instances.instances.get_mut(id) {
            (
                instance.configuration.get().disable_file_syncing,
    ...
    pub fn restore_mods_folder_if_stopped(&self, instance: &mut Instance) {
        ...
        if let Err(err) = std::fs::rename(&original_mods_dir, &mod_dir) {
            self.send
                .send_error(format!("Unable to restore mods directory: {}", err));
    ...
    pub async fn prelaunch_setup_mods(
        ...
        let mut known_files: FxHashSet<Arc<Path>> =
            FxHashSet::with_capacity_and_hasher(mods.len() * 2, FxBuildHasher);
        ...
        if !other_files.is_empty() {
            let tracker =
                modal_action.push_tracker("Copying extra files into mods directory".into());
            ...
            for other_file in other_files {
                ...
                if from.is_dir() {
                    let name = other_file
                        ...
                        .unwrap_or_default();
                    let inner =
                        modal_action.push_tracker(format!("Copying '{}' folder", name).into());

                    _ = std::fs::create_dir_all(&to);
                    let res =
                        crate::fs::copy_content_recursive(&from, &to, false, &|count, total| {
    ...
    pub async fn apply_modpack_and_collect_mods(
        ...
        for summary in &*mods {
            ...
            let content_summary = if self
                .download_modpack_children(summary, loader, minecraft_version, modal_action)
                .await
            {
            ...
            let modpack = match &content_summary.extra {
                ...
                ContentType::CurseforgeModpack {
                    ...
                    if !unknown_files.is_empty() {
                        log::warn!(
                            "Unable to resolve some curseforge files, mods might be missing from modpack"
                        );
                    }
                    Some(files)
                }
            ...
            if let Some(files) = modpack {
                let filtered_files = files
                    ...
                    .filter(|dl| {
                        ...
                        summary.disabled_children.is_enabled(
                            dl.default_disabled,
                            id,
                            name,
                            dl.path.as_str(),
        ...
        for modpack_install in modpack_installs {
            let content_library_dir = &self.directories.content_library_dir.clone();
            let mut aux: Option<AuxiliaryContentMeta> =
            ...
            if !modpack_install.files.is_empty() {
                ...
                for file in modpack_install.files {
                    let Some(rel_path) = file.path() else {
                        log::warn!(
                            "Skipping mod {:?} because path cannot be determined",
                            file.path
                        );
                        continue;
                    };
        ...
        for mod_copy in mod_copies {
            ...
            match mod_copy.source {
                PrelaunchModCopySource::FromContentLibrary { hash } => {
                    ...
                    let path = crate::fs::create_content_library_path_osstrext(
                        &content_library_dir,
                        hash,
                        extension,
                    );
                    ...
                    if !path.exists() {
                        log::error!(
                            "Unable to copy from content library because path doesn't exist: {:?}",
                            path
                        );
                    } else if let Err(err) = crate::fs::fastcopy(&path, &target_path, true, false) {
                        log::error!(
                            "Error copying mod from {:?} to {:?}:\n{:?}",
                            path,
                            target_path,
                            err
                        );
    ...
    pub async fn download_modpack_children(
        ...
        if !curseforge_file_ids.is_empty() {
            let tracker =
                modal_action.push_tracker("Requesting download URLs from CurseForge".into());
            tracker.set_total(1);
            ...
            if let Ok(files) = files_result {
                for file in files.data.iter() {
                    ...
                    let Some(filename) = SafePath::new(&file.file_name) else {
                        log::warn!(
                            "Skipping file because of invalid filename: {}",
                            file.file_name
                        );
                        continue;
                    };
                    ...
                    if let Some(download_url) = &file.download_url {
                        content_install_files.push(ContentInstallFile {
                            replace_old: None,
                            path: ContentInstallPath::ModpackFilePath(ModpackFilePath::Filename(
                                filename,
                            )),
                            download: ContentDownload::Url {
            ...
            if !manual_download_tasks.is_empty() {
                ...
                let zipped = curseforge_projects
                    .into_iter()
                    .zip(manual_download_files.into_iter());
        ...
        if !content_install_files.is_empty() {
            ...
            let tracker = modal_action.push_tracker("Waiting for manual downloads".into());
            tracker.add_total(1);
            self.create_manual_curseforge_download_session(manual_downloads.clone())
                .await;
            tracker.add_count(1);
            tracker.set_finished(ProgressTrackerFinishType::Normal);
    ...
    pub async fn create_instance(
        ...
        if !crate::fs::is_single_component_path_str(&name) {
            self.send.send_warning(format!(
                "Unable to create instance, name must not be a path: {}",
                name
            ));
            return None;
        }
        if !sanitize_filename::is_sanitized_with_options(
            ...
            self.send.send_warning(format!(
                "Unable to create instance, name is invalid: {}",
                name
            ));
            return None;
        }
        ...
        {
            self.send
                .send_warning("Unable to create instance, name is already used".to_string());
            return None;
        }
        ...
        crate::fs::write_safe(
            &info_path,
            serde_json::to_string(&instance_info).unwrap().as_bytes(),
        )
        .unwrap();
    ...
    pub async fn rename_instance(self: &Arc<Self>, id: InstanceID, name: &str) {
        if !crate::fs::is_single_component_path_str(&name) {
            self.send.send_warning(format!(
                "Unable to rename instance, name must not be a path: {}",
                name
            ));
            return;
        }
        if !sanitize_filename::is_sanitized_with_options(
            ...
            self.send.send_warning(format!(
                "Unable to rename instance, name is invalid: {}",
                name
            ));
            return;
        }
        ...
        {
            self.send
                .send_warning("Unable to rename instance, name is already used".to_string());
            return;
        }
        ...
        if let Some(instance) = self.instance_state.write().instances.get_mut(id) {
            ...
            if let Err(err) = std::fs::rename(&instance.root_path, new_instance_dir) {
                self.send
                    .send_error(format!("Unable to rename instance folder: {}", err));
            }
        }
    }
    ...
    pub async fn get_login_info(
        ...
        let Some((profile, access_token)) = self.login_flow(modal_action, selected_account).await
        else {
            if let Some((uuid, username)) = selected_account_with_name {
                self.send
                    .send_error("Unable to log into Minecraft account, using offline mode...");
                return Some(MinecraftLoginInfo {
                    uuid,
                    username,
                    access_token: None,
...
impl BackendStateFileWatching {
    ...
    pub fn unwatch_subdirectories_of_instance(&mut self, id: InstanceID) {
        ...
        let content_folder_targets =
            ContentFolder::iter().map(|folder| WatchTarget::InstanceContentDir { id, folder });
