                    ...
                    widget: SettingItemWidget::Backend(Rc::new(|backend, _, cx| {
                        Switch::new("enable-proxy")
                            ...
                            .on_click(cx.listener(|root, val, _, cx| {
                                let Some(mut proxy_settings) =
                                    root.backend_config().map(|b| b.proxy.clone())
                                else {
                                    return;
                                };
    ...
    SettingItemWidget::Backend(Rc::new(|backend, window, cx| {
        ...
        if created {
            cx.subscribe(&state, |root, _, event: &SelectEvent<_>, cx| {
                ...
                let Some(mut proxy_settings) = root.backend_config().map(|b| b.proxy.clone())
                else {
                    return;
                };
    ...
    SettingItemWidget::Backend(Rc::new(|backend, window, cx| {
        ...
        if created {
            cx.subscribe_in(
                &state,
                window,
                |root, state, event: &InputEvent, window, cx| {
        ...
        if created {
            cx.subscribe_in(
                ...
                |root, state, event: &InputEvent, window, cx| {
                    ...
                    root.backend_handle
                        .send(MessageToBackend::SetProxyPassword {
                            password: value.into(),
                        });
                    window.blur();
                },
