...
impl InstanceQuickplaySubpage {
    pub fn new(
        ...
        let world_list = cx.new(move |cx| {
            cx.observe(
                &worlds,
                |list: &mut ListState<WorldsListDelegate>, worlds, cx| {
            ...
            ListState::new(worlds_list_delegate, window2, cx)
                .selectable(false)
                .searchable(true)
        });
        ...
        let server_list = cx.new(move |cx| {
            cx.observe(
                &servers,
                |list: &mut ListState<ServersListDelegate>, servers, cx| {
            ...
            ListState::new(servers_list_delegate, window, cx)
                .selectable(false)
                .searchable(true)
        });
...
impl Render for InstanceQuickplaySubpage {
    fn render(
        ...
        if self.worlds_state.should_load() {
            self.backend_handle.send_with_serial(
                MessageToBackend::RequestLoadWorlds { id: instance_id },
                &self.worlds_serial,
            );
        }
        ...
        if self.servers_state.should_load() {
            self.backend_handle.send_with_serial(
                MessageToBackend::RequestLoadServers { id: instance_id },
                &self.servers_serial,
            );
        }
    ...
    fn render_item(
        ...
        let item = ListItem::new(ix).p_1().child(
            h_flex()
                ...
                .child(
                    div()
                        .child(Button::new(ix).success().icon(PandoraIcon::Play).on_click(
                            move |_, window, cx| {
                                root::start_instance(
                                    id,
                                    name.clone(),
                ...
                h_flex()
                    ...
                    .when_some(summary.ping.as_ref(), |this, ping| {
                        ...
                        let color = if millis < 25 {
                            ...
                            theme
                                .warning
                                .mix_oklab(theme.success, (millis - 25) as f32 / 150.0)
                        } else if millis < 575 {
                            theme
                                .danger
                                .mix_oklab(theme.warning, (millis - 175) as f32 / 400.0)
                        } else {
                            theme.danger
                        };
            ...
            .when_some(summary.status.as_ref(), |this, status| {
                this.child(
                    div()
                        ...
                        .child(crate::component::create_styled_text(
                            &status.description,
                            false,
                        )),
                )
...
impl ServersListDelegate {
    ...
    fn reorder_servers(
        ...
        self.data
            .backend_handle
            .send(MessageToBackend::ReorderServers {
                id: self.id,
                from_index,
                to_index,
            });
