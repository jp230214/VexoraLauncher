...
pub static USER_AGENT: Lazy<String> = Lazy::new(|| {
    if let Some(version) = option_env!("VEXORA_RELEASE_VERSION") {
        format!("VexoraLauncher/{version} (https://github.com/Moulberry/PandoraLauncher)")
    } else {
        "VexoraLauncher/dev (https://github.com/Moulberry/PandoraLauncher)".to_string()
    }
});
