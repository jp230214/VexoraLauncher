...
impl ServerListPinger {
    ...
    pub fn load_status(
        ...
        let protocol_version = {
            let mut protocol_versions = backend
                .server_list_pinger
                .protocol_versions
                .upgradable_read();
            ...
            if protocol_version.is_none() {
                let minecraft_jar_pathname =
                    format!("net/minecraft/{0}/minecraft-client-{0}.jar", version);
                let minecraft_jar_path = backend
                    .directories
                    .libraries_dir
                    .join(minecraft_jar_pathname);
                if let Ok(minecraft_jar) = std::fs::File::open(minecraft_jar_path) {
                    let protocol_version_number = Self::load_minecraft_data_version(minecraft_jar)
        ...
        tokio::spawn(async move {
            let (host, port) = if let Some((host, port)) = server.split_once(':') {
                let Ok(port) = port.parse::<u16>() else {
                    backend
                        ...
                        .data
                        .write()
                        .insert(key.clone(), PingEntry::Failed);
                    return;
                };
                (host, Some(port))
            } else {
            ...
            let status = backend
                .server_list_pinger
                .request_status(host, port, protocol_version)
                .await;
    ...
    async fn request_status(
        ...
        let (response, ping) = self
            .request_status_as_string(address, port, protocol)
            .await?;
    ...
    async fn srv_lookup(&self, host: &str) -> Option<(String, u16)> {
        let resolver = self
            ...
            .get_or_init(|| {
                Some(Box::new(
                    hickory_resolver::Resolver::builder_tokio().ok()?.build(),
                ))
            })
            .as_ref()?;
    ...
    async fn write_varint(&mut self, value: impl Into<VarInt>) -> std::io::Result<()> {
        ...
        loop {
            if (value & !VAR_INT_SECTION_BITS) == 0 {
                ...
                self.write_u8(
                    ((value & VAR_INT_SECTION_BITS) | VAR_INT_SECTION_CONTINUE_BIT) as u8,
                )
                .await?;
                value >>= 7;
            }
...
impl ClientboundPacket {
    async fn read_from_stream(stream: &mut TcpStream) -> std::io::Result<ClientboundPacket> {
        ...
        if length <= 0 {
            ...
            return Err(Error::new(
                ErrorKind::UnexpectedEof,
                "Packet length cannot be greater than 100kb",
            ));
        }
