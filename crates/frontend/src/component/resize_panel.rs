...
use gpui::{
    AnyElement, App, AvailableSpace, Bounds, ContentMask, DispatchPhase, Element, Hitbox,
    IntoElement, MouseButton, MouseDownEvent, MouseMoveEvent, MouseUpEvent, Pixels, Size, Style,
    Window, px,
};
    ...
    fn prepaint(
        ...
        if state.drag_offset.is_none() {
            if let Some(ratio) = state.ratio {
                let new_size = (bounds.size.width * ratio)
                    .round()
                    .clamp(state.min_size, state.max_size);
        ...
        let size = state
            .size
            .clamp(Pixels::ZERO, bounds.size.width - RESIZE_WIDTH);
        drop(state);

        let left_size = Size::new(
    ...
    fn paint(
        ...
        window.on_mouse_event({
            ...
            move |event: &MouseMoveEvent, phase, window, cx| {
                ...
                {
                    let new_size = (event.position.x - drag_offset)
                        .round()
                        .clamp(state.min_size, state.max_size);
                    if state.size != new_size {
                        state.size = new_size;
