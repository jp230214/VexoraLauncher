...
impl InstanceContentSubpage {
    pub fn new(
        ...
        let sort_dropdown = cx.new(|cx| {
            ...
            SelectState::new(
                NamedDropdown::new(items),
                Some(IndexPath::new(row)),
                window,
                cx,
            )
        });
        ...
        let content_list = cx.new(move |cx| {
            ListState::new(content_list_delegate, window, cx)
                .selectable(false)
                .searchable(true)
        });
        ...
        cx.observe(&content, |page, content, cx| {
            if let Some(new_content) = content.read(cx) {
                let (needs_update_check, update_count) = calculate_update_count(
                    page.instance_loader,
                    page.instance_version,
                    new_content,
                );
        ...
        cx.subscribe(
            ...
            |this, _, event: &SelectEvent<NamedDropdown<InstanceContentSortKey>>, cx| {
                ...
                cx.update_entity(&content_list, |list, cx| {
                    list.delegate_mut()
                        .set_sort_options(sort_key, enabled_first);
...
impl Render for InstanceContentSubpage {
    fn render(
        ...
        let (source, sort_enabled_first) = {
            let config = InterfaceConfig::get(cx);
            (
                config.preferred_add_content_source,
                self.content_type.sort_enabled_first(config),
            )
        };

        self.content_states
            .observe(self.content_type.content_folder());
                    ...
                    .dropdown_menu(move |this, window, _| {
                        let mr = PopupMenuItem::new(t::instance::content::install::from_modrinth())
                            .on_click(window.listener_for(
                                &self_entity,
                                InstanceContentSubpage::add_from_modrinth,
                            ));
                        let cf =
                            PopupMenuItem::new(t::instance::content::install::from_curseforge())
                                .on_click(window.listener_for(
                                    &self_entity,
                                    InstanceContentSubpage::add_from_curseforge,
                                ));
                        let file =
                            PopupMenuItem::new(t::instance::content::install::from_file())
                                .on_click(window.listener_for(
                                    &self_entity,
                                    InstanceContentSubpage::add_from_file,
                                ));
                ...
                this.child(
                    Button::new("update_all")
                        .label(match self.content_type {
                            ContentType::Mods => {
                                t::instance::content::update_all_mods(self.update_count)
                            }
                            ContentType::ResourcePacks => {
                                t::instance::content::update_all_resourcepacks(self.update_count)
                            }
                            ContentType::Shaders => {
                                t::instance::content::update_all_shaders(self.update_count)
                            }
                        })
                        .success()
                        .compact()
                            ...
                            cx.listener(move |page, _, window, cx| {
                                if let Some(content) = page.content.read(cx).clone() {
                                    for summary in content.iter() {
                                        if summary.update.can_update(
                                            page.instance_loader,
                                            page.instance_version.as_str(),
                                        ) {
            ...
            .child(
                div().child(
                    Select::new(&self.sort_dropdown)
                        .small()
                        .title_prefix(t::instance::content::sort_prefix()),
                ),
            )
            .child(
                h_flex()
                    ...
                    .child(
                        Switch::new("enabled_first")
                            ...
                            .on_click(cx.listener(|this, checked, _, cx| {
                                ...
                                this.content_type
                                    .set_sort_enabled_first(config, enabled_first);
