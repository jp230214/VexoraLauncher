...
impl InstancePage {
    pub fn new(
        ...
        let subpage =
            instance_subpage.create(&instance, data, data.backend_handle.clone(), window, cx);
                                        ...
                                        h_flex()
                                            ...
                                            .child(
                                                Button::new("ok")
                                                    .success()
                                                    .label(
                                                        t::instance::already_running::start_anyway(
                                                        ),
                                                    )
                                                    .on_click({
                                                        let name = name.clone();
...
impl Render for InstancePage {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if instance_subpage != self.subpage.page_type() {
            let subpage = instance_subpage.create(
                &self.instance,
                &self.data,
        ...
        let show_shader_tab = entry.configuration.show_shader_tab
            || matches!(self.subpage, InstanceSubpage::Shaders(_));
    ...
    pub fn create(
        ...
        Some(match self {
            InstanceSubpageType::Quickplay => InstanceSubpage::Quickplay(
                cx.new(|cx| InstanceQuickplaySubpage::new(instance, data, window, cx)),
            ),
            InstanceSubpageType::Logs => InstanceSubpage::Logs(
                cx.new(|cx| InstanceLogsSubpage::new(instance, backend_handle, window, cx)),
            ),
