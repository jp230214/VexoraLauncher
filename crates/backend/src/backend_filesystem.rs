...
impl BackendState {
    pub async fn handle_filesystem(
        ...
        match result {
            Ok(events) => {
                ...
                for event in events {
                    ...
                    if let Some(last_event) = last_event.take() {
                        ...
                        if last_path.is_none() || last_path != new_path {
                            self.handle_filesystem_event(last_event, &mut after_debounce_effects)
                                .await;
                        }
                    }
                ...
                SkinManager::skin_library_mark_dirty(
                    self,
                    after_debounce_effects.skin_manager_changes,
                );
    ...
    async fn handle_filesystem_change_event(
        ...
        if let Some(target) = target
            && self
                .filesystem_handle_change(target, &path, after_debounce_effects)
                .await
        {
            return;
        }
    ...
    async fn handle_filesystem_remove_event(
        ...
        if let Some(target) = target
            && self
                .filesystem_handle_removed(target, &path, after_debounce_effects)
                .await
        {
            return;
        }
        ...
        if let Some(parent) = parent {
            self.filesystem_handle_child_removed(
                parent,
                parent_path,
                &path,
    ...
    async fn handle_filesystem_event(
        ...
        match event {
            FilesystemEvent::Change(path) => {
                ...
                for path in paths {
                    self.handle_filesystem_change_event(path, after_debounce_effects)
                        .await;
                }
            }
            FilesystemEvent::Remove(path) => {
                ...
                for path in paths {
                    ...
                    self.handle_filesystem_remove_event(path, target, after_debounce_effects)
                        .await;
                }
            }
            FilesystemEvent::Rename(from, to) => {
                    ...
                    && let Some(to_name) = to.file_name()
                {
                    ...
                    for from in from_paths {
                        ...
                        self.handle_filesystem_remove_event(from, target, after_debounce_effects)
                            .await;
                        self.handle_filesystem_change_event(to, after_debounce_effects)
                            .await;
                    }
                } else {
                    let from_paths = self.file_watching.write().all_paths(from.clone());
    ...
    async fn filesystem_handle_removed(
        ...
        match target {
            WatchTarget::RootDir => {
                self.send
                    .send_error("Launcher directory has been removed! This is very bad!");
                true
            }
            WatchTarget::InstancesDir => {
                self.send
                    .send_error("Instances dir has been been removed! Uh oh!");

                let mut instance_state = self.instance_state.write();

                for instance in instance_state.instances.drain() {
                    self.send
                        .send(MessageToFrontend::InstanceRemoved { id: instance.id });
                }
            ...
            WatchTarget::RootDir => {
                ...
                if file_name == "instances" {
                    ...
                    if is_not_unloaded {
                        self.file_watching
                            .write()
                            .watch_filesystem(path.clone(), WatchTarget::SkinLibraryDir);
                        after_debounce_effects.skin_manager_changes.dirty_all();
                    }
                }
            }
            WatchTarget::InstancesDir => {
                if path.is_dir() {
                    ...
                    if !success {
                        self.file_watching
                            .write()
                            .watch_filesystem(path.clone(), WatchTarget::InvalidInstanceDir);
                    }
                }
            }
            WatchTarget::InstanceDir { id } => {
                ...
                RenameMode::Both => {
                    if event.paths[0].extension() == Some(OsStr::new("new")) {
                        ...
                        Some(FilesystemEvent::Rename(
                            event.paths[0].clone().into(),
                            event.paths[1].clone().into(),
                        ))
                    }
                }
