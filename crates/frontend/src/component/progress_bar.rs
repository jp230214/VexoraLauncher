use gpui::{
    App, Hsla, IntoElement, ParentElement, RenderOnce, Styled, Window, div, prelude::FluentBuilder,
    px, relative,
};
...
impl RenderOnce for ProgressBar {
    fn render(self, _: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let radius = theme.radius.min(px(4.0));
        let relative_w = relative(self.amount);

        let progress_bar_color = theme.progress_bar;
        let color = match self.color {
            ProgressBarColor::Normal => progress_bar_color,
            ProgressBarColor::Error => lerp(progress_bar_color, theme.red, self.color_scale),
            ProgressBarColor::Success => lerp(progress_bar_color, theme.green, self.color_scale),
        };
