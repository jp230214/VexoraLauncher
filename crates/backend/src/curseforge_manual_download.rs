...
pub struct ManualCurseforgeDownloadSession(
    Arc<RwLock<Option<ManualCurseforgeDownloadSessionInner>>>,
);
...
impl ManualCurseforgeDownloadSession {
    pub fn process_candidate(
        ...
        if let Some(source_extension) = source.extension() {
            if matches!(
                source_extension.as_encoded_bytes(),
                b"crdownload" | b"part" | b"tmp" | b"new"
            ) {
                return SessionState::StillActive;
            }
        }
        ...
        let destination = crate::fs::create_content_library_path_osstrext(
            &inner.content_library_dir,
            hash,
            extension,
        );
    ...
    pub async fn start(&self, mut files: Vec<ManualCurseforgeDownload>, backend: &BackendState) {
        ...
        let (download_dir_recv, frontend_alive) = {
            ...
            *session_write = Some(session);
            ...
            backend
                .send
                .send(MessageToFrontend::ManualCurseforgeDownloadsRequired {
        ...
        let (frontend_alive, stopped_notify) = {
            ...
            tokio::task::spawn_blocking(move || {
                ...
                while let Some(Ok(entry)) = read_dir.next() {
                    ...
                    if session.process_candidate(&entry.path(), metadata, id, false)
                        == SessionState::Inactive
                    {
                        break;
                    }
