...
use schema::{
    ...
    modrinth::{
        ...
        ModrinthProjectVersionsRequest, ModrinthProjectVersionsResult, ModrinthVersionStatus,
        ModrinthVersionType,
    },
};
...
pub fn open(
    ...
    let result: FrontendMetadataResult<ModrinthProjectVersionsResult> =
        project_versions.read(cx).result();
    match result {
        ...
        FrontendMetadataResult::Loaded(versions) => {
            ...
            let mut version_matrix: FxHashMap<&'static str, VersionMatrixLoaders> =
                FxHashMap::default();
            for version in versions.0.iter() {
                ...
                if let Some(status) = version.status
                    && !matches!(
                        status,
                        ModrinthVersionStatus::Listed | ModrinthVersionStatus::Archived
                    )
                {
                    continue;
                }
            ...
            if version_matrix.is_empty() {
                open_error_dialog(
                    title.clone(),
                    t::instance::content::load::versions::not_found().into(),
                    window,
                    cx,
                );
                return;
            ...
            if let Some(install_for) = install_for {
                let Some(instance) = data.instances.read(cx).entries.get(&install_for) else {
                    open_error_dialog(
                        title.clone(),
                        t::instance::unable_to_find().into(),
                        window,
                        cx,
                    );
                    return;
                ...
                let Some(loaders) = version_matrix.get(minecraft_version) else {
                    let error_message =
                        t::instance::content::load::versions::not_found_for(minecraft_version);
                ...
                if project_type.mod_or_modpack() {
                    valid_loader = instance_loader == Loader::Vanilla
                        || loaders
                            .loaders
                            .contains(instance_loader.as_modrinth_loader());
                }
                if !valid_loader {
                ...
                let force_target_loader =
                    project_type.mod_or_modpack() && instance_loader != Loader::Vanilla;
                ...
                let entries: Arc<[InstanceEntry]> = instance_entries
                    ...
                    .filter_map(|(_, instance)| {
                        ...
                        if let Some(loaders) = version_matrix.get(minecraft_version) {
                            let mut valid_loader = true;
                            if project_type == ModrinthProjectType::Mod
                ...
                let unsupported_instances = instance_entries
                    .read(cx)
                    .entries
                    .len()
...
impl InstallDialog {
        ...
        let required_dependencies = selected_mod_version
            ...
            .map(|deps| {
                    ...
                    && let Some(instance) = self.data.instances.read(cx).entries.get(instance_id)
                {
                    ...
                    for existing_content in instance.read(cx).content.values() {
                        if let Some(existing_content) = existing_content.read(cx) {
                            for summary in existing_content.iter() {
                                let ContentSource::ModrinthProject { project_id } =
                    ...
                    required.retain(|dep| {
                        !existing_projects.contains(dep.project_id.as_ref().unwrap())
                    });
                }
                    ...
                    Checkbox::new("install_deps")
                        ...
                        .label(if required_dependencies.len() == 1 {
                            SharedString::new_static(
                                t::instance::content::install::install_dependency(),
                            )
                        } else {
                            t::instance::content::install::install_dependencies(
                    ...
                    .on_click(cx.listener(move |this, _, window, cx| {
                        ...
                        let path = match this.project_type {
                            ModrinthProjectType::Mod => {
                                RelativePath::new("mods").join(&*install_file.filename)
                            }
                            ...
                            ModrinthProjectType::Other => {
                                window.push_notification(
                                    (
                                        ...
                                        t::instance::content::install::unable_install_other(),
                                    ),
                                    cx,
                                );
                                return;
                        ...
                        let Some(path) = SafePath::from_relative_path(&path) else {
                            window.push_notification(
                                (
                                    NotificationType::Error,
                                    t::instance::content::install::invalid_filename(),
                                ),
                                cx,
                            );
                            return;
                        ...
                        let loader = if let Some(loader) = this.target_loader {
                            ...
                            Loader::iter()
                                ...
                                .find(|loader| {
                                    single_loader_set.contains(loader.as_modrinth_loader())
                                })
                                .unwrap_or(Loader::Vanilla)
                        } else {
                            ModrinthLoader::from_name(&selected_loader_string)
                                .as_pandora()
                                .unwrap_or(Loader::Vanilla)
                        };
                        ...
                        let mut hash = [0u8; 20];
                        let Ok(_) = hex::decode_to_slice(&*install_file.hashes.sha1, &mut hash)
                        else {
                            ...
                            window.push_notification(
                                (NotificationType::Error, SharedString::new(warning)),
                                cx,
                            );
                            return;
                        };
    ...
    fn render_select_target(&self, _window: &mut Window, cx: &mut Context<Self>) -> AnyElement {
        let create_instance_label = match self.project_type {
            ...
            ModrinthProjectType::Modpack => {
                t::instance::content::install::new_instance_with::modpack()
            }
            ModrinthProjectType::Resourcepack => {
                t::instance::content::install::new_instance_with::resourcepack()
            }
            ModrinthProjectType::Shader => {
                t::instance::content::install::new_instance_with::shader()
            }
        ...
        v_flex()
            ...
            .when_some(self.instances.as_ref(), |content, instances| {
                ...
                let selected_instance: Option<InstanceEntry> =
                    read_instances.selected_value().cloned();
                ...
                content
                    ...
                    .child(format!("— {} —", t::common::or_upper()))
            })
            .child(
                Button::new("create")
                    .success()
                    .label(create_instance_label)
    ...
    fn render_select_minecraft_version(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> AnyElement {
        let select_state = self.minecraft_version_select_state.get_or_insert_with(|| {
            if let Some(minecraft_version) = self.fixed_minecraft_version.clone() {
                ...
                let mut keys: Vec<SharedString> = self
                    .version_matrix
                    .keys()
                    .cloned()
                ...
                cx.new(|cx| {
                    let mut select_state =
                        SelectState::new(SearchableVec::new(keys), None, window, cx)
                            .searchable(true);
                    select_state.set_selected_index(Some(IndexPath::default()), window, cx);
                    select_state
                })
    ...
    fn render_select_loader(
        ...
        let loader_select_state = self.loader_select_state.get_or_insert_with(|| {
            ...
            } else if let Some(loaders) =
                self.version_matrix.get(selected_minecraft_version.as_str())
            {
                if loaders.same_loaders_for_all_versions {
                    let single_loader = if loaders.loaders.len() == 1 {
                        SharedString::new_static(
                            loaders.loaders.iter().next().unwrap().pretty_name(),
                        )
                    } else {
                    ...
                    cx.new(|cx| {
                        let mut select_state =
                            SelectState::new(vec![single_loader], None, window, cx);
                        select_state.set_selected_index(Some(IndexPath::default()), window, cx);
                        select_state
                    })
                } else {
    ...
    fn render_select_mod_version(
        ...
        let mod_version_select_state = self.mod_version_select_state.get_or_insert_with(|| {
            ...
            let mod_versions: Vec<ModVersionItem> = self
                ...
                .filter_map(|version| {
                    ...
                    let matches_game_version = game_versions
                        .iter()
                        .any(|v| v.as_str() == selected_game_version);
                    ...
                    if matches_game_version && matches_loader {
                        ...
                        match version.version_type {
                            Some(ModrinthVersionType::Beta) => {
                                name = t::modrinth::versions::beta(&name).into()
                            }
                            Some(ModrinthVersionType::Alpha) => {
                                name = t::modrinth::versions::alpha(&name).into()
                            }
                            _ => {}
                        }
            ...
            cx.new(|cx| {
                let mut select_state =
                    SelectState::new(SearchableVec::new(mod_versions), None, window, cx)
                        .searchable(true);
        ...
        let mod_version_prefix = match self.project_type {
            ...
            ModrinthProjectType::Modpack => {
                format!("{}: ", t::instance::content::version::modpack())
            }
            ModrinthProjectType::Resourcepack => {
                format!("{}: ", t::instance::content::version::resourcepack())
            }
