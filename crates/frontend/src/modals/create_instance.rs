...
impl CreateInstanceModalState {
    pub fn new(
        ...
        let instance_names: Arc<[SharedString]> = instances
            .read(cx)
            .entries
            .iter()
            .map(|(_, v)| v.read(cx).name.clone())
            .collect();

        let minecraft_version_dropdown = cx
            .new(|cx| SelectState::new(VersionList::default(), None, window, cx).searchable(true));

        let _version_selected_subscription = cx.observe_in(
        ...
        let name_input_state =
            cx.new(|cx| InputState::new(window, cx).placeholder(t::instance::unnamed()));
        ...
        let versions = FrontendMetadata::request(
            &metadata,
            bridge::meta::MetadataRequest::MinecraftVersionManifest,
            cx,
        );

        let _versions_updated_subscription =
            cx.observe_in(&versions, window, move |this, _, window, cx| {
        ...
        cx.update_entity(&self.minecraft_version_dropdown, |dropdown, cx| {
            ...
            let (versions, latest) = match result {
                ...
                FrontendMetadataResult::Loaded(manifest) => {
                    ...
                    (
                        versions,
                        Some(SharedString::from(manifest.latest.release.as_str())),
                    )
                }
            };
    ...
    pub fn render(
        ...
        if !self.loaded_versions {
            ...
            show_snapshots_button = Skeleton::new()
                .w_full()
                .min_h_4()
                .max_h_4()
                .rounded_md()
                .into_any_element();
            loader_button_group = Skeleton::new()
                .w_full()
                .min_h_8()
                .max_h_8()
                .rounded_md()
        ...
        let content = v_flex()
            ...
            .child(crate::labelled(
                t::instance::name(),
                Input::new(&self.name_input_state).when(self.name_invalid, |this| {
            ...
            .when(!name_is_invalid, |modal| {
                modal.footer(
                    h_flex()
                        ...
                        .child(Button::new("ok").flex_1().label(t::common::ok()).on_click(
                            cx.listener(move |this, _, window, cx| {
                                if name_is_invalid {
                                    return;
                                }
                                let Some(selected_version) = this
                                    .minecraft_version_dropdown
                                    .read(cx)
                                    .selected_value()
                                    .cloned()
...
pub fn open_create_instance(
    ...
    let state =
        cx.new(|cx| CreateInstanceModalState::new(metadata, instances, backend_handle, window, cx));
