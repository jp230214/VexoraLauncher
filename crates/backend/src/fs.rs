...
pub(crate) fn get_sha1_hash(path: &Path) -> std::io::Result<[u8; 20]> {
    ...
    Ok(hasher
        .finalize()
        .try_into()
        .expect("expected sha1 hash to be 20 bytes"))
}
...
pub(crate) fn read_json<T: for<'de> Deserialize<'de>>(
    path: &Path,
) -> Result<T, IoOrSerializationError> {
...
pub(crate) fn pandora_aux_path_for_content(content: &InstanceContentSummary) -> Option<PathBuf> {
    pandora_aux_path(
        &content.content_summary.id,
        &content.content_summary.name,
        &content.path,
    )
}
...
pub(crate) fn create_content_library_path(
    ...
    create_content_library_path_osstrext(
        content_library_dir,
        expected_hash,
        extension.map(OsStr::new),
    )
}
    ...
    if let Err(err) = std::fs::rename(from, to) {
        if err.kind() == ErrorKind::CrossesDevices {
            ...
            if file_type.is_symlink() {
                ...
                return Err(Error::new(
                    ErrorKind::Other,
                    format!("{from:?} is not a symlink, file or folder"),
                ));
            }
            return Ok(());
        }
...
impl FileMetadata {
    pub fn new(path: &Path) -> std::io::Result<Self> {
        ...
        if result.is_ok() {
            metadata.high_precision_id = Some((
    ...
    pub fn is_same(&self, other: &FileMetadata) -> bool {
        self.low_precision_id == other.low_precision_id
            && self.high_precision_id == other.high_precision_id
    }
