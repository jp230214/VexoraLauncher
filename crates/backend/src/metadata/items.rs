...
use schema::{
    ...
    curseforge::{
        ...
        CurseforgeChangelogResult, CurseforgeFingerprintRequest, CurseforgeFingerprintResponse,
        CurseforgeGetFilesRequest, CurseforgeGetModFilesRequest, CurseforgeGetModFilesResult,
        CurseforgeProject, CurseforgeProjectResponse, CurseforgeSearchRequest,
    ...
    modrinth::{
        MODRINTH_PROJECT_URL, MODRINTH_SEARCH_URL, ModrinthChangelogRequest,
        ModrinthChangelogResult, ModrinthLoader, ModrinthProjectRequest, ModrinthProjectResult,
        ModrinthProjectVersion, ModrinthProjectVersionsRequest, ModrinthProjectVersionsResult,
        ModrinthProjectsRequest, ModrinthProjectsResponse, ModrinthSearchRequest,
        ModrinthSearchResult, ModrinthVersionFileUpdateResult, ModrinthVersionType,
        ModrinthVersionsFromHashesRequest, ModrinthVersionsFromHashesResponse,
    },
    version::MinecraftVersion,
    version_manifest::{
        MOJANG_VERSION_MANIFEST_URL, MinecraftVersionLink, MinecraftVersionManifest,
    },
};
use serde::Serialize;
use ustr::Ustr;
...
use crate::metadata::manager::{
    MetaLoadError, MetaStateWrapper, MetadataManager, MetadataManagerStates,
};
...
pub trait MetadataItem: Debug {
    ...
    ) -> Option<impl AsRef<Path> + Send + Sync + 'static> {
        None::<PathBuf>
    }
...
impl<'v> MetadataItem for MinecraftVersionMetadataItem<'v> {
    ...
    fn cache_file(
        ...
        if !crate::fs::is_single_component_path_str(&self.0.sha1) {
            panic!(
                "Invalid sha1 {}, possible directory traversal attack?",
                self.0.sha1
            );
        }
...
impl MetadataItem for ModrinthVersionUpdateMetadataItem {
    ...
    fn request(&self, client: &reqwest::Client) -> RequestBuilder {
        let url = format!(
            "https://api.modrinth.com/v2/version_file/{}/update",
            self.sha1
        );
...
impl MetadataItem for ModrinthV3VersionUpdateMetadataItem {
    ...
    fn request(&self, client: &reqwest::Client) -> RequestBuilder {
        let url = format!(
            "https://api.modrinth.com/v3/version_file/{}/update",
            self.sha1
        );
        client.post(url).json(&self.params)
    }
...
impl<'a> MetadataItem for ModrinthProjectsMetadataItem<'a> {
    ...
    fn deserialize(bytes: &[u8]) -> Result<Self::T, MetaLoadError> {
        Ok(ModrinthProjectsResponse(serde_json::from_slice::<
            Arc<[ModrinthProjectResult]>,
        >(bytes)?))
    }
}
