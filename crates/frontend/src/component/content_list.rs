...
use bridge::{
    ...
    instance::{
        ContentSummary, ContentType, InstanceContentID, InstanceContentSummary, InstanceID,
...
impl ContentEntryChild {
    pub fn compare_by(
        &self,
        other: &ContentEntryChild,
        key: InstanceContentSortKey,
    ) -> std::cmp::Ordering {
        match key {
            InstanceContentSortKey::Name => {
                let name_a = self
                    ...
                    .name
                    .as_deref()
                    .or(self.summary.id.as_deref())
                    .unwrap_or(&*self.path);
                let name_b = other
                    .summary
                    .name
                    .as_deref()
                    .or(other.summary.id.as_deref())
            ...
            InstanceContentSortKey::ModId => {
                let name_a = self
                    ...
                    .id
                    .as_deref()
                    .or(self.summary.name.as_deref())
                    .unwrap_or(&*self.path);
                let name_b = other
                    .summary
                    .id
                    .as_deref()
                    .or(other.summary.name.as_deref())
                    .unwrap_or(&*other.path);
                lexical_sort::natural_lexical_cmp(name_a, name_b)
            }
...
impl ContentListDelegate {
    ...
    pub fn render_summary(
                    ...
                    cx.listener(move |this, _, _, cx| {
                        ...
                        if delegate.is_selected(element_id) {
                            let content_ids = delegate
                                ...
                                .filter_map(|summary| {
                                    delegate
                                        .is_selected(summary.filename_hash)
                                        .then(|| summary.id)
                                })
                                .collect();

                            backend_handle
                                .send(MessageToBackend::DeleteContent { id, content_ids });
                        } else {
        ...
        let status = summary
            .update
            .status_if_matches(self.for_loader, self.for_version.as_str());
        ...
        let open_change_version = {
            ...
            move |_: &ClickEvent, window: &mut Window, cx: &mut App| {
                cx.stop_propagation();
                crate::modals::change_version::open(
                    id,
                    &summary,
                    &data,
                    updating.clone(),
        ...
        let source_name = match summary.content_source {
            ContentSource::ModrinthProject { .. } | ContentSource::ModrinthUnknown => {
                t::modrinth::name()
            }
        ...
        let update_button = match status {
            ...
            | bridge::instance::ContentUpdateStatus::ManualInstall => {
            ...
            | bridge::instance::ContentUpdateStatus::Curseforge => {
                ...
                Some(
                    Button::new(("update", element_id))
                        ...
                        .loading(is_loading)
                        .tooltip(t::instance::content::change_version::update_available(
                            source_name,
                        ))
                            ...
                            if delegate.is_selected(element_id) {
                                for summary in &delegate.content {
                                    if delegate.is_selected(summary.filename_hash)
                                        && summary.update.can_update(
                                            delegate.for_loader,
                                            delegate.for_version.as_str(),
                                        )
                                    {
        ...
        let controls = if let Some(expand_index) = expand_index {
            ...
            let expand_control = Button::new(("expand", element_id))
                .icon(expand_icon)
                .compact()
                .small()
                .info()
        ...
        let mut item_content = h_flex()
            ...
            .when(selected, |content| {
                content
                    ...
                    .bg(cx.theme().selection.alpha(0.2))
            });
                        ...
                        if let Some(searched) = &delegate.searched {
                            ...
                            let to_index = searched.iter().position(|element| match element {
                                SummaryOrChild::Summary(summary) => {
                                    summary.filename_hash == element_id
                                }
                                SummaryOrChild::Child(_) => false,
                            });
                            ...
                            let from_index = delegate
                                .content
                                .iter()
                                .position(|element| element.filename_hash == from);

                            let Some(from_index) = from_index else {
                                return;
                            };

                            let to_index = delegate
                                .content
                                .iter()
                                .position(|element| element.filename_hash == element_id);
    ...
    fn render_child_entry(&self, child: &ContentEntryChild, cx: &mut App) -> ListItem {
        ...
        let visually_enabled =
            enabled && child.parent_enabled && !child.disabled_third_party_downloads;
        ...
        let mut item_content = h_flex()
            ...
            .when_some(desc2, |div, desc2| {
                div.child(desc2.when(!visually_enabled, |this| this.line_through()))
            });
        ...
        for modification in new_content.iter() {
            ...
            } = &extra
            {
                for unknown_file in unknown_files.iter() {
                    let filename: Arc<str> =
                        t::instance::content::file_id(unknown_file.file_id).into();
            ...
            if let Some(files) = files {
                for file in files.iter() {
                    ...
                    let enabled = modification.disabled_children.is_enabled(
                    ...
                    let lowercase_filename: Arc<str> =
                        file.path.as_str().to_ascii_lowercase().into();
                    ...
                    let filesize = match &file.source {
                        bridge::instance::ModpackFileSource::DownloadUrl { size, .. } => {
                            *size as u64
                        }
                        bridge::instance::ModpackFileSource::DownloadCurseforge { .. } => 0,
                        bridge::instance::ModpackFileSource::Builtin { bytes } => {
                            bytes.len() as u64
                        }
                    };
