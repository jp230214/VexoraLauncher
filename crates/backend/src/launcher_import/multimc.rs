...
pub fn try_load_from_multimc(
    instance_cfg: &Path,
    mmc_pack: &Path,
) -> Option<(InstanceConfiguration, InstanceStats)> {
    ...
    let mut configuration =
        InstanceConfiguration::new(minecraft_version?, loader.unwrap_or(Loader::Vanilla));
            ...
            _ => {
                ...
                match (section, key) {
                    ...
                    (Some("[General]"), "JavaPath") => {
                        configuration.jvm_binary.get_or_insert_default().path =
                            Some(Path::new(value).into());
                    }
                    // Java Args
                    ...
                    (Some("[General]"), "CustomOpenALPath") => {
                        if value.is_empty() {
                            configuration
                                .system_libraries
                                .get_or_insert_default()
                                .openal = LwjglLibraryPath::Auto;
                        } else {
                            configuration
                                .system_libraries
                                .get_or_insert_default()
                                .openal = LwjglLibraryPath::Explicit(Path::new(value).into());
                        }
                    }
...
async fn import_accounts_from_multimc(
    ...
    backend.account_info.write().modify(|accounts| {
        for account in &accounts_json.accounts {
            match account {
                MultiMCAccount::Offline { active, profile }
                | MultiMCAccount::MSA {
                    active, profile, ..
                } => {
