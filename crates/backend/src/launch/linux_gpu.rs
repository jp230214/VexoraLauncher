...
fn is_discrete_gpu(device: &GpuDevice) -> std::io::Result<bool> {
    let has_switcheroo_discrete_tag = unsafe {
        ...
        udev::ffi::udev_device_has_tag(device.render.as_raw(), SWITCHEROO_DISCRETE_GPU_TAG.as_ptr())
            != 0
    };
...
fn is_discrete_gpu_xe(device: &GpuDevice) -> std::io::Result<bool> {
    ...
    if device_query.size
        < (DRM_XE_QUERY_CONFIG_INFO_OFFSET + DRM_XE_QUERY_CONFIG_FLAGS + 1)
            * std::mem::size_of::<u64>() as u32
    {
        log::warn!(
            "is_discrete_gpu_xe: query size is {}, not enough to read flags",
            device_query.size
        );
        return Ok(false);
    }
