...
use schema::{
    ...
    curseforge::{
        CurseforgeClassId, CurseforgeHit, CurseforgeSearchRequest, CurseforgeSearchResult,
        CurseforgeSortField,
    },
    loader::Loader,
};
...
pub struct CurseforgeSearchPage {
    ...
    specific_installed_content_by_project:
        enum_map::EnumMap<ContentFolder, FxHashMap<u32, Vec<InstalledContent>>>,
...
impl CurseforgeSearchPage {
    pub fn new(
        ...
        > = Default::default();
        let mut all_installed_content_by_project: FxHashMap<u32, Vec<InstalledContent>> =
            FxHashMap::default();
        ...
        if let Some(install_for) = install_for {
            if let Some(entry) = data.instances.read(cx).entries.get(&install_for) {
                ...
                for content_folder in ContentFolder::iter() {
                    let mut specific_installed_content: FxHashMap<u32, Vec<InstalledContent>> =
                        FxHashMap::default();
                    ...
                    if let Some(content) = instance_content[content_folder].read(cx) {
                        for summary in content.iter() {
                            ...
                            let installed =
                                all_installed_content_by_project.entry(project).or_default();
    ...
    fn load_more(&mut self, cx: &mut Context<Self>) {
        ...
        let data = FrontendMetadata::request(
            &self.data.metadata,
            MetadataRequest::CurseforgeSearch(request),
            cx,
        );
        ...
        match result {
            FrontendMetadataResult::Loading => {
                let subscription = cx.observe(&data, |page, data, cx| {
                    let result: FrontendMetadataResult<CurseforgeSearchResult> =
                        data.read(cx).result();
                    match result {
                        FrontendMetadataResult::Loading => {
                            return;
                            ...
                            if hit.class_id.is_some() && hit.class_id != Some(0) {
                                match primary_action {
                                    ...
                                    PrimaryAction::InstallLatest => {
                                        ...
                                        let Some(entry) =
                                            data.instances.read(cx).entries.get(&install_for)
                                        else {
                                            window.push_notification(
                                                (
                                                    NotificationType::Error,
                                        ...
                                        let minecraft_version =
                                            instance.configuration.minecraft_version;
                                        ...
                                        let content_install = ContentInstall {
                                            ...
                                            files: [ContentInstallFile {
                                                replace_old: None,
                                                path:
                                                    bridge::install::ContentInstallPath::Automatic,
    ...
    pub fn get_primary_action(&self, project_id: u32, cx: &App) -> PrimaryAction {
        get_primary_action(
            ...
            &self.all_installed_content_by_project,
            cx,
        )
    }
}
...
impl Render for CurseforgeSearchPage {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let loader_button_group = if filter_project_type == CurseforgeClassId::Mod
            || filter_project_type == CurseforgeClassId::Modpack
        {
            Some(
                ...
                this.child(
                    ButtonGroup::new("sort_field")
                            ...
                            Button::new(("sort", id))
                                .child(
                                    h_flex().w_full().justify_start().gap_2().child(
                                        t::curseforge::sort::get(field.as_str())
                        ...
                        .on_click(cx.listener(move |page, clicked: &Vec<usize>, window, cx| {
                            let sort_field = CurseforgeSortField::iter()
                                .nth(clicked[0])
                                .unwrap_or_default();
        ...
        let is_mod = filter_project_type == CurseforgeClassId::Mod
            || filter_project_type == CurseforgeClassId::Modpack;
