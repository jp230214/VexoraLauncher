...
impl BodyPartDef {
    ...
    fn add_projected_quads(
        ...
        let pitch = -15.0_f64.to_radians()
            * sway_strength
            * (sway_progress * std::f64::consts::TAU * sway_time_mult).sin()
            + pitch_offset;
...
pub fn determine_skin_variant(skin_png: &[u8]) -> Option<SkinVariant> {
    ...
    // Guard against malformed/small images: get_pixel panics out of bounds,
    // and a crash here would take down the whole launcher when the Skins
    // page iterates the library. Fall back to Classic instead.
    if skin.width() < 55 || skin.height() < 21 {
        return Some(SkinVariant::Classic);
    }
...
/// Core 3D skin renderer operating on already-decoded images, so callers
/// that render every frame (e.g. the rotating preview) can decode once
/// and reuse the result instead of re-decoding the PNG each time.
/// Behaves exactly like [`render_skin_3d`] for the same image content.
pub fn render_skin_3d_images(
    skin: &image::DynamicImage,
    cape: Option<&image::DynamicImage>,
    variant: SkinVariant,
    out_width: u32,
    out_height: u32,
    yaw_deg: f64,
    ...
    let is_slim = match variant {
        ...
        SkinVariant::Other => {
            !is_legacy && skin.width() > 54 && skin.height() > 20 && skin.get_pixel(54, 20)[3] < 20
        }
    };

    let mut projected_quads = collect_quads(
        is_legacy,
        is_slim,
        cape.is_some(),
        yaw_deg,
    ...
    projected_quads.sort_by(|a, b| {
        a.avg_z
            .partial_cmp(&b.avg_z)
            .unwrap_or(std::cmp::Ordering::Equal)
    });

    let scale = (out_width as f64 / MAX_WIDTH_AT_ANY_ANGLE)
        .min(out_height as f64 / MAX_HEIGHT_AT_ANY_ANGLE)
        * zoom;
    ...
    for mut projected_quad in projected_quads {
        ...
        for i in 0..4 {
            verts[i] = V3::new(
                verts[i].x * scale + offset_x,
                verts[i].y * scale + offset_y,
                verts[i].z,
            );
        }
    ...
    loop {
        ...
        for y in 0..=yaw_acc {
            for p in 0..=pitch_acc {
                ...
                let projected_quads =
                    collect_quads(false, true, true, yaw, pitch, MAX_CAPE_ANGLE_SWAY_PROGRESS);
                let mut w = f64::MIN;
                ...
                if w * 2.0 > max_w {
                    ...
                    && best_yaw.abs() + best_pitch.abs() > yaw.abs() + pitch.abs()
                {
                    best_yaw = yaw;
                    best_pitch = pitch;
                }
...
mod tests {
    ...
    #[test]
    fn invalid_skin_data_does_not_panic() {
        // Garbage bytes: both helpers must return None, never panic.
        assert_eq!(determine_skin_variant(b"not a png"), None);
        assert_eq!(
            render_skin_3d(
                b"not a png",
                None,
    ...
    #[test]
    fn tiny_skin_png_does_not_panic() {
        // Regression test: a valid-but-tiny PNG (e.g. a random image saved
        // into the skin library) used to panic in get_pixel(54, 20) and
        // crash the whole launcher when opening the Skins page.
        let tiny = png_bytes(1, 1);
        assert_eq!(determine_skin_variant(&tiny), Some(SkinVariant::Classic));
        // Wrong dimensions for a skin -> None, but must not panic.
        assert_eq!(
            render_skin_3d(
                &tiny,
                None,
    ...
    #[test]
    fn zero_size_output_returns_none() {
        let skin = png_bytes(64, 64);
        assert_eq!(
            render_skin_3d(
                &skin,
                None,
    ...
    #[test]
    fn vexora_themes_json_parses_as_theme_set() {
        // The launcher loads assets/themes/vexora.json through
        // ThemeRegistry::load_themes_from_str; a schema violation there
        // silently drops EVERY theme (only `_ =` is checked at the call
        // site), so guard the file with the real deserializer.
        let data = include_str!("../../../assets/themes/vexora.json");
        let set: gpui_component::ThemeSet =
            serde_json::from_str(data).expect("vexora.json must parse as ThemeSet");
        assert!(
            set.themes.iter().any(|t| t.name.as_ref() == "Vexora Dark"),
            "Vexora Dark theme must exist"
        );
        assert!(set.themes.len() >= 10, "expected many themes");
    }

    #[test]
    fn images_api_matches_bytes_api() {
        // The frame-cached renderer must produce byte-identical output to
        // the one-shot bytes API for the same inputs.
        let skin = png_bytes(64, 64);
        let params = (SkinVariant::Slim, 96u32, 96u32, 22.5, -8.0, 0.2, 4.0, 1.2);
        let (variant, w, h, yaw, pitch, sway, y_off, zoom) = params;
        let from_bytes = render_skin_3d(&skin, None, variant, w, h, yaw, pitch, sway, y_off, zoom);
        let decoded = image::load_from_memory_with_format(&skin, image::ImageFormat::Png).unwrap();
        let from_images =
            render_skin_3d_images(&decoded, None, variant, w, h, yaw, pitch, sway, y_off, zoom);
        assert!(from_bytes.is_some());
        assert_eq!(from_bytes, from_images);

        // Cape path included.
        let cape = png_bytes(64, 32);
        let decoded_cape =
            image::load_from_memory_with_format(&cape, image::ImageFormat::Png).unwrap();
        let from_bytes = render_skin_3d(
            &skin,
            Some(&cape),
            SkinVariant::Classic,
            w,
        ...
        let from_images = render_skin_3d_images(
            &decoded,
            Some(&decoded_cape),
            SkinVariant::Classic,
            w,
    ...
    #[test]
    fn valid_64x64_skin_renders() {
        let skin = png_bytes(64, 64);
        let out = render_skin_3d(
            &skin,
            None,
            SkinVariant::Classic,
            64,
