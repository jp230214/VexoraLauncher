...
use schema::{
    ...
    curseforge::{
        ...
        CurseforgeGetModFilesResult, CurseforgeProject, CurseforgeSearchRequest,
...
use crate::{
    HttpClientProvider,
    metadata::items::{
        MetadataItem, ModrinthV3VersionUpdateMetadataItem, ModrinthVersionUpdateMetadataItem,
    },
};
...
pub struct MetadataManagerStates {
    ...
    pub(super) java_runtime_manifests:
        HashMap<Ustr, MetaStateWrapper<JavaRuntimeComponentManifest>>,
    pub(super) modrinth_search:
        HashMap<ModrinthSearchRequest, MetaStateWrapper<ModrinthSearchResult>>,
    pub(super) modrinth_project_versions:
        HashMap<ModrinthProjectVersionsRequest, MetaStateWrapper<ModrinthProjectVersionsResult>>,
    pub(super) modrinth_project:
        HashMap<ModrinthProjectRequest, MetaStateWrapper<ModrinthProjectResult>>,
    pub(super) modrinth_projects:
        HashMap<ModrinthProjectsRequest, MetaStateWrapper<ModrinthProjectsResponse>>,
    ...
    pub(super) modrinth_version_v2_updates: HashMap<
        ModrinthVersionUpdateMetadataItem,
        MetaStateWrapper<ModrinthVersionFileUpdateResult>,
    >,
    pub(super) modrinth_version_v3_updates: HashMap<
        ModrinthV3VersionUpdateMetadataItem,
        MetaStateWrapper<ModrinthVersionFileUpdateResult>,
    >,
    pub(super) modrinth_versions_from_hashes: HashMap<
        ModrinthVersionsFromHashesRequest,
        MetaStateWrapper<ModrinthVersionsFromHashesResponse>,
    >,
    pub(super) curseforge_search:
        HashMap<CurseforgeSearchRequest, MetaStateWrapper<CurseforgeSearchResult>>,
...
pub struct MetadataManager {
    ...
    expiring:
        enum_map::EnumMap<ExpirationDuration, Mutex<VecDeque<(Instant, KeepAliveNotifySignal)>>>,
                ...
                if let Some(url) = error.url() {
                    if error.is_connect() {
                        ...
                        return f.write_fmt(format_args!(
                            "Unexpected error while constructing request to {}",
                            url
                        ));
                    }
...
impl MetadataManager {
    ...
    pub fn preload<I: MetadataItem>(&self, item: I) {
        ...
        if wrapper.should_reload(false) {
            if item.expires() {
                wrapper.keep_alive =
                    Some(self.create_expiry_keepalive(ExpirationDuration::Success));
            }

            let cache_file = item.cache_file(self);
            wrapper.load_state =
                Self::inner_start_loading(&item, cache_file, &self.http_client.client());
        }
    }

    pub async fn fetch<I: MetadataItem>(
        &self,
        item: I,
    ) -> Result<Arc<<I as MetadataItem>::T>, MetaLoadError> {
    ...
    pub async fn fetch_with_keepalive<I: MetadataItem>(
        ...
        Result<Arc<<I as MetadataItem>::T>, MetaLoadError>,
        Option<KeepAliveNotifySignalHandle>,
    ) {
        loop {
    ...
    pub async fn fetch_with_keepalive_inner<I: MetadataItem>(
        ...
        enum LoopAction<T> {
            Resolve(
                KeepAliveNotifySignal,
                JoinHandle<Result<Arc<T>, MetaLoadError>>,
            ),
                        ...
                        Err(error) => {
                            ...
                            if error.should_retry_error() {
                                ...
                                if wrapper.failure_count == 1 {
                                    ...
                                    wrapper.keep_alive =
                                        Some(self.create_expiry_keepalive(
                                            ExpirationDuration::RetryError1,
                                        ));
                                } else if wrapper.failure_count == 3 {
                                    wrapper.keep_alive =
                                        Some(self.create_expiry_keepalive(
                                            ExpirationDuration::RetryError2,
                                        ));
                                } else if wrapper.failure_count == 4 {
                                    wrapper.keep_alive =
                                        Some(self.create_expiry_keepalive(
                                            ExpirationDuration::RetryError3,
                                        ));
                                } else {
                                    wrapper.keep_alive =
                                        Some(self.create_expiry_keepalive(
                                            ExpirationDuration::RetryError4,
                                        ));
                                }
                            } else {
                                wrapper.failure_count = 0;
        ...
        let join_handle = tokio::task::spawn(async move {
                ...
                if status != StatusCode::OK {
                    if status == StatusCode::BAD_REQUEST {
                        if let Ok(bytes) = response.bytes().await {
                            ...
                            if let Ok(error_messages) =
                                serde_json::from_slice::<ErrorMessages>(&bytes)
                            {
                                if let Some(description) = error_messages.description {
                                    return Err(MetaLoadError::ErrorWithDescription(
            ...
            if let Err(error) = &result {
                if let Some(file_fallback) = file_fallback {
                    ...
                    log::error!(
                        "Error while fetching metadata {:?}: {error:?}",
                        std::any::type_name::<I::T>()
                    );
                }
            }
