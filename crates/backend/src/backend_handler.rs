...
use auth::{
    credentials::AccountCredentials, models::MinecraftAccessToken, secret::PlatformSecretStorage,
};
use bridge::{
    install::{
        ContentDownload, ContentInstall, ContentInstallFile, ContentInstallPath, InstallTarget,
    },
    instance::{ContentFolder, ContentSummary, ContentType, InstanceID},
    keep_alive::KeepAlive,
    message::{
        AccountCapesResult, AccountSkinResult, EmbeddedOrRaw, GameOutputMsg, LogFiles,
...
use crate::{
    ...
    metadata::{
        items::{
            AssetsIndexMetadataItem, CurseforgeChangelogMetadataItem,
            CurseforgeGetModFilesMetadataItem, CurseforgeSearchMetadataItem,
            FabricLoaderManifestMetadataItem, ForgeInstallerMavenMetadataItem,
            MinecraftVersionManifestMetadataItem, MinecraftVersionMetadataItem,
            ModrinthChangelogMetadataItem, ModrinthProjectMetadataItem,
            ...
            MojangJavaRuntimeComponentMetadataItem, MojangJavaRuntimesMetadataItem,
            NeoforgeInstallerMavenMetadataItem, VersionUpdateParameters, VersionV3LoaderFields,
            VersionV3UpdateParameters,
        },
...
impl BackendState {
    pub async fn handle_message(self: &Arc<Self>, message: MessageToBackend) {
        match message {
            MessageToBackend::RequestMetadata {
                request,
                force_reload,
            } => {
                ...
                tokio::task::spawn(async move {
                    let (result, keep_alive_handle) = match request {
                        bridge::meta::MetadataRequest::MinecraftVersionManifest => {
                            let (result, handle) = meta
                                .fetch_with_keepalive(
                                    MinecraftVersionManifestMetadataItem,
                                    force_reload,
                                )
                                .await;
                        ...
                        bridge::meta::MetadataRequest::FabricLoaderManifest => {
                            let (result, handle) = meta
                                .fetch_with_keepalive(
                                    FabricLoaderManifestMetadataItem,
                                    force_reload,
                                )
                                .await;
                        ...
                        bridge::meta::MetadataRequest::ForgeMavenManifest => {
                            let (result, handle) = meta
                                .fetch_with_keepalive(ForgeInstallerMavenMetadataItem, force_reload)
                                .await;
                        ...
                        bridge::meta::MetadataRequest::NeoforgeMavenManifest => {
                            let (result, handle) = meta
                                .fetch_with_keepalive(
                                    NeoforgeInstallerMavenMetadataItem,
                                    force_reload,
                                )
                                .await;
                        ...
                        bridge::meta::MetadataRequest::ModrinthSearch(ref search) => {
                            let (result, handle) = meta
                                .fetch_with_keepalive(
                                    ModrinthSearchMetadataItem(search),
                                    force_reload,
                                )
                                .await;
                            (result.map(MetadataResult::ModrinthSearchResult), handle)
                        }
                        bridge::meta::MetadataRequest::ModrinthProjectVersions(
                            ...
                            let (result, handle) = meta
                                ...
                                .await;
                            (
                                result.map(MetadataResult::ModrinthProjectVersionsResult),
                                handle,
                            )
                        }
                        bridge::meta::MetadataRequest::ModrinthProject(ref project) => {
                            let (result, handle) = meta
                                .fetch_with_keepalive(
                                    ModrinthProjectMetadataItem(project),
                                    force_reload,
                                )
                                .await;
                        ...
                        bridge::meta::MetadataRequest::ModrinthChangelog(ref request) => {
                            let (result, handle) = meta
                                .fetch_with_keepalive(
                                    ModrinthChangelogMetadataItem(request),
                                    force_reload,
                                )
                                .await;
                        ...
                        bridge::meta::MetadataRequest::CurseforgeSearch(ref search) => {
                            let (result, handle) = meta
                                .fetch_with_keepalive(
                                    CurseforgeSearchMetadataItem(search),
                                    force_reload,
                                )
                                .await;
                        ...
                        bridge::meta::MetadataRequest::CurseforgeGetModFiles(ref request) => {
                            let (result, handle) = meta
                                .fetch_with_keepalive(
                                    CurseforgeGetModFilesMetadataItem(request),
                                    force_reload,
                                )
                                .await;
                            (
                                result.map(MetadataResult::CurseforgeGetModFilesResult),
                                handle,
                            )
                        }
                        bridge::meta::MetadataRequest::CurseforgeChangelog(ref request) => {
                            let (result, handle) = meta
                                .fetch_with_keepalive(
                                    CurseforgeChangelogMetadataItem(request),
                                    force_reload,
                                )
                                .await;
                            (
                                result.map(MetadataResult::CurseforgeChangelogResult),
                                handle,
                            )
                        }
                    };
            ...
            MessageToBackend::ReorderServers {
                ...
                tokio::task::spawn(Instance::reorder_servers(
                    self.clone(),
                    id,
                    from_index,
                    to_index,
            ...
            MessageToBackend::DeleteInstance { id } => {
                if let Some(instance) = self.instance_state.write().instances.get_mut(id) {
                    ...
                    if let Err(err) = result {
                        self.send
                            .send_error(format!("Unable to delete instance folder: {}", err));
                    }
                }
            }
            MessageToBackend::DuplicateInstance {
                id,
                name,
                modal_action,
            } => {
            ...
            MessageToBackend::SetInstanceWrapperCommand {
                id,
                wrapper_command,
            } => {
                if let Some(instance) = self.instance_state.write().instances.get_mut(id) {
            ...
            MessageToBackend::SetInstanceSystemLibraries {
                id,
                system_libraries,
            } => {
                if let Some(instance) = self.instance_state.write().instances.get_mut(id) {
                        ...
                        if let Ok(format) = image::guess_format(&*image_bytes) {
                            if format == image::ImageFormat::Png {
                                ...
                                if let Some(instance) =
                                    self.instance_state.write().instances.get_mut(id)
                                {
                                ...
                                self.send
                                    .send_error("Unable to apply icon: only pngs are supported");
                            }
                        } else {
            ...
            MessageToBackend::KillInstance { id } => {
                ...
                if instance.processes.is_empty() && instance.closing_processes.is_empty() {
                    self.send
                        .send_error("Can't kill instance, instance wasn't running");
                    return;
                }
                ...
                for mut process in instance.processes.drain(..) {
                    ...
                    if result.is_err() {
                        ...
                        instance
                            .closing_processes
                            .push((process, now + Duration::from_secs(3)));
                    }
            ...
            MessageToBackend::StartInstanceByName { name, quick_play } => {
                ...
                if let Some(id) = id {
                    self.start_instance(id, quick_play, None, Default::default())
                        .await
                }
            }
            MessageToBackend::StartInstance {
                ...
                self.start_instance(id, quick_play, live_game_output, modal_action)
                    .await
            }
            MessageToBackend::SetContentEnabled {
                ...
                if cannot_modify_while_running {
                    self.send
                        .send_warning("Cannot modify mods folder while instance is running");
                }
            }
            MessageToBackend::SetContentChildEnabled {
                ...
                {
                    let Some(aux_path) = crate::fs::pandora_aux_path_for_content(instance_mod)
                    else {
                        return;
                    };
                    ...
                    let mut aux: AuxiliaryContentMeta =
                        crate::fs::read_json(&aux_path).unwrap_or_default();
                    ...
                    if changed {
                        let bytes = match serde_json::to_vec(&aux) {
                            ...
                            Err(err) => {
                                ...
                                self.send
                                    .send_error("Unable to serialize AuxiliaryContentMeta");
                                return;
                            }
                        };
            ...
            MessageToBackend::DownloadContentChildren {
                ...
                tokio::spawn(async move {
                    this.download_modpack_children(
                        &summary,
                        loader,
                        minecraft_version,
                        &modal_action,
            ...
            MessageToBackend::InstallContent {
                content,
                modal_action,
            } => {
                let this = self.clone();
            ...
            MessageToBackend::CreateInstanceFromFile { file, modal_action } => {
                ...
                let Some(name) = summary.name.clone() else {
                    modal_action
                        .set_finished_with_error("Unable to determine name from modpack".into());
                    return;
                };
                ...
                let Some(minecraft_version) = minecraft_version else {
                    modal_action.set_finished_with_error(
                        "Unable to determine minecraft version from modpack".into(),
                    );
                    return;
                };
                ...
                tokio::spawn(async move {
                    this.install_content(content_install, modal_action.clone())
                        .await;
                    modal_action.set_finished();
                    this.send.send(MessageToFrontend::Refresh);
                });
            ...
            MessageToBackend::UpdateCheck {
                ...
                let (loader, version, update_channel) =
                    if let Some(instance) = self.instance_state.write().instances.get_mut(id) {
                        ...
                        (
                            configuration.loader,
                            configuration.minecraft_version,
                            configuration.update_channel,
                        )
                    } else {
                        self.send.send_error("Can't update instance, unknown id");
                        modal_action
                            .set_finished_with_error("Can't update instance, unknown id".into());
                        return;
                    };

                let mut content = Vec::new();
                for folder in ContentFolder::iter() {
                    let Some(summaries) = Instance::load_content(self.clone(), id, folder).await
                    else {
                        modal_action.set_finished();
                        return;
                    };
                ...
                if instance_modrinth_loader == ModrinthLoader::Unknown {
                    modal_action.set_finished_with_error(
                        "Unable to update instance, unsupported loader".into(),
                    );
                    return;
                }
...

                {
                    ...
                    for summary in content.iter() {
                        let source = sources
                            .get(&summary.content_summary.hash)
                            .unwrap_or(ContentSource::Manual);
                ...
                let results: Result<Vec<UpdateResult>, MetaLoadError> =
                ...
                match results {
                    Ok(updates) => {
                        ...
                        if let Some(instance) = self.instance_state.write().instances.get_mut(id) {
                            for content_folder in ContentFolder::iter() {
                                instance.mark_content_dirty(
                                    self,
                                    content_folder,
                                    FolderChanges::all_dirty(),
                                    true,
                                );
                            }
                        }
                    ...
                    Err(error) => {
                        ...
                        modal_action.set_finished_with_error(
                            format!("Error checking for updates: {}", error).into(),
                        );
                        return;
                    }
                }
            ...
            MessageToBackend::UpdateContent {
                ...
                let content_install = if let Some(instance) =
                    self.instance_state.write().instances.get_mut(id)
                {
                    let configuration = instance.configuration.get();
                    ...
                    let Some((mod_summary, _)) = instance.try_get_content(mod_id) else {
                        self.send
                            .send_error("Can't update mod in instance, unknown mod id");
                        modal_action.set_finished();
                        return;
                    };
                    ...
                    let Some(update_info) = self
                        ...
                        self.send
                            .send_error("Can't update mod in instance, missing update action");
                        modal_action.set_finished();
                        return;
                    };
                    ...
                    match update_info {
                        ContentUpdateAction::ErrorNotFound => {
                            self.send
                                .send_error("Can't update mod in instance, 404 not found");
                            modal_action.set_finished();
                            return;
                        }
                        ContentUpdateAction::ErrorInvalidHash => {
                            self.send
                                .send_error("Can't update mod in instance, returned invalid hash");
                            modal_action.set_finished();
                            return;
                        }
                        ContentUpdateAction::AlreadyUpToDate => {
                            self.send
                                .send_error("Can't update mod in instance, already up-to-date");
                            modal_action.set_finished();
                            return;
                        }
                        ContentUpdateAction::ManualInstall => {
                            self.send.send_error(
                                "Can't update mod in instance, mod was manually installed",
                            );
                            modal_action.set_finished();
                            return;
                        }
                        ...
                        ContentUpdateAction::Curseforge { file, project_id } => {
                            ...
                            let Some(sha1) = sha1 else {
                                self.send
                                    .send_error("Can't update mod in instance, missing sha1 hash");
                                modal_action.set_finished();
                                return;
                            };
                    ...
                    self.send
                        .send_error("Can't update mod in instance, unknown instance id");
                    modal_action.set_finished();
                    return;
                };
            ...
            MessageToBackend::UnzipModpack {
                ...
                if let Err(err) = std::fs::remove_file(modpack_path) {
                    self.send
                        .send_error(format!("Unable to delete original modpack: {err}"));
                }
            ...
            MessageToBackend::GetImportFromOtherLauncherJob {
                ...
                let result =
                    crate::launcher_import::get_import_from_other_launcher_job(launcher, path);
            ...
            MessageToBackend::GetSyncState { channel } => {
                ...
                match result {
                    ...
                    Err(error) => {
                        self.send
                            .send_error(format!("Error while getting sync state: {error}"));
                    }
                }
            }
            MessageToBackend::SetSyncing {
                target,
                is_file,
                value,
            } => {
                ...
                match result {
                    ...
                    Err(error) => {
                        self.send
                            .send_error(format!("Error while enabling syncing: {error}"));
                        return;
                    }
                }
                ...
                write.modify(|config| {
                    let (set, other_set) = if is_file {
                        (
                            &mut config.sync_targets.files,
                            &mut config.sync_targets.folders,
                        )
                    } else {
                        (
                            &mut config.sync_targets.folders,
            ...
            MessageToBackend::UploadLogFile { path, modal_action } => {
                ...
                if replaced.trim_ascii().is_empty() {
                    modal_action
                        .set_finished_with_error("Log file was empty, didn't upload".into());
                    return;
                }
                ...
                if response.success {
                    if let Some(url) = response.url {
                        ...
                        modal_action
                            .set_finished_with_error("Success returned, but missing url".into());
                    }
                } else {
                    if let Some(e) = response.error {
                        ...
                        modal_action
                            .set_finished_with_error("Failure returned, but missing error".into());
                    }
                }
                    ...
                    .await
                {
                    Ok(storage) => {
                        if password.is_empty() {
                            ...
                            self.send
                                .send_error("Failed to save proxy password to system keyring");
                            return;
                        }
                    }
                    Err(e) => {
                        ...
                        self.send
                            .send_error("Failed to access system keyring for proxy password");
                        return;
                    }
                }
            ...
            MessageToBackend::CreateInstanceShortcut { id, path } => {
                if let Some(instance) = self.instance_state.write().instances.get_mut(id) {
                    ...
                    crate::shortcut::create_shortcut(
                        path,
            ...
            MessageToBackend::RelocateInstance { id, path } => {
                ...
                {
                    self.send
                        .send_warning(format!("Cannot relocate instance: {err}"));
                    return;
                }
                ...
                if let Some(instance) = self.instance_state.write().instances.get_mut(id) {
                    if cfg!(windows) {
                        self.file_watching
                            .write()
                            .unwatch_subdirectories_of_instance(id);
                    ...
                    if let Ok(target) = junction::get_target(&instance.root_path) {
                        if let Err(err) =
                            crate::fs::rename_with_fallback_across_devices(&target, &path)
                        {
                            log::error!(
                                "Unable to move instance files from {target:?} to {path:?}: {err:?}"
                            );
                            self.send
                                .send_error(format!("Unable to move instance files: {err}"));
                            return;
                        }
                        ...
                        if !is_normal_instance_folder {
                            if let Err(err) = junction::create(&path, &instance.root_path) {
                                log::error!(
                                    "Error while creating junction to moved instance: {err:?}"
                                );
                    ...
                    if let Ok(target) = std::fs::read_link(&instance.root_path) {
                        ...
                        if !is_normal_instance_folder {
                            ...
                            if let Err(err) = std::os::unix::fs::symlink(&path, &instance.root_path)
                            {
                                ...
                                self.send.send_error(format!(
                                    "Error while linking to moved instance: {err}"
                                ));
                                return;
                            }
                            #[cfg(windows)]
                            if let Err(err) =
                                std::os::windows::fs::symlink_dir(&path, &instance.root_path)
                            {
                                log::error!("Error while linking to moved instance: {err:?}");
                                self.send.send_error(format!(
                                    "Error while linking to moved instance: {err}"
                    ...
                    if let Err(err) =
                        crate::fs::rename_with_fallback_across_devices(&instance.root_path, &path)
                    {
            ...
            MessageToBackend::ImportFromOtherLauncher {
                ...
                crate::launcher_import::import_from_other_launcher(
                    self,
                    launcher,
                    import_job,
                    modal_action,
            ...
            MessageToBackend::SetAccountSkin {
                account,
                skin,
                variant,
            } => {
                ...
                let response = match response {
                    ...
                    Err(err) => {
                        ...
                        self.send
                            .send_error("Error while making skin change request");
                        return;
                    }
                };
                ...
                if status != reqwest::StatusCode::OK {
                    ...
                    if let Ok(response) = response.json::<MojangApiResponse>().await {
                        ...
                        self.send
                            .send_error(format!("Skin change failed: {}", &response.error_message));
                    } else {
                        log::error!("Skin change failed with non-200 status code: {}", status);
                        self.send.send_error(format!(
                            "Skin change failed with non-200 status code: {}",
                            status
                        ));
                    }
                    return;
            ...
            MessageToBackend::SetAccountCape { account, cape } => {
                ...
                let response = match response {
                    ...
                    Err(err) => {
                        ...
                        self.send
                            .send_error("Error while making cape change request");
                        return;
                    }
                };
                ...
                if status != reqwest::StatusCode::OK {
                    ...
                    if let Ok(response) = response.json::<MojangApiResponse>().await {
                        ...
                        self.send
                            .send_error(format!("Cape change failed: {}", &response.error_message));
                    } else {
                        log::error!("Cape change failed with non-200 status code: {}", status);
                        self.send.send_error(format!(
                            "Cape change failed with non-200 status code: {}",
                            status
                        ));
                    }
                    return;
            ...
            MessageToBackend::AddToSkinLibrary { source } => {
                    ...
                    bridge::message::UrlOrFile::Url { url } => {
                        ...
                        let response = match response {
                            ...
                            Err(err) => {
                                ...
                                self.send.send_error(
                                    "Error while requesting skin, see logs for more details",
                                );
                                return;
                            }
                        };
                        ...
                        let bytes = match response.bytes().await {
                            ...
                            Err(err) => {
                                ...
                                self.send.send_error(
                                    "Error while downloading skin, see logs for more details",
                                );
                                return;
                            }
                        };
                    ...
                    bridge::message::UrlOrFile::File { path } => {
                        let bytes = match std::fs::read(&path) {
                            ...
                            Err(err) => {
                                ...
                                self.send.send_error(
                                    "Error while reading skin file, see logs for more details",
                                );
                                return;
                            }
                        };
                ...
                if !SkinManager::is_valid_size(&image) {
                    self.send
                        .send_error("Invalid skin file. Must be 64x64 or 64x32.");
                    return;
                }
                ...
                let filename =
                    crate::fs::unique_name(&self.directories.skin_library_dir, &filename, false);
                ...
                if let Err(err) = crate::fs::write_safe(&path, &bytes) {
                    ...
                    self.send
                        .send_error("Error while saving skin, see logs for more details");
                }
            }
            MessageToBackend::CopyPlayerSkin { username } => {
                ...
                if response.status() == reqwest::StatusCode::NOT_FOUND {
                    self.send
                        .send_error(format!("Player '{}' not found", username));
                    return;
                }
                if !response.status().is_success() {
                    log::error!(
                        "CopyPlayerSkin: Mojang API returned status {}",
                        response.status()
                    );
                    self.send.send_error(format!(
                        "Failed to request Mojang API: status {}",
                        response.status()
                    ));
                    return;
                }
                let body = match response.text().await {
                    ...
                    Err(err) => {
                        log::error!(
                            "CopyPlayerSkin: failed to read Mojang API response: {:?}",
                            err
                        );
                ...
                let profile_lookup: serde_json::Value = match serde_json::from_str(&body) {
                    ...
                    Err(err) => {
                        log::error!(
                            "CopyPlayerSkin: failed to deserialize Mojang API response: {:?}",
                            err
                        );
                        self.send
                            .send_error("Failed to deserialize Mojang API response");
                        return;
                    }
                };
                    ...
                    .await
                {
                    ...
                    Err(err) => {
                        log::error!(
                            "CopyPlayerSkin: failed to request session server: {:?}",
                            err
                        );
                        self.send
                            .send_error("Failed to request Mojang session server");
                        return;
                    }
                };
                if !response.status().is_success() {
                    log::error!(
                        "CopyPlayerSkin: session server returned status {}",
                        response.status()
                    );
                    self.send.send_error(format!(
                        "Failed to request Mojang session server: status {}",
                        response.status()
                    ));
                    return;
                }
                let body = match response.text().await {
                    ...
                    Err(err) => {
                        log::error!(
                            "CopyPlayerSkin: failed to read session server response: {:?}",
                            err
                        );
                        self.send
                            .send_error("Failed to read Mojang session server response");
                        return;
                    }
                };
                ...
                let skin_url = match Self::extract_skin_url_from_profile(&body) {
                    ...
                    None => {
                        self.send
                            .send_error(format!("Player '{}' has no skin", username));
                        return;
                    }
                };
                    ...
                    .await
                {
                    ...
                    Err(err) => {
                        ...
                        self.send
                            .send_error("Error while requesting skin, see logs for more details");
                        return;
                    }
                };
                if !response.status().is_success() {
                    log::error!(
                        "CopyPlayerSkin: skin texture request returned status {}",
                        response.status()
                    );
                    self.send.send_error(format!(
                        "Failed to request skin texture: status {}",
                        response.status()
                    ));
                    return;
                }
                let bytes = match response.bytes().await {
                    ...
                    Err(err) => {
                        ...
                        self.send
                            .send_error("Error while downloading skin, see logs for more details");
                        return;
                    }
                };
                ...
                if !SkinManager::is_valid_size(&image) {
                    self.send
                        .send_error("Player skin has invalid dimensions. Must be 64x64 or 64x32.");
                    return;
                }
    ...
    async fn start_instance(
        ...
        live_game_output: Option<
            tokio::sync::oneshot::Sender<tokio::sync::mpsc::UnboundedReceiver<GameOutputMsg>>,
        >,
        ...
        let (dot_minecraft, configuration) =
            if let Some(instance) = self.instance_state.write().instances.get_mut(id) {
                ...
                {
                    modal_action
                        .set_finished_with_error("Can't launch instance, already launching".into());
                    return;
                }
        ...
        let Some(login_info) = self
            .get_login_info(&modal_action, configuration.preferred_account)
            .await
        else {
    ...
    pub async fn get_minecraft_profile(&self, account: Uuid) -> Option<MinecraftProfileResponse> {
        ...
        let credentials = secret_storage
            .read_credentials(account)
            .await
            .ok()
            .flatten()
            .unwrap_or_default();
        ...
        Some(
            self.noninteractive_login_flow_inner(account, credentials)
                .await?
                .0,
        )
    }
    ...
    pub async fn noninteractive_login_flow_inner(
        ...
        if let Err(LoginError::NeedsUserInteraction) | Err(LoginError::CancelledByUser) =
            login_result
        {
            return None;
        }
        ...
        if let Err(error) = secret_storage
            .write_credentials(profile.id, &credentials)
            .await
        {
            ...
            .await
        {
            ...
            Err(error) => {
                ...
                if let Some(modal_action) = modal_action {
                    modal_action.set_finished_with_error(
                        format!("Error initializing secret storage: {error}").into(),
                    );
                }
                return None;
            }
    ...
    pub async fn login_flow(
        ...
        if let Some(selected_account) = selected_account
            ...
            && let Some(cached_profile) =
                self.cached_minecraft_profiles.read().get(&selected_account)
        {
            let now = Instant::now();
        ...
        let login_result = self
            .login(&mut credentials, Some(&login_tracker), Some(&modal_action))
            .await;
    ...
    pub fn update_account_info_with_profile(
        &self,
        ...
        if info.accounts.contains_key(&profile.id)
            && (!select || info.selected_account == Some(profile.id))
        {
            drop(account_info);
            ...
            if let Some(arguments) = &version_info.arguments {
                for argument in arguments.game.iter() {
                    let value = match argument {
                        ...
                        LaunchArgument::Ruled(launch_argument_ruled) => {
                            &launch_argument_ruled.value
                        }
                    };
                    match value {
            ...
            for (jre_component, components) in &platform.components {
                ...
                for runtime_component in components {
                    ...
                    if !known_executable_path {
                        panic!(
                            "{}/{} doesn't contain known java executable",
                            jre_component, platform_name
                        );
                    }
                }
            }
