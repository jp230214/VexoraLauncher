...
pub async fn check_for_updates(http_client: reqwest::Client, send: FrontendHandle) {
    ...
    let url = format!(
        "{repository_url}/releases/latest/download/update_manifest_{}.json",
        std::env::consts::OS
    );
    ...
    let manifest_bytes = match response.bytes().await {
        ...
        Err(err) => {
            ...
            send.send_error(
                "Unable to download Pandora update manifest, see logs for more details",
            );
            return;
        }
    };
...
pub async fn install_update(
    ...
    if let Err(error) = install_update_inner(
        http_client,
        &dirs,
        send.clone(),
        update,
...
async fn install_update_inner(
    ...
    match update.install_type {
        ...
        UpdateInstallType::App(current_app_folder) => {
            let mut temp_extract = dirs
                .temp_dir
                .join(format!("app_unpack_{}", rand::thread_rng().next_u64()));
            ...
            let mut temp_backup = dirs
                .temp_dir
                .join(format!("app_backup_{}", rand::thread_rng().next_u64()));
            while temp_backup.exists() {
                log::warn!(
                    "Randomly generated app_backup folder exists... what are the chances? ({:?})",
                    temp_backup
                );
                temp_backup = dirs
                    .temp_dir
            ...
            let result =
                install_app_update(current_app_folder, &bytes, &temp_extract, &temp_backup);
...
fn add_new_extension(path: &Path) -> PathBuf {
    let mut new_exe_data =
        path.with_added_extension(format!("{}.new", rand::thread_rng().next_u64()));
...
fn launch_update_helper(old_exe_path: PathBuf, new_exe_path: PathBuf, new_exe_data: &Path) {
    ...
    log::info!(
        "Running with powershell.exe: {}",
        ps_command.to_string_lossy()
    );
...
fn install_app_update(
    ...
    let needs_authorization = match std::fs::rename(&current_app_folder, &temp_backup) {
        ...
        Err(err) => {
            ...
            return Err(
                "I/O error while backing up current .app, see logs for more details".into(),
            );
        }
    };
    ...
    if needs_authorization {
        ...
        let result = runas::Command::new("sh")
            .arg("-c")
            .arg(command)
            .gui(true)
            .status();
        ...
        if !success {
            ...
            return Err(
                "Error completing elevated .app installation, see logs for more details".into(),
            );
        }
    } else {
...
fn run_admin_powershell(script: &OsStr) -> std::process::ExitStatus {
    unsafe {
        ...
        if windows::Win32::UI::Shell::ShellExecuteExW(&mut sei).is_err()
            || sei.hProcess.is_invalid()
        {
