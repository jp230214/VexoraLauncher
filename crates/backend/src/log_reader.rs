...
static REPLACEMENTS: Lazy<[(Regex, &'static str); 7]> = Lazy::new(|| {
    [
        ...
        (
            regex::Regex::new(r#"SignedJWT: [^\s]+"#).unwrap(),
            "SignedJWT: *****",
        ),
        (
            regex::Regex::new(r#"Session ID is [^\s)]+"#).unwrap(),
            "Session ID is *****",
        ),
        (
            regex::Regex::new(r#"--accessToken, [^\s,]+"#).unwrap(),
            "--accessToken, *****",
        ),
...
pub fn start_game_output(
    ...
    std::thread::spawn(move || {
        ...
        let result = {
            let panic_result =
                std::panic::catch_unwind(move || log_reader.handle_output(&mut log_input));
            match panic_result {
                Ok(result) => result,
                Err(panic_error) => {
                    ...
                    let panic_message = format!(
                        "(Pandora) There was an error while reading the log: {panic_error_str}"
                    );
        ...
        if let Err(error) = result {
            let error_message =
                format!("(Pandora) There was an error while reading the log: {error}");
...
impl LogReader {
    ...
    fn skip_balanced_angle_brackets(
        mut depth: usize,
        input: &mut LogInput,
    ) -> Result<(), HandleOutputError> {
        loop {
        ...
        loop {
            ...
            match state {
                ElementParseState::ReadingName => {
                    let end = available
                        .iter()
                        .position(|b| is_xml_whitespace(*b) || *b == b'>');
                ...
                ElementParseState::ReadingKey => {
                    let end = available.iter().position(|b| {
                        is_xml_whitespace(*b)
                            || *b == b'>'
                            || *b == b'\''
                            || *b == b'"'
                ...
                ElementParseState::ReadingValueDoubleQuoted(key)
                | ElementParseState::ReadingValueSingleQuoted(key) => {
    ...
    fn apply_end_element(&mut self, name: &[u8]) -> Result<(), HandleOutputError> {
        match self.stack.last_mut() {
            Some(LogOutputState::Event { .. }) => {
                if name != b"log4j:Event" {
                    return Err(HandleOutputError::UnmatchedElement(
                        str::from_utf8(name)?.into(),
                    ));
                }
