...
use schema::{
    ...
    instance::{
        AUTO_LIBRARY_PATH_GLFW, AUTO_LIBRARY_PATH_OPENAL, InstanceConfiguration,
    ...
    version::{
        GameLibrary, GameLibraryArtifact, GameLibraryDownloads, GameLibraryExtractOptions,
        GameLogging, LaunchArgument, LaunchArgumentValue, MinecraftVersion, OsArch, OsName,
        PartialMinecraftVersion, Rule, RuleAction,
    },
...
use crate::{
    ...
    metadata::{
        items::{
            ...
            ForgeInstallerMavenMetadataItem, MinecraftVersionManifestMetadataItem,
            MinecraftVersionMetadataItem, MojangJavaRuntimeComponentMetadataItem,
            MojangJavaRuntimesMetadataItem, NeoforgeInstallerMavenMetadataItem,
        },
...
impl Launcher {
    ...
    pub async fn launch(
        ...
        let load_log_configuration =
            self.load_log_configuration(http_client, version_info.logging.as_ref());
        ...
        let child = launch_context
            .launch(&version_info, read_game_output)
            .await?;
        ...
        match instance_info.loader {
            Loader::Vanilla => {
                ...
                let Some(version) = versions
                    .versions
                    .iter()
                    .find(|v| v.id == instance_info.minecraft_version)
                else {
                    return Err(LaunchError::CantFindVersion(
                ...
                Ok((
                    self.meta
                        .fetch(MinecraftVersionMetadataItem(version))
                        .await?,
                    AddVanillaJar::Yes,
                ))
            }
            Loader::Fabric => {
                ...
                let fabric_launch = {
                    ...
                    async move {
                        let loader_version = if let Some(preferred_version) =
                            instance_info.preferred_loader_version
                        {
                ...
                let version_future = {
                    ...
                    async move {
                        ...
                        let Some(version) =
                            versions.versions.iter().find(|v| v.id == minecraft_version)
                        else {
            ...
            Loader::Forge => {
                ...
                let Some(loader_version) = instance_info.determine_forge_loader_version(&manifest)
                else {
            ...
            Loader::NeoForge => {
                ...
                let Some(loader_version) =
                    instance_info.determine_neoforge_loader_version(&manifest)
                else {
                    return Err(LaunchError::CantFindLoader {
    ...
    async fn create_forgelike_launch_version(
        ...
        let Some(version_link) = minecraft_versions
            .versions
            .iter()
            .find(|v| v.id == instance_info.minecraft_version)
        else {
        ...
        let Some(install_profile_file) = installer_zip.by_name("install_profile.json") else {
            return Err(LaunchError::MissingFileInZipError(Cow::Borrowed(
    ...
    async fn create_forgelike_install_version_modern(
        ...
        if &*install_profile.minecraft != instance_info.minecraft_version.as_str() {
            return Err(LaunchError::MismatchedLoaderVersions(
                install_profile.minecraft.clone(),
            ));
        }
            ...
            .filter_map(|library| {
                ...
                if let Some(builtin) = installer_zip.by_name(format!("maven/{}", artifact.path)) {
                    if !path_is_normal(artifact.path.as_str()) {
                        log::warn!(
                            "Refusing to install artifact with illegal path: {}",
                            artifact.path
                        );
                        return None;
                    }
        ...
        self.load_libraries(http_client, &libraries, modal_action, launch_tracker)
            .await?;
        ...
        data.insert(
            "MINECRAFT_JAR".into(),
            minecraft_jar_path.as_os_str().to_os_string(),
        );
        data.insert(
            "MINECRAFT_VERSION".into(),
            OsString::from(&*install_profile.minecraft),
        );
        // ROOT is omitted
        data.insert(
            "INSTALLER".into(),
        ...
        data.insert(
            "LIBRARY_DIR".into(),
            self.directories.libraries_dir.as_os_str().to_os_string(),
        );
        ...
        for processor in install_profile.processors.iter() {
            ...
            let Some(safe_jar_path) = SafePath::new(&relative_jar_path) else {
                log::error!(
                    "Unable to run processor, invalid path: {}",
                    relative_jar_path
                );
                processor_tracker.add_count(1);
                continue;
    ...
    async fn create_forgelike_install_version_legacy(
        ...
        if &*install_profile.install.minecraft != instance_info.minecraft_version.as_str() {
            return Err(LaunchError::MismatchedLoaderVersions(
                install_profile.install.minecraft.clone(),
            ));
        }
        ...
        // Read partial minecraft version
        let version: PartialMinecraftVersion = install_profile
            .version_info
            .into_partial_version(ForgeSide::Client);
        ...
        Ok((
            Arc::new(version.apply_to(&base_version)),
            AddVanillaJar::Yes,
        ))
    }
    ...
    async fn load_mojang_java_binary(
        ...
        let mut runtime_platform = runtimes
            .platforms
            .get(&platform)
            .ok_or(LoadJavaRuntimeError::UnknownPlatform)?;
        ...
        let result = do_java_runtime_load(
            http_client,
            runtime_component_dir,
            fresh_install,
            runtime,
            &java_runtime_tracker,
        )
        .await;
    ...
    async fn load_assets(
        ...
        let result =
            do_asset_objects_load(http_client, assets_index, assets_dir, &assets_tracker).await;
    ...
    async fn load_log_configuration(
        ...
        let Ok(_) = hex::decode_to_slice(client.file.sha1.as_str(), &mut expected_hash) else {
            log::error!(
                "Log configuration has invalid sha1: {}",
                client.file.sha1.as_str()
            );
            return None;
        };
    ...
    for (filename, contents) in &runtime.files {
        ...
        match contents {
            ...
            JavaRuntimeComponentFile::File {
                ...
                let mut expected_hash = [0u8; 20];
                let Ok(_) = hex::decode_to_slice(downloads.raw.sha1.as_str(), &mut expected_hash)
                else {
    ...
    pub fn collect_libraries(
        ...
        for library in libraries {
            ...
            let coordinate_id = if let Some(specifier) = coordinate.specifier {
                format!(
                    "{}:{}:{}",
                    coordinate.group_id, coordinate.artifact_id, specifier
                )
            } else {
    ...
    pub fn check_rule(&self, rule: &Rule) -> bool {
        if let Some(features) = &rule.features {
            ...
            if features.is_quick_play_singleplayer
                && !matches!(self.quick_play, Some(QuickPlayLaunch::Singleplayer(_)))
            {
                return false;
            }
            if features.is_quick_play_multiplayer
                && !matches!(self.quick_play, Some(QuickPlayLaunch::Multiplayer(_)))
            {
                return false;
            }
            if features.is_quick_play_realms
                && !matches!(self.quick_play, Some(QuickPlayLaunch::Realms(_)))
            {
                return false;
            }
        }
...
impl LaunchContext {
    pub async fn launch(
        ...
        #[cfg(target_os = "linux")]
        {
            if self
                .configuration
                .linux_wrapper
                .map(|w| w.use_discrete_gpu)
                .unwrap_or(true)
            {
                if let Err(err) = linux_gpu::use_discrete_gpu(&mut command) {
                    log::error!(
                        "Error while setting up environment variables for discrete gpu: {err:?}"
                    );
        ...
        let mut child = if self.configuration.sandbox {
            ...
            if !command::is_command_available("bwrap") {
                return Err(std::io::Error::new(
                    ...
                    "missing 'bwrap' command for sandbox",
                ));
            ...
            command
                .spawn_sandboxed(PandoraSandbox {
                    ...
                    description: Arc::from(OsStr::new(
                        "Sandbox for Minecraft instances run by Vexora Launcher",
                    )),
                    #[cfg(windows)]
        ...
        if let Some(system_libraries) = self.configuration.system_libraries {
            ...
            if system_libraries.override_openal {
                if let Some(path) = system_libraries
                    .openal
    ...
    fn resolve_expansion(&self, key: ArgumentExpansionKey) -> Cow<'_, OsStr> {
        match key {
            ...
            ArgumentExpansionKey::LauncherName => OsStr::new("VexoraLauncher").into(),
            ArgumentExpansionKey::LauncherVersion => OsStr::new("1.0.0").into(),
            ArgumentExpansionKey::Classpath => {
                std::env::join_paths(&self.classpath).unwrap().into()
            }
            ArgumentExpansionKey::AuthPlayerName => OsStr::new(&*self.login_info.username).into(),
            ArgumentExpansionKey::VersionName => {
                OsStr::new(&*self.configuration.minecraft_version).into()
            }
            ...
            ArgumentExpansionKey::AuthUuid => {
                OsString::from(self.login_info.uuid.as_hyphenated().to_string()).into()
            }
            ...
            ArgumentExpansionKey::ResolutionWidth => OsString::from(format!(
                "{}",
                self.rule_context.custom_resolution.unwrap().0
            ))
            .into(),
            ArgumentExpansionKey::ResolutionHeight => OsString::from(format!(
                "{}",
                self.rule_context.custom_resolution.unwrap().1
            ))
            .into(),
...
fn expand_forge_argument<'a>(
    argument: &'a str,
    map: &FxHashMap<String, OsString>,
) -> Cow<'a, OsStr> {
    let mut builder = OsString::new();
    let mut copied_to_builder = 0;
    for (i, character) in argument.char_indices() {
