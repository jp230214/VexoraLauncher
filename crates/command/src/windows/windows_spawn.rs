    ...
    Win32::{
        ...
        System::{
            ...
            Threading::{
                ...
                STARTF_FORCEONFEEDBACK, STARTF_USESTDHANDLES, STARTUPINFOEXW, STARTUPINFOW,
            },
        },
    },
    core::Free,
...
use crate::{
    ...
    process::PandoraProcess, spawner::SpawnContext, windows::windows_helpers,
};
    ...
    for (k, v) in env_map {
        if k.0.as_encoded_bytes().contains(&b'\0') || v.0.as_encoded_bytes().contains(&b'\0') {
            return Err(Error::new(
                ...
                "environment variable contained null byte",
            ));
        }
