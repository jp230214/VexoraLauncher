...
use crate::{
    entity::instance::InstanceEntries, get_unique_instance_name, is_valid_instance_name,
    modals::generic,
};
...
impl DuplicateInstanceModalState {
    pub fn new(
        ...
        let name_input_state =
            cx.new(|cx| InputState::new(window, cx).placeholder(default_name.clone()));
...
pub fn open_duplicate_instance(
    ...
    let state = cx.new(|cx| {
        DuplicateInstanceModalState::new(
            instance_id,
            instance_name,
            instances,
            backend_handle,
