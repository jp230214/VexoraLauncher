...
impl SkinsPage {
    ...
    fn request_account_skin(&mut self, uuid: Uuid, cx: &mut Context<Self>) {
        ...
        self.data
            .backend_handle
            .send(MessageToBackend::GetAccountSkin {
                account: uuid,
                result: send,
            });
        ...
        self.data
            .backend_handle
            .send(MessageToBackend::GetAccountCapes {
                account: uuid,
                result: send2,
            });
...
impl Render for SkinsPage {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        return self.render_page(window, cx);
    }
}

impl SkinsPage {
    /// Page body split out of `render` so the render entry itself keeps a
    /// tiny stack frame (the Skins page builds a very large element tree;
    /// see `render_controls` / `render_library` below for the pieces).
    fn render_page(&mut self, window: &mut Window, cx: &mut Context<Self>) -> AnyElement {
            ...
            && let Some((_, cape_url)) = &self.selected_cape
        {
            ...
            let bytes = window
                .use_asset::<DataAssetLoader>(&Resource::Uri(uri), cx)
                .flatten();
        ...
        if let Some(account) = &self.data.accounts.read(cx).selected_account {
            ...
            if account.offline {
                ...
                if let Some(AccountSkinResult::Success { skin, variant }) =
                    self.account_skins.get(&uuid)
                {
                                    ...
                                    .on_click({
                                        ...
                                        cx.listener(move |page, _, _, cx| {
                                            ...
                                            if page.active_cape != page.selected_cape {
                                                page.data.backend_handle.send(
                                                    MessageToBackend::SetAccountCape {
                                                        account: uuid,
                                                        cape: page
                                                            .selected_cape
                                                            .as_ref()
                    ...
                    if InterfaceConfig::get(cx).collapse_capes_in_skins_page {
                        library = library.child(
                            h_flex()
                                ...
                                .on_click(|_, _, cx| {
                                    InterfaceConfig::get_mut(cx).collapse_capes_in_skins_page =
                                        false;
                                }),
                        )
                    } else {
                        ...
                        let cape_buttons = capes.iter().enumerate().map(|(i, cape)| {
                            let selected =
                                self.selected_cape.as_ref().map(|(id, _)| *id) == Some(cape.id);
                            let active =
                                self.active_cape.as_ref().map(|(id, _)| *id) == Some(cape.id);
                            ...
                            let button = v_flex()
                                ...
                                .on_click({
                                    ...
                                    let uuid = cape.id;
                                    cx.listener(move |page, _, _, cx| {
                                        if page.selected_cape.as_ref().map(|(id, _)| *id)
                                            == Some(uuid)
                                        {
                                            page.select_no_cape(cx);
                                        } else {
                            ...
                            let bytes = cx
                                .fetch_asset::<DataAssetLoader>(&Resource::Uri(uri))
                                .0
                                .now_or_never()
                                .flatten();
                            if let Some(bytes) = bytes {
                                let cape_img = crate::png_render_cache::render_with_transform(
                                    bytes, TRANSFORM, cx,
                                );
        ...
        library = library
                                    ...
                                    let add_from_file_task = window.spawn(cx, async move |cx| {
                                        ...
                                        _ = cx.update_window_entity(
                                            ...
                                            move |this, window, cx| match result {
                                                Ok(Some(paths)) => {
                                                    for path in paths {
                                                        this.data.backend_handle.send(
                                                            MessageToBackend::AddToSkinLibrary {
            ...
            .child(
                h_flex()
                    ...
                    .gap_2()
                    .flex_wrap()
                    .children(skins.filter_map(|(i, skin)| {
                        ...
                        let variant = if active && let Some(v) = active_skin_variant {
                            ...
                            crate::skin_renderer::determine_skin_variant(skin)
                        ...
                        let skin_child: AnyElement = if show_3d {
                            let thumbnail = self
                                ...
                                .update(cx, |cache, cx| cache.get_or_queue(skin, variant, cx));
                        ...
                        Some(
                            div()
                                ...
                                .when_else(
                                    ...
                                    |this| {
                                        this.bg(secondary).hover(|style| style.bg(secondary_hover))
                                    },
                                )
                                .child(skin_child)
                                .when_else(
                                        ...
                                        this.child(
                                            Button::new("delete-skin")
                                                ...
                                                .on_click({
                                                    ...
                                                    cx.listener(move |page, _, window, cx| {
                                                        page.data.backend_handle.send(
                                                        MessageToBackend::RemoveFromSkinLibrary {
                                                            skin: { skin.clone() },
                                                        },
                                                    );
                                                        cx.stop_propagation();
                                                        ...
                                                        if skin == page.selected_skin {
                                                            if let Some(active_skin) =
                                                                active_skin.clone()
                                                            {
                                                                let variant = if let Some(
                                                                    active_skin_variant,
                                                                ) =
                                                                    active_skin_variant
                                                                {
                                ...
                                .on_click({
                                    ...
                                    cx.listener(move |page, _, _, cx| {
                                        let variant = if active
                                            && let Some(active_skin_variant) = active_skin_variant
                                        {
                                            active_skin_variant
                                        } else {
                                            crate::skin_renderer::determine_skin_variant(&skin)
                ...
                library
                    ...
                    .on_drop(cx.listener(|page, paths: &ExternalPaths, _window, _cx| {
                        for path in paths.paths() {
                            page.data
                                .backend_handle
                                .send(MessageToBackend::AddToSkinLibrary {
