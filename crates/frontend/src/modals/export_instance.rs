...
use bridge::{
    ...
    instance::InstanceID,
    message::{
        ExportCurseforgeOptions, ExportFormat, ExportModrinthOptions, ExportOptions,
        MessageToBackend,
    },
...
impl ExportInstanceModalState {
    pub fn new(
        ...
        cx.subscribe(&format_select_state, Self::on_format_selected)
            .detach();

        let version_input = cx.new(|cx| InputState::new(window, cx).placeholder("1.0.0"));
        let name_input =
            cx.new(|cx| InputState::new(window, cx).default_value(instance_name.clone()));
        ...
        let curseforge_recommended_ram_input =
            cx.new(|cx| InputState::new(window, cx).default_value("4096"));
    ...
    fn build_options(&self, cx: &mut App) -> ExportOptions {
        ...
        let recommended_ram = if self.curseforge_recommended_ram_enabled {
            self.curseforge_recommended_ram_input
                .read(cx)
                .value()
                .trim()
                .parse::<u32>()
    ...
    pub fn render(
        ...
        let modrinth_options = v_flex()
            .gap_2()
            .child(labelled(
                t::instance::export::name(),
                Input::new(&self.name_input),
            ))
            .child(labelled(
                t::instance::export::version(),
                Input::new(&self.version_input),
            ))
            .child(labelled(
                t::instance::export::summary(),
                Textarea::new(&self.modrinth_summary_input),
            ));
        ...
        let curseforge_options = v_flex()
            ...
            .child(labelled(
                t::instance::export::author(),
                Input::new(&self.curseforge_author_input),
            ))
            .child(
                h_flex()
                    .gap_2()
        ...
        let content = v_flex()
            ...
            .when(self.format == ExportFormat::Modrinth, |this| {
                this.child(labelled(
                    t::instance::export::modrinth_options(),
                    modrinth_options,
                ))
            })
            .when(self.format == ExportFormat::Curseforge, |this| {
                this.child(labelled(
                    t::instance::export::curseforge_options(),
                    curseforge_options,
                ))
            });
                                ...
                                move |_, window, cx| {
                                    ...
                                    let suggested = match format {
                                        ...
                                        ExportFormat::Modrinth => {
                                            format!("{}.mrpack", instance_name)
                                        }
                                        ExportFormat::Curseforge => {
                                            format!("{}.zip", instance_name)
                                        }
                                    };
