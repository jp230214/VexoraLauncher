use gpui::{
    AnyElement, AvailableSpace, Element, InteractiveElement, Interactivity, IntoElement,
    ParentElement, Pixels, Point, Size, StyleRefinement, Styled, size,
};
            ...
            |style, window, _cx| {
                ...
                window.request_measured_layout(
                    ...
                    move |known, available_space, _window, _cx| {
                        ...
                        let (width, horizontal_count) =
                            if base_width <= min_element_width || children_count == 0 {
                                (base_width, 1)
                            } else {
                                let bounds_width_plus_padding =
                                    base_width.to_f64() + gap_width.to_f64();
                                let min_element_width_plus_padding =
                                    min_element_width.to_f64() + gap_width.to_f64();
                                let horizontal_count =
                                    (bounds_width_plus_padding / min_element_width_plus_padding)
                                        .floor()
                                        .max(1.0) as usize;

                                let (element_width, horizontal_count) =
                                    if horizontal_count >= children_count {
                                        (min_element_width, children_count)
                                    } else {
                                        let padding_width = gap_width * (horizontal_count - 1);
                                        let width =
                                            (base_width - padding_width) / horizontal_count as f32;

                                        (width, horizontal_count)
                                    };

                                (
                                    element_width * children_count
                                        + gap_width * children_count.saturating_sub(1),
                                    horizontal_count,
                                )
                            };
    ...
    fn prepaint(
        ...
        self.interactivity.prepaint(
            ...
            |style, _scroll_offset, _hitbox, window, cx| {
                ...
                let min_element_width_plus_padding =
                    element_size.width.to_f64() + gap_width.to_f64();
                let horizontal_count = (bounds_width_plus_padding / min_element_width_plus_padding)
                    .floor()
                    .max(1.0) as usize;
