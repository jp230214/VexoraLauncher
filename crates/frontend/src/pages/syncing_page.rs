    ...
    pub fn update_sync_state(&mut self, cx: &mut Context<Self>) {
        ...
        self._get_sync_state_task = cx.spawn(async move |page, cx| {
            ...
            let _ = page.update(cx, move |page, cx| {
                page.loading
                    .retain(|loading| !page.pending.contains(loading));
        ...
        self.backend_handle
            .send(MessageToBackend::GetSyncState { channel: send });
    }
    ...
    pub fn create_entry(
        ...
        if is_loading {
            ...
            if (enabled || synced_count > 0) && !is_file {
                base = base.child(h_flex().gap_1().flex_shrink(1.0).text_color(info).child(
                    t::instance::sync::folders_count(synced_count, sync_state.total_count),
                ));
            }
            if enabled && cannot_sync_count > 0 {
                let cannot_sync_tooltip =
                    if let Some(sync_target_state) = sync_state.targets.get(&name) {
                        format!(
                            "{}\n{}",
                ...
                base = base.child(
                    Button::new(warning_id)
                        ...
                        .label(t::instance::sync::unable_count(
                            cannot_sync_count,
                            sync_state.total_count,
                        ))
        ...
        let content = v_flex()
            ...
            .child(
                h_flex()
                    ...
                    .gap_2()
                    .child(
                        Button::new("custom_file")
                            .label(t::instance::sync::sync_file())
                            .on_click(cx.listener(|page, _, window, cx| {
                                let picked = DialogBuilder::file()
                                    .set_title(t::instance::sync::sync_file().to_string())
                                    .set_location("~")
                                    .open_single_file()
                                    .show();
                                ...
                                if let Ok(Some(path)) = picked {
                                    if let Some(path_str) = path.to_str() {
                                        if let Some(safe) = SafePath::new(path_str) {
                                            let name: Arc<str> = path_str.into();
                    ...
                    .child(
                        Button::new("custom_folder")
                            .label(t::instance::sync::sync_folder())
                            .on_click(cx.listener(|page, _, window, cx| {
                                let picked = DialogBuilder::file()
                                    .set_title(t::instance::sync::sync_folder().to_string())
                                    .set_location("~")
                                    .open_single_dir()
                                    .show();
