...
impl InstanceConfiguration {
    ...
    pub fn determine_neoforge_loader_version(
        &self,
        ...
        let mut minecraft_version_parts =
            VersionFragment::string_to_parts(self.minecraft_version.as_str());
...
impl UpdateChannel {
    ...
    pub fn curseforge_release_types_with_fallback(
        self,
    ) -> &'static [&'static [CurseforgeReleaseType]] {
        match self {
            Self::Release => &[
...
fn is_default_wrapper_command_configuration(
    config: &Option<InstanceWrapperCommandConfiguration>,
) -> bool {
    if let Some(config) = config {
...
fn is_default_linux_wrapper_configuration(
    config: &Option<InstanceLinuxWrapperConfiguration>,
) -> bool {
    if let Some(config) = config {
        !config.use_mangohud
            && !config.use_gamemode
...
fn is_default_system_libraries_configuration(
    config: &Option<InstanceSystemLibrariesConfiguration>,
) -> bool {
    if let Some(config) = config {
        matches!(config.glfw, LwjglLibraryPath::Auto)
            && matches!(config.openal, LwjglLibraryPath::Auto)
    } else {
        true
    }
}
...
pub static AUTO_LIBRARY_PATH_GLFW: Lazy<Option<Arc<Path>>> =
    Lazy::new(|| get_shared_library_path_for_name("glfw"));
pub static AUTO_LIBRARY_PATH_OPENAL: Lazy<Option<Arc<Path>>> =
    Lazy::new(|| get_shared_library_path_for_name("openal"));
