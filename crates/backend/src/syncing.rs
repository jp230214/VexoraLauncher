...
pub fn apply_to_instance(
    ...
    for file_target in sync_targets.files.iter() {
        if &**file_target == "options.txt" {
            ...
            log::warn!(
                "Skipping file sync target because it is not a safe path: {}",
                file_target
            );
        }
    }
    ...
    for folder_target in sync_targets.folders.iter() {
        let Some(path) = SafePath::new(folder_target) else {
            log::warn!(
                "Skipping folder sync target because it is not a safe path: {}",
                folder_target
            );
            continue;
        };
...
fn create_combined_options_txt(
    fallback: &Path,
    current: &Path,
    instances: &mut BackendStateInstances,
) -> String {
...
pub fn get_sync_state(
    ...
    for instance in instances.instances.iter_mut() {
        if !instance.configuration.get().disable_file_syncing {
            syncable_instances.push((
                instance.name.as_str().into(),
...
pub fn enable_all(
    ...
    let Some(safe_path) = SafePath::new(name) else {
        log::warn!(
            "Skipping folder sync because it is not a safe path: {}",
            name
        );
        return Ok(false);
    };
