...
pub async fn start_server(
    ...
    log::info!(
        "Starting auth redirect server on {}",
    ...
    loop {
        ...
        loop {
            ...
            if url.path() == "/logo.svg" {
                // Vexora logo served from the exact user-supplied asset:
                // D:\Downloads\logo-without-bg.svg (copied to assets/branding/).
                const LOGO: &[u8] = include_bytes!("../../../assets/branding/logo-without-bg.svg");
                let header = format!(
                    "HTTP/1.1 200 OK\r\nContent-Type: image/svg+xml\r\nContent-Length: {}\r\nCache-Control: max-age=86400\r\n\r\n",
                    LOGO.len()
                );
                stream.write_all(header.as_bytes()).await?;
                stream.write_all(LOGO).await?;
                break;
            }
            ...
            if let Some(error) = error {
                let full_error = if let Some(error_description) = error_description {
                    ...
                    let response =
                        create_response(&format!("An error occurred: {}", &*error), "", true);
                    stream.write_all(response.as_bytes()).await?;
                    format!("An error occurred: {}", error)
                };
