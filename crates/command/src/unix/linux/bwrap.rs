...
use crate::{
    PandoraArg, PandoraChild, PandoraCommand, PandoraSandbox, spawner::SpawnContext,
    unix::unix_helpers::cvt,
};
...
pub fn spawn(
    ...
    command
        .args
        .insert(0, resolved_executable.clone().into_os_string().into());

    let Some(directories) = directories::BaseDirs::new() else {
        return Err(Error::new(
            ErrorKind::NotFound,
            "unable to determine base directories",
        ));
    };
    ...
    if let Some(xdg_runtime_dir) = directories.runtime_dir() {
        ...
        if let Some(pulse_server) = std::env::var_os("PULSE_SERVER") {
            ...
            {
                let pulse_server_path =
                    unsafe { OsStr::from_encoded_bytes_unchecked(pulse_server_path) };
    ...
    builder.bind_if_exists(
        BindType::ReadOnly,
        Path::new(&format!("/tmp/.X11-unix/X{display_index}")),
    );
...
fn start_dbus_proxy(sandbox_dir: &Path, context: &mut SpawnContext) -> std::io::Result<DbusProxy> {
    ...
    let Some(proxy_executable) = crate::path_cache::get_command_path(OsStr::new("xdg-dbus-proxy"))
    else {
        return Err(Error::new(
            ErrorKind::NotFound,
            "unable to find 'xdg-dbus-proxy'",
        ));
    };
...
fn create_seccomp_filter() -> std::io::Result<std::os::fd::OwnedFd> {
    ...
    let Ok(mut filter) = ScmpFilterContext::new(ScmpAction::Errno(libc::EPERM)) else {
        ...
        return Err(Error::new(
            ErrorKind::Other,
            "unable to init seccomp filter context",
        ));
    };
    ...
    unsafe {
        ...
        if filter
            .export_bpf(std::os::fd::BorrowedFd::borrow_raw(fd))
            .is_err()
        {
