...
fn try_load_from_curseforge(config_path: &Path) -> Option<InstanceConfiguration> {
    let instance_cfg_bytes = match std::fs::read(config_path) {
        ...
        Err(err) => {
            log::error!(
                "CurseForge import: Unable to read {:?}: {:?}",
                config_path,
                err
            );
            return None;
    ...
    let instance_cfg = match serde_json::from_slice::<CurseforgeInstance>(&instance_cfg_bytes) {
        ...
        Err(err) => {
            log::error!(
                "Unable to parse CurseforgeInstance {:?}: {:?}",
                config_path,
                err
            );
            return None;
    ...
    let (loader, preferred_loader_version) =
        if let Some(base_mod_loader) = instance_cfg.base_mod_loader {
            let loader = CurseforgeModLoaderType::from_u32(base_mod_loader.r#type);
            if loader == CurseforgeModLoaderType::Any {
                log::warn!(
                    "CurseForge import: unknown mod loader type id {}",
                    base_mod_loader.r#type
                );
                (Loader::Vanilla, None)
                ...
                && let Some(loader_version) = base_mod_loader.loader_version
            {
                ...
                (
                    loader.as_pandora().unwrap_or(Loader::Vanilla),
                    Some(preferred_loader_version),
                )
            } else {
