...
impl Render for InstancesPage {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let items_count = self
            .instance_table
            .update(cx, |table, cx| table.delegate().rows_count(cx));

        if items_count == 0 {
            return InstanceList::render_empty(cx).into_any_element();
        }
        ...
        match InterfaceConfig::get(cx).instances_view_mode {
            ...
            InstancesViewMode::List => {
                // NOTE: DataTable renders with size_full internally, so the
                // container must actually fill the available space. Without
                // flex_1/size_full here the table measured 0 height and the
                // List view appeared empty even though rows existed.
                div()
                    .p_4()
                    .flex_1()
                    .size_full()
                    .min_h_0()
                    .child(DataTable::new(&self.instance_table).bordered(false))
                    .into_any_element()
            }
        }
    }
