...
use crate::{
    ...
    instance::{
        ContentFolder, InstanceContentID, InstanceContentSummary, InstanceID, InstancePlaytime,
...
impl BridgeDataLoadState {
    ...
    pub fn set_observed(&self) {
        self.0
            .fetch_or(Self::OBSERVED, std::sync::atomic::Ordering::AcqRel);
    }

    pub fn set_dirty(&self) {
        self.0
            .fetch_or(Self::DIRTY, std::sync::atomic::Ordering::AcqRel);
    }

    pub fn load_started(&self) {
        self.0
            .store(Self::LOADING, std::sync::atomic::Ordering::Release);
    }

    pub fn load_finished(&self) {
        self.0
            .fetch_and(!Self::LOADING, std::sync::atomic::Ordering::AcqRel);
    }
}
...
pub enum AccountCapesResult {
    Success { capes: Vec<MinecraftProfileCape> },
    NeedsLogin,
}
