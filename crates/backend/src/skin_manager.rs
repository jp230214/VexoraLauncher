...
impl SkinManager {
    ...
    fn set_failed(skin_manager: Arc<RwLock<Self>>, skin_url: Arc<str>) {
        let previous = skin_manager
            .write()
            .skins_download
            .insert(skin_url, SkinEntry::Failed);

        if let Some(SkinEntry::Loading {
            frontend_requests, ..
        }) = previous
        {
    ...
    pub fn load_skin_library(backend: &Arc<BackendState>) {
        ...
        backend.file_watching.write().watch_filesystem(
            backend.directories.skin_library_dir.clone(),
            crate::WatchTarget::SkinLibraryDir,
        );
        ...
        if all_dirty {
            tokio::task::spawn_blocking(move || {
                ...
                let skins = skins
                    .into_iter()
                    .map(|(time, path, image, bytes)| {
                        (
                            time,
                            path,
                            skin_manager.create_skin(image.to_rgba8(), &bytes),
                        )
                    })
                    .collect::<Vec<_>>();
                skin_manager.skin_library =
                    skins.iter().map(|(_, _, bytes)| bytes.clone()).collect();
                skin_manager.skin_path_map = skins
                    .iter()
                    .map(|(_, path, bytes)| (bytes.clone(), path.clone()))
                    .collect();
