...
use gpui::{prelude::*, *};
use gpui_component::{
    ActiveTheme as _, Icon, InteractiveElementExt, StyledExt, WindowExt, h_flex,
...
use crate::{
    ...
    entity::{
        ...
        account::AccountExt,
        instance::{
            InstanceAddedEvent, InstanceEntries, InstanceModifiedEvent, InstanceMovedToTopEvent,
    ...
    pages::{
        curseforge_page::CurseforgeSearchPage, import::ImportPage,
        instance::instance_page::InstancePage, instances_page::InstancesPage,
        modrinth_page::ModrinthSearchPage, modrinth_project_page::ModrinthProjectPage, page::Page,
...
impl LauncherPage {
    fn render(self, ui: &LauncherUI, window: &mut Window, cx: &mut App) -> impl IntoElement {
        fn process(
            entity: Entity<impl Page>,
            window: &mut Window,
            cx: &mut App,
        ) -> (bool, AnyElement, AnyElement) {
        ...
        let page_path = PagePath::new(
            ui.data.clone(),
            config.main_page.clone(),
            config.page_path.clone(),
        );
        ...
        if scrollable {
            ...
            // Non-scrollable pages (Instances List View, Skins) manage their
            // own internal scrolling. They still need to fill the remaining
            // space, otherwise size_full children (e.g. DataTable) measure
            // zero height and render empty.
            v_flex().size_full().child(title_bar).child(
                div()
                    .flex_1()
                    .min_h_0()
...
impl LauncherUI {
    pub fn new(data: &DataEntities, window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        let _instance_added_subscription =
            cx.subscribe::<_, InstanceAddedEvent>(&data.instances, |this, _, event, cx| {
                ...
                let _ = this
                    .recent_instances
                    .insert(0, (event.instance.id, event.instance.name.clone()));
                cx.notify();
            });
        let _instance_modified_subscription = cx.subscribe_in::<_, InstanceModifiedEvent>(
            &data.instances,
            window,
            |this, _, event, window, cx| {
                if let Some((_, name)) = this
                    .recent_instances
                    .iter_mut()
                    .find(|(id, _)| *id == event.instance.id)
                {
        ...
        let _instance_removed_subscription = cx.subscribe_in::<_, InstanceRemovedEvent>(
            &data.instances,
            window,
            |this, _, event, window, cx| {
        ...
        let _instance_moved_to_top_subscription =
            cx.subscribe::<_, InstanceMovedToTopEvent>(&data.instances, |this, _, event, cx| {
                this.recent_instances
                    .retain(|entry| entry.0 != event.instance.id);
                if this.recent_instances.is_full() {
                    this.recent_instances.pop();
                }
                let _ = this
    ...
    fn create_page(
        ...
        match page {
            PageType::Instances => Ok(LauncherPage::Instances(
                cx.new(|cx| InstancesPage::new(data, window, cx)),
            )),
            PageType::Skins => Ok(LauncherPage::Skins(
                cx.new(|cx| SkinsPage::new(data, window, cx)),
            )),
            PageType::Modrinth { installing_for } => {
                ...
                if let Some(None) = installing_for {
                    return Err(PageType::Modrinth {
                        installing_for: None,
                    });
                }

                let page = cx
                    .new(|cx| ModrinthSearchPage::new(installing_for.flatten(), data, window, cx));
            ...
            PageType::Curseforge { installing_for } => {
                ...
                if let Some(None) = installing_for {
                    return Err(PageType::Curseforge {
                        installing_for: None,
                    });
                }
            ...
            PageType::Import => Ok(LauncherPage::Import(
                cx.new(|cx| ImportPage::new(data, window, cx)),
            )),
            PageType::Syncing => Ok(LauncherPage::Syncing(
                cx.new(|cx| SyncingPage::new(data, window, cx)),
            )),
            ...
            PageType::InstancePage { ref name } => {
                ...
                Ok(LauncherPage::InstancePage(
                    cx.new(|cx| InstancePage::new(id, data, window, cx)),
                ))
            }
        }
    }
        ...
        match Self::create_page(&self.data, page, window, cx) {
            Ok(page) => {
                ...
                if page_path.contains(&previous_page_type) {
                    self.previous_pages
                        .insert(previous_page_type, previous_page);
                }
                self.previous_pages.retain(|k, _| page_path.contains(k));
            }
            Err(fallback) => {
    ...
    pub fn nav_backwards(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let config = InterfaceConfig::get(cx);
        self.page_history_forwards
            .push((config.main_page.clone(), config.page_path.clone()));
...
impl Render for LauncherUI {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        // Vexora branding - use our own branding instead of Pandora
        let app_name = "VEXORA";
        let app_tagline = "LAUNCHER";

        // Library group — icon-led nav rows with active indicator pills.
        let library_group = MenuGroup::new("LIBRARY")
            .child(
                MenuGroupItem::new(t::instance::title())
                    .icon(PandoraIcon::Layers)
        ...
        // Content group
        let content_group = MenuGroup::new("CONTENT")
            .child(
                MenuGroupItem::new(t::modrinth::name())
                    .icon(PandoraIcon::Compass)
                    .active(matches!(
                        page_type,
                        PageType::Modrinth {
                            installing_for: None
        ...
        let files_group = MenuGroup::new("FILES")
            .child(
                MenuGroupItem::new(t::import::label())
                    .icon(PandoraIcon::Inbox)
        ...
        // Recent instances group
        if !self.recent_instances.is_empty() {
            let mut recent_instances_group = MenuGroup::new("RECENT");
        ...
        let (account_head, account_name) = if let Some(account) = &accounts.selected_account {
            ...
            let head = if hide_skins {
                gpui::img(ImageSource::Resource(Resource::Embedded(
                    "images/hidden_head.png".into(),
                )))
            } else if let Some(head) = &account.head {
                let resize = png_render_cache::ImageTransformation::ResizeToWidth {
                    width: 60,
                };
        ...
        // Sidebar header drag state - must be created before theme to avoid borrow conflicts
        let header_drag_state = window.use_keyed_state("sidebar-header-drag-state", cx, |_, _| {
            TitleBarState::default()
        });

        let theme = cx.theme();

        // Account card: avatar with status ring + two-line identity.
        // Structurally different from the old flat button: bordered card,
        // ringed avatar, chevron affordance.
        let account_button = h_flex()
            ...
            .rounded_lg()
            .border_1()
            .border_color(theme.sidebar_border.opacity(0.7))
            .bg(theme.sidebar_accent.opacity(0.45))
            .hover(|this| {
            ...
            .child(
                div()
                    ...
                    .child(account_head.size_8().min_w_8().min_h_8().rounded_full()),
            )
            .child(
                v_flex()
                    .flex_1()
                    ...
                    .child(
                        h_flex()
                            .gap_1()
                            .items_center()
                            .child(div().size_1().rounded_full().bg(theme.success))
                            .child(
                                div()
                                    .text_xs()
                                    .text_color(theme.muted_foreground)
                                    .child("Vexora Account"),
                            ),
                    ),
            )
            .child(
                Icon::new(PandoraIcon::ChevronsUpDown)
                    .size_4()
                    .text_color(theme.sidebar_foreground.opacity(0.6)),
            )
            .on_click({
        ...
        // Footer utility cells (rendered inside the segmented bar above)
        let settings_button = div()
            ...
            .child(
                Icon::new(PandoraIcon::Settings)
                    .size_4()
                    .text_color(theme.sidebar_foreground),
            )
            .tooltip(move |window, cx| Tooltip::new(t::settings::title()).build(window, cx))
            .on_click({
                let data = self.data.clone();
                move |_, window, cx| {
        ...
        let discord_invite =
            option_env!("DISCORD_INVITE").unwrap_or("https://discord.gg/vS8QHXarCk");
        ...
        // Segmented utility bar: one bordered strip, divided icon cells.
        let footer_buttons = h_flex()
            ...
            .child(div().w_px().h_6().bg(theme.sidebar_border.opacity(0.7)))
            .child(
                div()
                    .flex_1()
                    .flex()
            ...
            .when(!discord_invite.is_empty(), |this| {
                this.child(div().w_px().h_6().bg(theme.sidebar_border.opacity(0.7)))
                    .child(
                        div()
                            .flex_1()
                            .flex()
        ...
        // Sidebar header: logo tile + stacked wordmark + status line.
        // Structurally different from the old single-row logo: the mark now
        // sits in a rounded primary tile with a two-line wordmark beside it.
        let header = h_flex()
            .id("sidebar-header")
            ...
            .on_mouse_move(
                window.listener_for(&header_drag_state, |state, _, window, _| {
                ...
                div()
                    ...
                    .child(
                        gpui::img(gpui::ImageSource::Resource(Resource::Embedded(
                            "branding/logo-without-bg.svg".into(),
                        )))
                        .size_full(),
                    ),
            )
                        ...
                        h_flex()
                            ...
                            .child(
                                div()
                                    ...
                                    .text_xs()
                                    .font_semibold()
                                    .bg(theme.sidebar_primary.opacity(0.18))
        ...
        // Sidebar: header tile, hairline divider, grouped icon nav, footer.
        // Includes a right-edge hairline so the sidebar reads as a distinct
        // rail rather than a flat color block.
        let sidebar = h_flex()
            .size_full()
            .min_size_full()
            .max_size_full()
                ...
                v_flex()
                    ...
                    .child(
                        div()
                            .mx_3()
                            .h(px(1.0))
                            .bg(theme.sidebar_border.opacity(0.7)),
                    )
                    .child(
                        v_flex()
                            .flex_1()
            ...
            .child(div().w(px(1.0)).h_full().bg(theme.sidebar_border));

        // Main content area with subtle background
        let main_content = self.page.clone().render(&self, window, cx);

        ResizePanel::new(&self.sidebar_state, sidebar, main_content)
    }
}
...
"#,
    ...
    _ = writeln!(
        &mut body,
        "Version: {:?}",
        option_env!("PANDORA_RELEASE_VERSION")
    );
    _ = writeln!(
        &mut body,
        "Distributor: {:?}",
        option_env!("PANDORA_DISTRIBUTION")
    );
    _ = writeln!(
        &mut body,
        "OS: {} ({})",
    ...
    let Some(github) = option_env!("GITHUB_REPOSITORY_URL") else {
        let mut notification: Notification = (
            ...
            SharedString::from(
                "Unable to report bug, GITHUB_REPOSITORY_URL was not set at compile time",
            ),
        )
            .into();
