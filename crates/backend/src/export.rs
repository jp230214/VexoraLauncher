...
use crate::{
    ...
    metadata::{
        items::{
            CurseforgeFingerprintMetadataItem, FabricLoaderManifestMetadataItem,
            ForgeInstallerMavenMetadataItem, ModrinthProjectsMetadataItem,
            ModrinthVersionsFromHashesMetadataItem, NeoforgeInstallerMavenMetadataItem,
        },
    ...
    async fn determine_loader_version(&self, backend: &BackendState) -> Option<Ustr> {
        match self.configuration.loader {
            Loader::Fabric => backend
                ...
                .and_then(|manifest| {
                    self.configuration
                        .determine_fabric_loader_version(&manifest)
                }),
...
pub async fn export_instance(
    ...
    let result: Result<(), ExportError> = match format {
        ExportFormat::Zip => {
            export_instance_zip(&backend, &instance, &options, &output, &modal_action).await
        }
        ExportFormat::Modrinth => {
            export_modrinth_pack(&backend, &instance, &options, &output, &modal_action).await
        }
        ExportFormat::Curseforge => {
            export_curseforge_pack(&backend, &instance, &options, &output, &modal_action).await
        }
    };
...
async fn export_modrinth_pack(
    ...
    let resolved = resolve_modrinth_files(
        backend,
        instance,
        options,
        &files,
...
async fn export_curseforge_pack(
    ...
    let resolved = resolve_curseforge_files(
        backend,
        instance,
        options,
        &files,
...
fn should_skip(
    rel: &SafePath,
    rel_to_dot_minecraft: Option<&SafePath>,
    options: &ExportOptions,
) -> bool {
    // Exclude log artifacts regardless of where they are in the instance.
    if !options.include_logs {
        if let Some(file_name) = rel.file_name() {
            if ends_with_ignore_ascii_case(file_name, ".log")
                || ends_with_ignore_ascii_case(file_name, ".log.gz")
            {
                return true;
            }
        }
...
async fn resolve_modrinth_files(
    ...
    let mut versions_by_hash: HashMap<&str, &schema::modrinth::ModrinthProjectVersion> =
        HashMap::new();
    ...
    let projects_map: HashMap<Arc<str>, schema::modrinth::ModrinthProjectResult> =
        if project_ids.is_empty() {
            HashMap::new()
        } else {
            let ids_vec: Vec<Arc<str>> = project_ids.into_iter().collect();
            let req = ModrinthProjectsRequest {
                ids: ids_vec.into(),
            };
    ...
    for info in candidate_infos {
        ...
        if let Some(project) = projects_map.get(version.project_id.as_ref()) {
            let client = project
                .client_side
                .unwrap_or(ModrinthSideRequirement::Required);
            let server = project
                .server_side
                .unwrap_or(ModrinthSideRequirement::Required);
...
fn build_curseforge_manifest(
    ...
    obj.insert(
        "manifestType".into(),
        serde_json::Value::from("minecraftModpack"),
    );
    obj.insert("manifestVersion".into(), serde_json::Value::from(1));
    obj.insert(
        "name".into(),
        serde_json::Value::from(options.curseforge.name.as_ref()),
    );
    obj.insert(
        "version".into(),
        serde_json::Value::from(options.curseforge.version.as_ref()),
    );
    ...
    minecraft.insert(
        "version".into(),
        serde_json::Value::from(config.minecraft_version.as_str()),
    );
