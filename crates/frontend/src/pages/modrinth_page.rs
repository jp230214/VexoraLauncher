...
use schema::{
    ...
    loader::Loader,
    modrinth::{
        ModrinthHit, ModrinthProjectType, ModrinthSearchIndex, ModrinthSearchRequest,
...
pub struct ModrinthSearchPage {
    ...
    specific_installed_content_by_project:
        enum_map::EnumMap<ContentFolder, FxHashMap<Arc<str>, Vec<InstalledContent>>>,
...
pub fn env_display(
    ...
    match (client_side, server_side) {
        (ModrinthSideRequirement::Required, ModrinthSideRequirement::Required) => (
            PandoraIcon::Globe,
            t::modrinth::environment::client_and_server().into(),
        ),
        (ModrinthSideRequirement::Required, ModrinthSideRequirement::Unsupported) => (
            PandoraIcon::Computer,
            t::modrinth::environment::client_only().into(),
        ),
        (ModrinthSideRequirement::Required, ModrinthSideRequirement::Optional) => (
            PandoraIcon::Computer,
            t::modrinth::environment::client_only_server_optional().into(),
        ),
        (ModrinthSideRequirement::Unsupported, ModrinthSideRequirement::Required) => (
            PandoraIcon::Router,
            t::modrinth::environment::server_only().into(),
        ),
        (ModrinthSideRequirement::Optional, ModrinthSideRequirement::Required) => (
            PandoraIcon::Router,
            t::modrinth::environment::server_only_client_optional().into(),
        ),
        (ModrinthSideRequirement::Optional, ModrinthSideRequirement::Optional) => (
            PandoraIcon::Globe,
            t::modrinth::environment::client_or_server().into(),
        ),
        _ => (
            PandoraIcon::Cpu,
            t::modrinth::environment::unknown_environment().into(),
        ),
    }
}
...
impl ModrinthSearchPage {
    pub fn new(
        ...
        > = Default::default();
        let mut all_installed_content_by_project: FxHashMap<Arc<str>, Vec<InstalledContent>> =
            FxHashMap::default();
                ...
                for content_folder in ContentFolder::iter() {
                    ...
                    if let Some(content) = instance_content[content_folder].read(cx) {
                        for summary in content.iter() {
                            ...
                            let installed = all_installed_content_by_project
                                .entry(project_id.clone())
                                .or_default();
                            installed.push(installed_content);

                            let installed = specific_installed_content
                                .entry(project_id.clone())
                                .or_default();
                            installed.push(installed_content);
                        }
                    ...
                    specific_installed_content_by_project[content_folder] =
                        specific_installed_content;

                    let content = instance_content[content_folder].clone();
                    cx.observe(&content, move |page, entity, cx| {
                        let specific =
                            &mut page.specific_installed_content_by_project[content_folder];
                        ...
                        for content_folder in ContentFolder::iter() {
                            for (key, value) in
                                &page.specific_installed_content_by_project[content_folder]
                            {
                                let installed = page
    ...
    fn set_filter_loaders(
        &mut self,
        loaders: EnumSet<Loader>,
        _window: &mut Window,
        cx: &mut Context<Self>,
    ) {
    ...
    fn load_more(&mut self, cx: &mut Context<Self>) {
        ...
        let is_mod = filter_project_type == ModrinthProjectType::Mod
            || filter_project_type == ModrinthProjectType::Modpack;
        if is_mod
        ...
        let data = FrontendMetadata::request(
            &self.data.metadata,
            MetadataRequest::ModrinthSearch(request),
            cx,
        );
        ...
        match result {
            FrontendMetadataResult::Loading => {
                let subscription = cx.observe(&data, |page, data, cx| {
                    let result: FrontendMetadataResult<ModrinthSearchResult> =
                        data.read(cx).result();
                    match result {
                        FrontendMetadataResult::Loading => {}
...
impl Render for ModrinthSearchPage {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let type_button_group = ButtonGroup::new("type")
            ...
            .on_click(
                cx.listener(|page, clicked: &Vec<usize>, window, cx| match clicked[0] {
        ...
        let loader_button_group = if filter_project_type == ModrinthProjectType::Mod
            || filter_project_type == ModrinthProjectType::Modpack
        {
            Some(
        ...
        let category = v_flex()
            ...
            .child(
                Button::new("toggle-categories")
                    ...
                    .on_click(move |_, _, _| {
                        show_categories
                            .store(!is_category_shown, std::sync::atomic::Ordering::Relaxed);
                    }),
            )
                            ...
                            Button::new(*id)
                                .child(
                                    h_flex()
                                        ...
                                        .child(
                                            t::modrinth::category::get(id, true)
                                                .unwrap_or("missing_translation"),
                                        ),
                                )
        ...
        let sort = v_flex()
            ...
            .child(
                Button::new("toggle-sort")
                    ...
                    .on_click(move |_, _, _| {
                        show_sort_options
                            .store(!is_sort_shown, std::sync::atomic::Ordering::Relaxed);
                    }),
            )
                    ...
                    ButtonGroup::new("sort_group")
                        ...
                        .children(ModrinthSearchIndex::iter().map(|search_index| {
                            Button::new(search_index.as_str())
                                .child(
                                    h_flex().w_full().justify_start().gap_2().child(
                                        t::modrinth::sort::get(search_index.as_str())
                                            .unwrap_or("missing_translation"),
                                    ),
                                )
                        ...
                        .on_click(cx.listener(move |page, clicked: &Vec<usize>, window, cx| {
                            let sort_option = ModrinthSearchIndex::iter()
                                .nth(clicked[0])
                                .unwrap_or_default();
