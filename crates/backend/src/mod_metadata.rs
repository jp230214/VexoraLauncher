...
use bridge::{
    instance::{
        ContentSummary, ContentType, ContentUpdateStatus, ModpackFile, ModpackFilePath,
        ModpackFileSource, UNKNOWN_CONTENT_SUMMARY,
    },
    safe_path::SafePath,
};
...
impl ModMetadataManager {
    ...
    pub fn write_changes(&self) {
        if self.curseforge_info_dirty.swap(false, Ordering::AcqRel) {
            ...
            for (file_id, info) in self.cached_curseforge_info.read().iter() {
                ...
                data[checksum_start..checksum_start + 4]
                    .copy_from_slice(&u32::to_le_bytes(checksum));
                data[checksum_start + 4..checksum_start + 8]
                    .copy_from_slice(&u32::to_le_bytes(data_len as u32));
            }
            _ = crate::fs::write_safe(&self.cached_curseforge_info_dat, &data);
        }
        self.content_sources
            .write()
            .write_dirty_to_folder(&self.sources_dir);
    }
    ...
    pub fn set_cached_curseforge_info(&self, file_id: u32, info: CachedCurseforgeFileInfo) {
        ...
        if let Some(parents) = self
            .parents_by_missing_curseforge_id
            .write()
            .remove(&file_id)
        {
    ...
    pub fn get_bytes(
        ...
        let summary = self.load_mod_summary(
            actual_hash,
    ...
    fn load_mod_summary<R: rc_zip_sync::ReadZip>(
        ...
        for (zip_metadata_file, file) in candidates {
            let summary = match zip_metadata_file {
                ZipMetadataFile::McModInfo => {
                    self.load_legacy_forge_mod(hash, filesize, &archive, file)
                }
                ZipMetadataFile::FabricModJson => {
                    self.load_fabric_mod(hash, filesize, &archive, file)
                }
                ZipMetadataFile::ModsToml => {
                    self.load_forge_mod(hash, filesize, &archive, file, ContentType::Forge)
                }
                ...
                ZipMetadataFile::JavaManifest => {
                    self.load_from_java_manifest(hash, filesize, &archive, file)
                }
                ZipMetadataFile::PackMcmeta => {
                    self.load_from_pack_mcmeta(hash, filesize, &archive, file)
                }
                ZipMetadataFile::ModrinthIndexJson => {
                    self.load_modrinth_modpack(hash, filesize, &archive, file)
                }
                ZipMetadataFile::ManifestJson => {
                    self.load_curseforge_modpack(hash, filesize, &archive, file)
                }
            };
    ...
    fn load_fabric_mod<R: rc_zip_sync::HasCursor>(
        ...
        let name = fabric_mod_json
            .name
            .unwrap_or_else(|| Arc::clone(&fabric_mod_json.id));
        ...
        let icon = match fabric_mod_json.icon {
            Some(icon) => match icon {
                ...
                Icon::Sizes(hash_map) => {
                    ...
                    hash_map
                        .iter()
                        .min_by_key(|size| size.0.abs_diff(DESIRED_SIZE))
                        .map(|e| Arc::clone(e.1))
                }
            },
            None => None,
    ...
    fn load_forge_mod<R: rc_zip_sync::HasCursor>(
        ...
        let name = first
            .display_name
            .clone()
            .unwrap_or_else(|| Arc::clone(&first.mod_id));
    ...
    fn load_modrinth_modpack<R: rc_zip_sync::HasCursor>(
        ...
        for (mut path, bytes) in overrides {
            ...
            let (hash, summary) = self
                .load_child_content_summary_from_bytes(&bytes, path.extension().map(OsStr::new));
        ...
        let summaries = modrinth_index_json.files.par_iter().flat_map(|download| {
            ...
            let Some(first_download) = download.downloads.first() else {
                log::warn!(
                    "Skipping file {} because it has no downloads",
                    download.path
                );
                return None;
            };
            ...
            let Ok(_) = hex::decode_to_slice(&*download.hashes.sha1, &mut file_hash) else {
                log::warn!(
                    "Skipping file {} because of invalid sha1 hash: {}",
                    download.path,
            ...
            let summary = if let Some(cached) = self.by_hash.read().get(&file_hash).cloned() {
                ...
                if let Ok(mut file) = std::fs::File::open(&content_path) {
                    ...
                    let summary = self.load_mod_summary(
                        ...
                        filesize,
                        &mut file,
                        content_path.extension(),
                        false,
                    );
                    self.put(file_hash, summary.clone());
                    Some(summary)
                } else {
                    self.parents_by_missing_child
                        .write()
                        .entry(file_hash)
                        .or_default()
                        .insert(hash);
    ...
    fn load_child_content_summary_from_bytes(
        ...
        let summary =
            self.load_mod_summary(hash, Some(bytes.len() as u64), &bytes, extension, false);
    ...
    fn load_curseforge_modpack<R: rc_zip_sync::HasCursor>(
        ...
        let manifest_json: CurseforgeModpackManifestJson =
            serde_json::from_slice(&file.bytes().ok()?)
                .inspect_err(|e| {
        ...
        let summaries = known_files
            .par_iter()
            .flat_map(|(curseforge_file, cached_info)| {
                ...
                let summary = if let Some(cached) =
                    self.by_hash.read().get(&cached_info.hash).cloned()
                {
                    Some(cached)
                } else {
                    let content_path = crate::fs::create_content_library_path(
    ...
    fn load_jarjar<R: rc_zip_sync::HasCursor>(
        ...
        for child in &metadata_json.jars {
            ...
            let extension = child
                .path
                .rsplit_once('.')
                .map(|(_, last)| OsStr::new(last));
            let summary = self.load_mod_summary(
                hash,
                Some(child_bytes.len() as u64),
                &child_bytes,
                extension,
                true,
            );
    ...
    fn load_from_java_manifest<R: rc_zip_sync::HasCursor>(
        ...
        let author: Option<Arc<str>> =
            if let Some(impl_author) = manifest_map.get("Implementation-Vendor") {
...
fn load_icon<R: rc_zip_sync::HasCursor>(
    icon_file: rc_zip_sync::EntryHandle<R>,
) -> Option<UniqueBytes> {
        ...
        match source {
            ...
            ContentSource::ModrinthProject { project_id } => {
                ...
                if project_id.len() > 127 {
                    panic!(
                        "modrinth project id was unexpectedly big: {:?}",
                        &project_id
                    );
                }
