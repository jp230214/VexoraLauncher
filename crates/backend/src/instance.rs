...
use bridge::{
    instance::{
        ContentFolder, ContentSummary, ContentUpdateContext, ContentUpdateStatus,
        InstanceContentID, InstanceContentSummary, InstanceID, InstancePlaytime,
        InstanceServerSummary, InstanceStatus, InstanceWorldSummary,
    },
...
impl Instance {
    pub fn on_root_renamed(&mut self, backend: &Arc<BackendState>, path: &Path) {
        ...
        self.configuration = Persistent::load_or(
            path.join("info_v1.json").into(),
            self.configuration.get().clone(),
        );
        self.stats =
            Persistent::load_or(path.join("stats_v1.json").into(), self.stats.get().clone());

        let mut dot_minecraft_path = path.to_owned();
        dot_minecraft_path.push(".minecraft");

        for content_folder in ContentFolder::iter() {
            self.content_state[content_folder].path =
                dot_minecraft_path.join(content_folder.folder_name()).into();
    ...
    pub fn rewatch_directories(&mut self, file_watching: &mut BackendStateFileWatching) {
        ...
        if self.worlds_state.is_not_unloaded() {
            file_watching.watch_filesystem(
                self.saves_path.clone(),
                WatchTarget::InstanceSavesDir { id: self.id },
            );
            watch_dot_minecraft = true;
        }
    ...
    pub fn try_get_content(
        &self,
        id: InstanceContentID,
    ) -> Option<(&InstanceContentSummary, ContentFolder)> {
    ...
    fn load_worlds_inner(
        ...
        async move {
            ...
            let (future, keep_alive) = loop {
                ...
                file_watching.watch_filesystem(
                    this.saves_path.clone(),
                    WatchTarget::InstanceSavesDir { id: this.id },
                );
    ...
    fn load_worlds_all(saves_path: &Path) -> Arc<[InstanceWorldSummary]> {
        ...
        for entry in directory {
            ...
            let Ok(entry) = entry else {
                log::error!(
                    "Error reading directory in saves folder: {:?}",
                    entry.unwrap_err()
                );
                continue;
            };
    ...
    pub async fn reorder_servers(
        ...
        let raw = match std::fs::read(&server_dat_path) {
            ...
            Err(err) => {
                ...
                backend
                    .send
                    .send_error("Error while reading server.dat: {err}");
                return;
            }
        };
        ...
        let mut result = match nbt::decode::read_named(&mut nbt_data) {
            ...
            Err(err) => {
                ...
                backend
                    .send
                    .send_error("Error while decoding server.dat: {err}");
                return;
            }
        };
        ...
        if servers.move_index(from_index, to_index) {
            ...
            if let Err(err) = crate::fs::write_safe(&server_dat_path, &bytes) {
                ...
                backend
                    .send
                    .send_error("Error while writing server.dat: {err}");
                return;
            }
        }
    }
    ...
    fn load_servers_inner(
        ...
        async move {
            ...
            backend
                .send
                .send(MessageToFrontend::InstanceServersUpdated {
                    id,
    ...
    fn load_content_inner(
        ...
        async move {
            ...
            } else if content_folder == ContentFolder::Mods
                && !this.configuration.get().show_shader_tab
            {
            ...
            backend
                .send
                .send(MessageToFrontend::InstanceContentUpdated {
                    id,
                    content_folder,
    ...
    fn load_content_all(
        ...
        for entry in directory {
            let Ok(entry) = entry else {
                log::error!(
                    "Error reading file in content folder: {:?}",
                    entry.unwrap_err()
                );
                continue;
            };

            if let Some(summary) = create_instance_content_summary(
                &entry.path(),
                &mod_metadata_manager,
                for_loader,
                for_version,
    ...
    pub fn mark_content_dirty(
            ...
            if total_aux_paths > 0 {
                ...
                if let Some(summaries) = &state.summaries {
                    for summary in summaries.iter() {
                        let Some(aux_path) = crate::fs::pandora_aux_path_for_content(&summary)
                        else {
                            continue;
                        };
        ...
        if reload && state.load_state.should_load() {
            tokio::task::spawn(Self::load_content_inner(
                backend.clone(),
                self.id,
                content_folder,
            ));
    ...
    pub fn playtime(&mut self) -> InstancePlaytime {
        ...
        InstancePlaytime {
            total_secs: stats
                .total_playtime_secs
                .saturating_add(current_session_secs),
...
fn create_instance_content_summary(
    ...
    if !path.is_file() {
        ...
        if let Ok(pack_mcmeta_bytes) = std::fs::read(path.join("pack.mcmeta")) {
            ...
            return try_load_resourcepack_folder(
                &pack_mcmeta_bytes,
                pack_png_bytes.as_deref(),
                path,
            );
        }
    ...
    let content_source = mod_metadata_manager
        .read_content_sources()
        .get(&summary.hash)
        .unwrap_or_default();
...
fn load_world_summary(path: &Path) -> anyhow::Result<InstanceWorldSummary> {
    ...
    let root = result
        .as_compound()
        .context("Unable to get root compound")?;
    let data = root.find_compound("Data").context("Unable to get Data")?;
    let last_played: i64 = data
        .find_numeric("LastPlayed")
        .context("Unable to get LastPlayed")?;
...
fn load_servers_summary(
    ...
    for server in servers.iter() {
        ...
        if icon.is_none() {
            icon = server.find_string("icon").and_then(|v| {
                base64::engine::general_purpose::STANDARD
                    .decode(v)
                    .map(UniqueBytes::from)
                    .ok()
