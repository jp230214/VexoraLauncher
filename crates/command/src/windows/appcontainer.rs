...
use windows::{
    Win32::{
        Foundation::{
            ERROR_ACCESS_DENIED, ERROR_ALREADY_EXISTS, ERROR_INSUFFICIENT_BUFFER, ERROR_SUCCESS,
        ...
        Security::{
            ...
            Authorization::{
                ...
                GetNamedSecurityInfoW, GetSecurityInfo, NO_MULTIPLE_TRUSTEE, SE_FILE_OBJECT,
                SE_WINDOW_OBJECT, SetEntriesInAclW, SetNamedSecurityInfoW, SetSecurityInfo,
                TRUSTEE_IS_GROUP, TRUSTEE_IS_SID,
            },
            CONTAINER_INHERIT_ACE, CreateWellKnownSid, DACL_SECURITY_INFORMATION,
            DeriveCapabilitySidsFromName, FreeSid, GetAce, InitializeSecurityDescriptor,
            ...
            SID_AND_ATTRIBUTES, SetFileSecurityW, SetSecurityDescriptorDacl, WELL_KNOWN_SID_TYPE,
            WinCapabilityInternetClientServerSid, WinCapabilityInternetClientSid,
            WinCapabilityPrivateNetworkClientServerSid,
        },
        Storage::FileSystem::{
            FILE_ALL_ACCESS, FILE_GENERIC_EXECUTE, FILE_GENERIC_READ, FILE_TRAVERSE, READ_CONTROL,
            WRITE_DAC,
        },
        System::{
...
use crate::{
    PandoraChild, PandoraCommand, PandoraSandbox, spawner::SpawnContext, windows::windows_spawn,
};
...
pub fn spawn(
    ...
    unsafe {
        InitializeProcThreadAttributeList(Some(lpproc_thread_attribute_list), 1, None, &mut lpsize)?
    };
    ...
    if sandbox.grant_network_access {
        ...
        owned_capabilities.push(OwnedCapability::new(
            WinCapabilityPrivateNetworkClientServerSid,
        )?);
    }
    ...
    parents.retain(|path| {
        if let Err(err) = add_to_acl(&app_container_sid, path, PermissionType::TraverseNoInherit) {
            let raw_access_denied =
                err.raw_os_error().unwrap_or(0) == HRESULT::from_win32(ERROR_ACCESS_DENIED.0).0;
            if err.kind() == ErrorKind::PermissionDenied || raw_access_denied {
                log::warn!(
                    "Lacking permission to allow traversal of {path:?}... will need to elevate"
                );
                return true;
            }
    ...
    pub fn new(known: WELL_KNOWN_SID_TYPE) -> std::io::Result<Self> {
        ...
        unsafe {
            CreateWellKnownSid(
                known,
                None,
                Some(PSID(sid_alloc.as_mut_ptr() as *mut _)),
                &mut size,
            )?
        };
        Ok(Self { sid_alloc })
...
fn create_app_container(sandbox: &PandoraSandbox) -> windows::core::Result<PSID> {
    ...
    let encoded_description = sandbox
        .description
        .encode_wide()
        .chain([0])
        .collect::<Vec<_>>();
    ...
    {
        result =
            unsafe { DeriveAppContainerSidFromAppContainerName(PCWSTR(encoded_name.as_ptr())) };
    }
