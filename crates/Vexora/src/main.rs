...
fn main() {
    ...
    let launcher_dir = data_dir.join("VexoraLauncher");
    ...
    if lockfile.try_lock().is_ok() {
        ...
        #[cfg(windows)]
        let bind = PlatformListener::bind(std::ffi::OsStr::new("vexora-launcher-socket"));
        ...
        let (backend_recv, backend_handle, frontend_recv, frontend_handle) =
        ...
        runtime.block_on(async {
            ...
            #[cfg(windows)]
            let connect =
                PlatformClientStream::connect(std::ffi::OsStr::new("pandora-launcher-socket"))
                    .await;
...
impl PlatformServerStream {
    ...
    ) -> std::pin::Pin<&mut tokio::net::windows::named_pipe::NamedPipeServer> {
...
impl PlatformClientStream {
    ...
    fn project(
        self: std::pin::Pin<&mut Self>,
    ) -> std::pin::Pin<&mut tokio::net::windows::named_pipe::NamedPipeClient> {
